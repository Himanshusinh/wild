...
const FAL_ERROR_MESSAGES: Record<string, (detail: FalErrorDetail) => string> = {
  ...
  image_too_large: (detail) => {
    const ctx = detail.ctx as
      | { max_height?: number; max_width?: number }
      | undefined;
  ...
  image_load_error: () =>
    "Failed to load the image. Please check the image format and try again.",
  file_download_error: () =>
    "Failed to download the file. Ensure the URL is publicly accessible.",
  face_detection_error: () =>
    "No face detected in the image. Please upload an image with a clearly visible face.",
  ...
  feature_not_supported: () =>
    "This feature combination is not supported by the selected model.",
  invalid_archive: () =>
    "The archive file is invalid or corrupted. Please check the file and try again.",
  ...
  audio_duration_too_long: (detail) => {
    const ctx = detail.ctx as
      | { max_duration?: number; provided_duration?: number }
      | undefined;
  ...
  audio_duration_too_short: (detail) => {
    const ctx = detail.ctx as
      | { min_duration?: number; provided_duration?: number }
      | undefined;
...
const extractFalEnvelope = (error: any): FalErrorEnvelope | null => {
  ...
  if (responseData) {
    ...
    if (body && typeof body === "object") {
      ...
      const detailArray = Array.isArray(body.detail)
        ? body.detail
        : Array.isArray(body.data?.detail)
          ? body.data.detail
          : [];
      ...
      const retryableHeader =
        headers["x-fal-retryable"] || headers["X-Fal-Retryable"];
      const retryable =
  ...
  const simpleMessage = toFriendlyFalMessage(
...
export const extractFalErrorMessage = (
  error: any,
  ...
  if (typeof error?.message === "string" && error.message.trim().length > 0)
    return toFriendlyFalMessage(error.message);
  return fallback;
};
...
export const showFalErrorToast = async (
  error: any,
  ...
  if (!envelope) {
    ...
    try {
      ...
      const message = toFriendlyFalMessage(
  ...
  try {
    ...
    const shouldSuppressProviderDetails =
      envelope.type === "downstream_service_unavailable" ||
      envelope.type === "downstream_service_error";
    ...
    const message =
      ...
      fallbackMessage ||
      "Request failed";
    let finalMessage = shouldSuppressProviderDetails
      ? "Try again later"
      : message;

    // Add retry hint for retryable errors
    if (
      !shouldSuppressProviderDetails &&
      payload.retryable &&
      type === "error"
    ) {
