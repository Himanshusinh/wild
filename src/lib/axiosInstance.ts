...
const isApiDebugEnabled = (): boolean => {
  try {
    if (typeof window !== "undefined" && (window as any).__API_DEBUG === true)
      return true;
...
axiosInstance.interceptors.request.use(async (config) => {
  try {
      ...
      try {
        ...
        if (hasHint && !hasSession) {
          if (isApiDebugEnabled())
            console.log(
              "[API][request-delay] auth_hint present, delaying 100ms for session cookie",
            );
      ...
      try {
        ...
        if (auth?.currentUser) {
          try {
            ...
            if (isApiDebugEnabled()) {
              console.warn(
                "[API][request] Failed to get fresh token, using stored token:",
                tokenError?.message,
              );
            }
          ...
          token = getStoredIdToken();
          if (isApiDebugEnabled()) {
            console.log("[API][request] No Firebase user, using stored token", {
              hasToken: !!token,
            });
          }
        }
        ...
        if (isApiDebugEnabled()) {
          console.warn(
            "[API][request] Error getting token, using stored token:",
            error?.message,
          );
        }
      }
    ...
    try {
      ...
      const base =
        (config.baseURL as string) || axiosInstance.defaults.baseURL || "";
      ...
      const isProtectedApi =
        path.startsWith("/api/") && path !== "/api/auth/session";
      ...
      if (isProtectedApi) {
        try {
          ...
          if (hasHint && !hasSession) {
            if (isApiDebugEnabled())
              console.log(
                "[API][request-delay] protected call while auth_hint present, delaying 100ms",
                { path },
              );
            await new Promise((r) => setTimeout(r, 100));
          }
      ...
      if (isProtectedApi) {
        ...
        if (!idToken && auth?.currentUser) {
          ...
          return auth.currentUser
            ...
            .then((fresh) => {
              ...
              if (isApiDebugEnabled())
                console.log("[API][attach-bearer][fresh]", {
                  path,
                  hasToken: Boolean(fresh),
                });
              config.headers = hdrs;
              return config;
            })
        ...
        if (idToken) {
          ...
          if (isApiDebugEnabled())
            console.log("[API][attach-bearer][cached]", {
              path,
              hasToken: true,
            });
        } else {
          if (isApiDebugEnabled())
            console.log("[API][attach-bearer][missing]", { path });
          // Log warning if token is missing for protected API
          console.warn(
            "[API][attach-bearer] No token available for protected API:",
            { path, url: raw, baseURL: base },
          );
        }
      }
    } catch (err) {
      ...
      console.error(
        "[API][attach-bearer] Error determining path or attaching token:",
        err,
        {
    ...
    try {
      ...
      if (isApiDebugEnabled()) {
        ...
        console.warn(
          "[API][request] Missing Authorization header for protected route:",
          {
            method: (config.method || "get").toUpperCase(),
            url,
            baseURL: base,
  ...
  try {
    ...
    if (auth?.currentUser) {
      ...
      try {
        ...
        console.error(
          "[isUserAuthenticated] Failed to set auth_hint cookie:",
          e,
        );
      }
      return true;
    ...
    console.error(
      "[isUserAuthenticated] Error checking authentication:",
      error,
    );
    return false;
  }
...
export async function ensureSessionReady(
  ...
  try {
    ...
    if (stored) {
      console.log(
        "[ensureSessionReady] Bearer token present; skipping session creation",
      );
      return true;
    }
    if (!auth?.currentUser) {
    ...
    try {
      ...
      try {
        console.log(
          "[ensureSessionReady] Clearing stored tokens and retrying...",
        );
...
const canCreateSession = (): boolean => {
  try {
    const fromStorage = Number(
      sessionStorage.getItem("session_last_create") || "0",
    );
    const last = Math.max(
      lastSessionCreateAt,
      isNaN(fromStorage) ? 0 : fromStorage,
    );
    return getNow() - last > SESSION_CREATE_COOLDOWN_MS;
  } catch {
    return getNow() - lastSessionCreateAt > SESSION_CREATE_COOLDOWN_MS;
  }
};
...
const refreshSessionIfNeeded = async (): Promise<void> => {
  ...
  if (isRefreshingSession) {
    if (isApiDebugEnabled())
      console.log("[API][session-refresh] Already refreshing, skipping");
    return;
  }
  ...
  if (now - lastSessionRefreshAt < SESSION_REFRESH_COOLDOWN_MS) {
    if (isApiDebugEnabled())
      console.log("[API][session-refresh] Cooldown active, skipping");
    return;
  }
  ...
  try {
    ...
    if (isApiDebugEnabled())
      console.log("[API][session-refresh] Starting automatic session refresh");
    ...
    if (!currentUser) {
      ...
      if (storedToken) {
        ...
        console.warn(
          "[API][session-refresh] Firebase user is null but stored token exists. Session cookie should still be valid.",
        );
        if (isApiDebugEnabled()) {
          console.log(
            "[API][session-refresh] Skipping refresh - session cookie should still work without Firebase user",
          );
        }
        return;
      } else {
        ...
        if (isApiDebugEnabled()) {
          console.warn(
            "[API][session-refresh] No current user and no stored token, cannot refresh",
          );
        }
        return;
      }
    ...
    try {
      ...
      console.warn(
        "[API][session-refresh] Failed to get fresh ID token, but session cookie may still be valid:",
        {
      ...
      if (storedToken) {
        ...
        if (isApiDebugEnabled()) {
          console.warn(
            "[API][session-refresh] No ID token and no stored token, cannot refresh",
          );
        }
        return;
      }
    ...
    if (!freshIdToken) {
      if (isApiDebugEnabled())
        console.warn("[API][session-refresh] Failed to get fresh ID token");
    ...
    if (refreshResponse.data?.responseStatus === "success") {
      ...
      if (isApiDebugEnabled())
        console.warn(
          "[API][session-refresh] Refresh response indicates failure",
    ...
    console.warn(
      "[API][session-refresh] Failed to refresh session (CRITICAL - potential logout cause):",
      {
    ...
    if (error?.response?.status === 401) {
      console.error(
        "[API][session-refresh] CRITICAL: Refresh endpoint returned 401 - session may be invalid",
      );
    }
  } finally {
...
axiosInstance.interceptors.response.use(
  ...
  async (error) => {
    ...
    try {
      ...
      const isNoSessionTokenError =
        errorMessage.includes("No session token") ||
        errorMessage.includes("Cookie not sent");
    ...
    if (status !== 401 || isAuthCredentialRoute) {
      ...
      if (status === 403 && errorCode && MODERATION_CODES.includes(errorCode)) {
        try {
          // Store moderation info in Redux so the modal can display the right message
          const { setModerationInfo } = await import("@/store/slices/uiSlice");
          store.dispatch(
            setModerationInfo({
              code: errorCode as any,
              message:
                errorData?.message || "Your account has been restricted.",
    ...
    try {
      ...
      if (!currentUser) {
        ...
        if (storedToken) {
          console.log(
            "[API][401][refresh] No Firebase user, but stored token exists - using stored token",
          );
        ...
        if (isApiDebugEnabled())
          console.warn(
            "[API][401][refresh] no currentUser and no stored token - retry once without session create",
          );
      ...
      try {
        ...
        console.warn(
          "[API][401][refresh] Failed to get fresh ID token, trying stored token:",
          tokenError?.message,
        );
        const storedToken = getStoredIdToken();
        if (storedToken) {
      ...
      if (!freshIdToken) {
        if (isApiDebugEnabled())
          console.warn(
            "[API][401][refresh] No ID token available - retry once",
          );
        await new Promise((r) => setTimeout(r, 200));
        return axiosInstance(original);
      }
        ...
        if (isSessionError && canCreateSession()) {
          try {
            ...
            if (sessionResponse.status === 200) {
              ...
              if (isApiDebugEnabled())
                console.log(
                  "[API][401][session-create] created (throttled), retrying original",
                );
              ...
              if (isApiDebugEnabled())
                console.warn(
                  "[API][401][session-create] failed with status:",
            ...
            console.error(
              "[API][401][session-create] Failed to create session:",
              {
          ...
          if (isApiDebugEnabled()) {
            if (!isSessionError) {
              console.log(
                "[API][401][session-create] skipped (not a session error)",
              );
            } else {
