...
export const GEMINI_ATTACHMENT_LIMITS = {
  ...
  audio: {
    maxCount: 1,
    maxBytes: 500 * 1024 * 1024,
    accept: 'audio/mpeg,audio/mp3,audio/wav,audio/x-wav,audio/ogg,audio/mp4,audio/x-m4a,audio/webm',
...
export function getChatModelLabel(modelId: string): string {
  return CHAT_MODELS.find((model) => model.id === modelId)?.label || 'Assistant';
}
