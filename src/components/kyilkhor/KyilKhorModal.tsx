...
import { KyilKhorHeader } from "./KyilKhorHeader";
import {
  INITIAL_STATE,
  KyilKhorState,
  StyleFamily,
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: KyilKhorState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: KyilKhorState, action: Action): KyilKhorState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: KyilKhorState): string {
  const family = KYILKHOR_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the kyil-khor center hierarchy clear; do not invent extra symbols, text, or deities unless requested.";
  ...
  return [
    ...
    "- Do not add or invent new text, scripts, mantras, letters, numbers, or seals unless explicitly requested in PROJECT INPUTS.",
    "- Do not add random ornamental clutter; ornament must remain structural and support center-surrounding hierarchy.",
    ...
    "- Keep the visible output aligned to WildMind's Kyil-khor style experience.",
  ]
    .filter(Boolean)
    .join("\n");
}

export function KyilKhorModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
      ...
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            ...
            source: "homepage-kyilkhor-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `kyilkhor-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `kyilkhor-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const handleOpenStudio = useCallback(async () => {
    ...
    const payload = {
      ...
      style: "Kyil-khor (Mandala)",
      ...(uploadedImages?.length ? { uploadedImages } : {}),
      metadata: {
        source: "homepage-kyilkhor-modal",
        kyilkhorVersion: state.style,
        kyilkhorModal: true,
      },
    };
  ...
  const familyMeta = KYILKHOR_PROMPT_FAMILIES[state.style];
  ...
  return (
    <div className="fixed inset-0 z-[160] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Kyil-khor Generator"
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. center dominance, palace enclosure, gates, sand-mandala grains"
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a kyil-khor center + palace + gates (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
