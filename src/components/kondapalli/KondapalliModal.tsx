...
import { KONDAPALLI_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/kondapalliPromptCatalog";
...
import { KondapalliHeader } from "./KondapalliHeader";
import {
  INITIAL_STATE,
  KondapalliState,
  StyleFamily,
...
const STYLE_TAG = "KondapalliToys";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: KondapalliState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: KondapalliState, action: Action): KondapalliState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: KondapalliState): string {
  const family = KONDAPALLI_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Do not invent extra figures, props, or environments; keep the toy-world minimal and coherent.";
  ...
  return [
    ...
    "- Keep the output carved-and-painted toy-world; avoid generic realism or generic cartoon.",
    "",
    "RENDER SETTINGS:",
    ...
    "- Keep the visible output aligned to WildMind's Kondapalli toy style experience.",
  ]
    .filter(Boolean)
    .join("\n");
}

export function KondapalliModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "kondapalli",
            style_version: state.style,
            source: "homepage-kondapalli-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `kondapalli-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `kondapalli-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const handleOpenStudio = useCallback(async () => {
    ...
    const payload = {
      ...
      style: "Kondapalli Toys",
      ...(uploadedImages?.length ? { uploadedImages } : {}),
      metadata: {
        source: "homepage-kondapalli-modal",
        kondapalliVersion: state.style,
        kondapalliModal: true,
      },
    };
  ...
  const familyMeta = KONDAPALLI_PROMPT_FAMILIES[state.style];
  ...
  return (
    <div className="fixed inset-0 z-[160] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Kondapalli Toys Generator"
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. miniature toy scene, hand-painted zones, carved mass"
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[280px] text-xs leading-relaxed text-white/10">
                    Describe a miniature toy scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
