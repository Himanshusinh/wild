...
import { MAJULI_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/majuli/majuliPromptCatalog";
...
import { MajuliHeader } from "./MajuliHeader";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: MajuliState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: MajuliState, action: Action): MajuliState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: MajuliState): string {
  const family = MAJULI_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep Majuli mask logic: wearable devotional mask-face construction, stage readability, and Bhaona theatre hierarchy.";
  ...
  return [
    ...
    "- Keep full-frame Majuli identity with mask-first character architecture, devotional theatre readability, and satra-grounded performance logic.",
    "- Preserve wearable shell-built mask behavior, handmade bamboo-clay-cloth memory, and role-coded theatrical face structures.",
    "- Keep performance-mask-first construction and avoid portrait realism, horror styling, or generic decorative mask treatment.",
    "",
    "RENDER SETTINGS:",
...
export function MajuliModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `majuli-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `majuli-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = MAJULI_PROMPT_FAMILIES[state.style];
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea disabled={state.panelState === "loading"}
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. deity/demon role type, facial extensions (horn/beak/jaw), Bhaona staging cues"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Majuli mask theatre scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
