...
import { KUSHMANDIMASK_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/kushmandimask/kushmandimaskPromptCatalog";
...
import { KushmandimaskHeader } from "./KushmandimaskHeader";
...
const STYLE_TAG = "Kushmandi Mask";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: KushmandimaskState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: KushmandimaskState, action: Action): KushmandimaskState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: KushmandimaskState): string {
  const family = KUSHMANDIMASK_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep ritual carving and Gomira expressive grammar.";
  ...
  return [
    ...
    "- Keep output in Kushmandi ritual mask carving and wood grammar.",
    "- Avoid smooth digital surfaces, plastic-like finishes, or non-expressive faces.",
...
export function KushmandimaskModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "kushmandimask",
            style_version: state.style,
            source: "homepage-kushmandimask-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `kushmandi-mask-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `kushmandi-mask-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = KUSHMANDIMASK_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Kushmandi Mask Generator"
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. fierce Kali mask, Gamari wood texture, bold ritual carving"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Kushmandi scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
