...
import { MADHUBANI_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/madhubaniPromptCatalog";
...
import { MadhubaniHeader } from "./MadhubaniHeader";
import {
  INITIAL_STATE,
  MadhubaniState,
  StyleFamily,
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: MadhubaniState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: MadhubaniState, action: Action): MadhubaniState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: MadhubaniState): string {
  const family = MADHUBANI_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Do not invent extra figures, props, or environments; keep the composition complete but not cluttered.";
  ...
  return [
    ...
    "- Madhubani rule: keep the field visually active and filled (no empty space), but do not add random stickers; filler must be structurally integrated.",
    "",
    "RENDER SETTINGS:",
    ...
    "- Keep the visible output aligned to WildMind's Madhubani style experience.",
  ]
    .filter(Boolean)
    .join("\n");
}

export function MadhubaniModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
      ...
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            ...
            source: "homepage-madhubani-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `madhubani-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `madhubani-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = MADHUBANI_PROMPT_FAMILIES[state.style];
  ...
  return (
    <div className="fixed inset-0 z-[160] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Madhubani Generator"
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. dense symbolic fillers, border enclosure, flat matte fills"
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Mithila/Madhubani scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
