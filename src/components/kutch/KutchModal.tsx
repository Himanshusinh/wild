...
import { KUTCH_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/kutch/kutchPromptCatalog";
...
import { KutchHeader } from "./KutchHeader";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: KutchState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: KutchState, action: Action): KutchState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: KutchState): string {
  const family = KUTCH_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep one Kutch branch logic: branch-specific stitches, density, and embroidery field order.";
  ...
  return [
    ...
    "- Keep full-frame Kutch branch identity with stitched cloth construction and branch-correct surface logic.",
    "- Preserve branch-specific stitch behavior, motif/geometry bias, and mirror rule (only if branch-correct).",
    "- Keep cloth-first embroidery construction and avoid branch-mixed decorative styling.",
    "",
    "RENDER SETTINGS:",
...
export function KutchModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `kutch-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `kutch-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = KUTCH_PROMPT_FAMILIES[state.style];
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea disabled={state.panelState !== "empty"}
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. choose branch (Rabari/Paako/Suf), stitch density, mirror rule"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Kutch embroidery scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
