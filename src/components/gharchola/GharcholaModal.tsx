...
import { GharcholaHeader } from "./GharcholaHeader";
import {
  INITIAL_STATE,
  type GharcholaState,
  type StyleFamily,
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: GharcholaState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: GharcholaState, action: Action): GharcholaState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: GharcholaState): string {
  const family = GHARCHOLA_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the GHARCHOLA textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within GHARCHOLA textile geometry and weaving logic.",
...
export function GharcholaModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
      ...
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            ...
            source: "homepage-gharchola-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `gharchola-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `gharchola-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = GHARCHOLA_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="GHARCHOLA Textile Generator"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an GHARCHOLA-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
