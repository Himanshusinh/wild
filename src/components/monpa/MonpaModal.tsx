...
import { MONPA_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/monpa/monpaPromptCatalog";
...
import { MonpaHeader } from "./MonpaHeader";
import {
  INITIAL_STATE,
  MonpaState,
  StyleFamily,
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: MonpaState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: MonpaState, action: Action): MonpaState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: MonpaState): string {
  const family = MONPA_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Do not invent extra figures, props, or environments; keep the woven logic minimal and plausible.";
  ...
  return [
    ...
    "- Keep the output textile-first (woven), not printed or embroidered.",
    "",
    "RENDER SETTINGS:",
    ...
    "- Keep the visible output aligned to WildMind's Monpa textile style experience.",
  ]
    .filter(Boolean)
    .join("\n");
}
...
export function MonpaModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `monpa-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `monpa-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Monpa Textile Generator"
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea disabled={state.panelState !== "empty"}
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. banded zoning, matte woven texture, integrated motifs"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[280px] text-xs leading-relaxed text-white/10">
                    Describe a woven pattern world (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
