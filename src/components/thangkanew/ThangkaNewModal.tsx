...
import { thangkanewPromptCatalog } from "@/app/view/HomePage/compo/styles/thangkanew/thangkanewPromptCatalog";
...
import { ThangkaNewHeader } from "./ThangkaNewHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, ThangkaNewState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "THANGKANEW";
...
function reducer(state: ThangkaNewState, action: Action): ThangkaNewState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: ThangkaNewState): string {
  const prompts = thangkanewPromptCatalog.prompts;
  const familyPrompt =
  ...
  return [
    ...
    state.inputMode === "image" && state.uploadedImage ? thangkanewPromptCatalog.promptI2I[state.style === "V1" ? "v1" : state.style === "V2" ? "v2" : "v3"].trim() : familyPrompt.trim(),
    "",
    "PROJECT INPUTS:",
    projectLine,
    "",
...
export function ThangkaNewModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "thangkanew",
            style_version: state.style,
            source: "homepage-thangkanew-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `thangkanew-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `thangkanew-${state.style}-${index + 1}`);
        toast.dismiss(t);
