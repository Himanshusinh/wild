...
import { KERALAMURAL_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/keralamural/keralamuralPromptCatalog";
...
import { KeralaMuralHeader } from "./KeralaMuralHeader";
import {
  INITIAL_STATE,
  type KeralaMuralState,
  type StyleFamily,
  type InputMode,
  type ModelId,
  type ImageCount,
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: KeralaMuralState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: KeralaMuralState, action: Action): KeralaMuralState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: KeralaMuralState): string {
  const family = KERALAMURAL_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the KERALA MURAL textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within KERALA MURAL textile geometry and weaving logic.",
...
export function KeralaMuralModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "keralamural",
            style_version: state.style,
            source: "homepage-keralamural-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `keralamural-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `keralamural-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = KERALAMURAL_PROMPT_FAMILIES[state.style];
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="KERALA MURAL Textile Generator"
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an KERALA MURAL-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
