...
import { EXPOSEDLATERITE_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/exposedlaterite/exposedlateritePromptCatalog";
...
import { ExposedLateriteHeader } from "./ExposedLateriteHeader";
import {
  INITIAL_STATE,
  type ExposedLateriteState,
  type StyleFamily,
  type InputMode,
  type ModelId,
  type ImageCount,
...
const STYLE_TAG = "ExposedLaterite";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: ExposedLateriteState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: ExposedLateriteState, action: Action): ExposedLateriteState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: ExposedLateriteState): string {
  const family = EXPOSEDLATERITE_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the EXPOSED LATERITE textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within EXPOSED LATERITE textile geometry and weaving logic.",
...
export function ExposedLateriteModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "exposedlaterite",
            style_version: state.style,
            source: "homepage-exposedlaterite-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `exposedlaterite-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `exposedlaterite-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = EXPOSEDLATERITE_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="EXPOSED LATERITE Textile Generator"
        ...
        <div className="grid min-h-0 flex-1 overflow-hidden lg:[grid-template-columns:420px_1fr]">
          <aside className="flex flex-col overflow-hidden border-r border-white/10 bg-[#0E0E12]">
            ...
            <div className="border-t border-white/[0.06] bg-[#0E0E12] px-5 py-4">
              ...
              >
                Generate EXPOSED LATERITE Textile
              </button>
            </div>
          </aside>
