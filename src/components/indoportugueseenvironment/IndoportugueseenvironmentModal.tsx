...
import { INDOPORTUGUESEENVIRONMENT_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/indoportugueseenvironment/indoportugueseenvironmentPromptCatalog";
...
import { IndoportugueseenvironmentHeader } from "./IndoportugueseenvironmentHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, IndoportugueseenvironmentState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "INDOPORTUGUESEENVIRONMENT";
...
function reducer(state: IndoportugueseenvironmentState, action: Action): IndoportugueseenvironmentState {
  switch (action.type) {
...
function buildPrompt(state: IndoportugueseenvironmentState): string {
  const family = INDOPORTUGUESEENVIRONMENT_PROMPT_FAMILIES[state.style];
...
export function IndoportugueseenvironmentModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const res = await dispatch(falGenerate({
        ...
        meta: { style_premium: true, style_key: "indoportugueseenvironment", style_version: state.style, source: "homepage-indoportugueseenvironment-modal" },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try { await downloadAllImageUrls(urls, `indoportugueseenvironment-${state.style}`); toast.dismiss(t); toast.success("Downloads started"); } 
  ...
  const handleSaveImage = useCallback(async (index: number) => {
    ...
    try { await downloadImageUrl(url, `indoportugueseenvironment-${state.style}-${index + 1}`); toast.dismiss(t); toast.success("Download started"); } 
    catch { toast.dismiss(t); toast.error("Save failed"); }
  }, [state.generatedImages, state.style]);

  if (!isOpen) return null;
  const familyMeta = INDOPORTUGUESEENVIRONMENT_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div role="dialog" aria-modal="true" aria-label="INDOPORTUGUESEENVIRONMENT Generator" className={`relative flex w-[min(1080px,calc(100vw-24px))] h-[min(760px,calc(100vh-24px))] flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#0E0E12]/95 shadow-[0_24px_70px_rgba(0,0,0,0.7)] ring-1 ring-white/[0.04] transition-all duration-300 ${isVisible ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-2 scale-[0.985]"}`}>
        <IndoportugueseenvironmentHeader style={state.style} onStyleChange={(s) => dispatchLocal({ type: "SET_STYLE", payload: s })} onClose={onClose} disabled={state.panelState === "loading"} />
        <div className="grid min-h-0 flex-1 overflow-hidden lg:[grid-template-columns:420px_1fr]">
          <aside className="flex flex-col overflow-hidden border-r border-white/10 bg-[#0E0E12]">
            <div className="flex flex-1 flex-col gap-5 overflow-y-auto px-5 py-5 [scrollbar-width:thin] [&::-webkit-scrollbar-thumb]:rounded [&::-webkit-scrollbar-thumb]:bg-white/[0.06] [&::-webkit-scrollbar]:w-1">
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  <UploadZone disabled={state.panelState === "loading"} uploadedImage={state.uploadedImage} onUpload={(v) => dispatchLocal({ type: "SET_UPLOADED_IMAGE", payload: v })} />
                  <textarea disabled={state.panelState === "loading"} value={state.imageNote} onChange={(e) => dispatchLocal({ type: "SET_IMAGE_NOTE", payload: e.target.value })} rows={3} placeholder="Optional notes..." className="disabled:opacity-50 disabled:cursor-not-allowed w-full resize-none rounded-xl border border-white/10 bg-[#13131a] px-4 py-3 text-[13px] leading-relaxed text-white/80 outline-none transition-colors placeholder:text-white/20 focus:border-white/20" />
                </div>
              )}
              <div className="flex flex-col gap-2">
            ...
            <div className="border-t border-white/[0.06] bg-[#0E0E12] px-5 py-4">
              <button type="button" onClick={() => void handleGenerate()} disabled={state.panelState === "loading"} className="w-full rounded-lg bg-[#2F6BFF] py-2.5 text-[12px] font-semibold text-white transition hover:bg-[#2F6BFF]/90 disabled:opacity-50">Generate INDOPORTUGUESEENVIRONMENT</button>
            </div>
          </aside>
            ...
            <div className="flex flex-1 flex-col overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              ...
              {state.panelState === "results" ? (
                <div className="flex flex-col gap-0 h-full flex-1">
                  <OutputGrid prompt={assembledPrompt} images={state.generatedImages} count={state.imageCount} ratio={state.ratio} onSaveImage={(i) => void handleSaveImage(i)} onExpandImage={(i) => { const url = state.generatedImages[i]; if (!url) return; setFullscreenUrl(url); }} />
                </div>
              ) : null}
            </div>
          </main>
