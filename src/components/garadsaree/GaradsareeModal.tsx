...
import { GARADSAREE_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/garadsaree/garadsareePromptCatalog";
...
import { GaradsareeHeader } from "./GaradsareeHeader";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: GaradsareeState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: GaradsareeState, action: Action): GaradsareeState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: GaradsareeState): string {
  const family = GARADSAREE_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep pure mulberry silk and red-border ritual grammar.";
  ...
  return [
    ...
    "- Keep output in Garad silk weaving and ritual purity grammar.",
    "- Avoid synthetic shine, digital vector sharpness, or heavy multicolor prints.",
...
export function GaradsareeModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "garadsaree",
            style_version: state.style,
            source: "homepage-garadsaree-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `garadsaree-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `garadsaree-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = GARADSAREE_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Garad Saree Generator"
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. pure silk texture, red temple border, cream-white field"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Garad scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
