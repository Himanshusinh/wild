...
import { GODNAART_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/godnaart/godnaartPromptCatalog";
...
import { GodnaArtHeader } from "./GodnaArtHeader";
import {
  INITIAL_STATE,
  type GodnaArtState,
  type StyleFamily,
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: GodnaArtState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: GodnaArtState, action: Action): GodnaArtState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: GodnaArtState): string {
  const family = GODNAART_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the GODNA ART textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within GODNA ART textile geometry and weaving logic.",
...
export function GodnaArtModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
      ...
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            ...
            source: "homepage-godnaart-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `godnaart-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `godnaart-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = GODNAART_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="GODNA ART Textile Generator"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an GODNA ART-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
