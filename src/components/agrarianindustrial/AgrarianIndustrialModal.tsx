...
import { AGRARIANINDUSTRIAL_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/agrarianindustrial/agrarianindustrialPromptCatalog";
...
import { AgrarianIndustrialHeader } from "./AgrarianIndustrialHeader";
import {
  INITIAL_STATE,
  type AgrarianIndustrialState,
  type StyleFamily,
  type InputMode,
  type ModelId,
  type ImageCount,
...
const STYLE_TAG = "AgrarianIndustrial";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: AgrarianIndustrialState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: AgrarianIndustrialState, action: Action): AgrarianIndustrialState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: AgrarianIndustrialState): string {
  const family = AGRARIANINDUSTRIAL_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the AGRARIAN–INDUSTRIAL BLEND textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within AGRARIAN–INDUSTRIAL BLEND textile geometry and weaving logic.",
...
export function AgrarianIndustrialModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "agrarianindustrial",
            style_version: state.style,
            source: "homepage-agrarianindustrial-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `agrarianindustrial-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `agrarianindustrial-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = AGRARIANINDUSTRIAL_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="AGRARIAN–INDUSTRIAL BLEND Textile Generator"
        ...
        <div className="grid min-h-0 flex-1 overflow-hidden lg:[grid-template-columns:420px_1fr]">
          <aside className="flex flex-col overflow-hidden border-r border-white/10 bg-[#0E0E12]">
            ...
            <div className="border-t border-white/[0.06] bg-[#0E0E12] px-5 py-4">
              ...
              >
                Generate AGRARIAN–INDUSTRIAL BLEND Textile
              </button>
            </div>
          </aside>
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an AGRARIAN–INDUSTRIAL BLEND-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
