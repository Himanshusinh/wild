...
import { motibharatPromptCatalog } from "@/app/view/HomePage/compo/styles/motibharat/motibharatPromptCatalog";
...
import { MotibharatHeader } from "./MotibharatHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, MotibharatState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "MOTI BHARAT";
...
function reducer(state: MotibharatState, action: Action): MotibharatState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: MotibharatState): string {
  const prompts = motibharatPromptCatalog.prompts;
  const familyPrompt =
  ...
  return [
    ...
    state.inputMode === "image" && state.uploadedImage ? motibharatPromptCatalog.promptI2I.trim() : familyPrompt.trim(),
    "",
    "PROJECT INPUTS:",
    projectLine,
    "",
...
export function MotibharatModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "motibharat",
            style_version: state.style,
            source: "homepage-motibharat-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `motibharat-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `motibharat-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Moti Bharat Generator"
