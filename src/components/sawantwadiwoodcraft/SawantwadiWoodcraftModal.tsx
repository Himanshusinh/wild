...
import { SAWANTWADIWOODCRAFT_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/sawantwadiwoodcraft/sawantwadiwoodcraftPromptCatalog";
...
import { SawantwadiWoodcraftHeader } from "./SawantwadiWoodcraftHeader";
import {
  INITIAL_STATE,
  type SawantwadiWoodcraftState,
  type StyleFamily,
  type InputMode,
  type ModelId,
  type ImageCount,
...
const STYLE_TAG = "SawantwadiWoodcraft";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: SawantwadiWoodcraftState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: SawantwadiWoodcraftState, action: Action): SawantwadiWoodcraftState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: SawantwadiWoodcraftState): string {
  const family = SAWANTWADIWOODCRAFT_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the SAWANTWADI WOODCRAFT textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within SAWANTWADI WOODCRAFT textile geometry and weaving logic.",
...
export function SawantwadiWoodcraftModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "sawantwadiwoodcraft",
            style_version: state.style,
            source: "homepage-sawantwadiwoodcraft-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `sawantwadiwoodcraft-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `sawantwadiwoodcraft-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = SAWANTWADIWOODCRAFT_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="SAWANTWADI WOODCRAFT Textile Generator"
        ...
        <div className="grid min-h-0 flex-1 overflow-hidden lg:[grid-template-columns:420px_1fr]">
          <aside className="flex flex-col overflow-hidden border-r border-white/10 bg-[#0E0E12]">
            ...
            <div className="border-t border-white/[0.06] bg-[#0E0E12] px-5 py-4">
              ...
              >
                Generate SAWANTWADI WOODCRAFT Textile
              </button>
            </div>
          </aside>
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an SAWANTWADI WOODCRAFT-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
