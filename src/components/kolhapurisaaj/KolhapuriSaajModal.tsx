...
import { KOLHAPURISAAJ_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/kolhapurisaaj/kolhapurisaajPromptCatalog";
...
import { KolhapuriSaajHeader } from "./KolhapuriSaajHeader";
import {
  INITIAL_STATE,
  type KolhapuriSaajState,
  type StyleFamily,
  type InputMode,
  type ModelId,
  type ImageCount,
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: KolhapuriSaajState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: KolhapuriSaajState, action: Action): KolhapuriSaajState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: KolhapuriSaajState): string {
  const family = KOLHAPURISAAJ_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the KOLHAPURI SAAJ textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within KOLHAPURI SAAJ textile geometry and weaving logic.",
...
export function KolhapuriSaajModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "kolhapurisaaj",
            style_version: state.style,
            source: "homepage-kolhapurisaaj-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `kolhapurisaaj-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `kolhapurisaaj-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = KOLHAPURISAAJ_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="KOLHAPURI SAAJ Textile Generator"
        ...
        <div className="grid min-h-0 flex-1 overflow-hidden lg:[grid-template-columns:420px_1fr]">
          <aside className="flex flex-col overflow-hidden border-r border-white/10 bg-[#0E0E12]">
            ...
            <div className="border-t border-white/[0.06] bg-[#0E0E12] px-5 py-4">
              ...
              >
                Generate KOLHAPURI SAAJ Textile
              </button>
            </div>
          </aside>
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an KOLHAPURI SAAJ-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
