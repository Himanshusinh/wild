...
import { opaquewrapPromptCatalog } from "@/app/view/HomePage/compo/styles/opaquewrap/opaquewrapPromptCatalog";
...
import { OpaquewrapHeader } from "./OpaquewrapHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, OpaquewrapState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "OPAQUE WRAP";
...
function reducer(state: OpaquewrapState, action: Action): OpaquewrapState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: OpaquewrapState): string {
  const prompts = opaquewrapPromptCatalog.prompts;
  const familyPrompt =
  ...
  return [
    ...
    state.inputMode === "image" && state.uploadedImage ? opaquewrapPromptCatalog.promptI2I.trim() : familyPrompt.trim(),
    "",
    "PROJECT INPUTS:",
    projectLine,
    "",
...
export function OpaquewrapModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "opaquewrap",
            style_version: state.style,
            source: "homepage-opaquewrap-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `opaquewrap-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `opaquewrap-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Opaque Wrap Generator"
