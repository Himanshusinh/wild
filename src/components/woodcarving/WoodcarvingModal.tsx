...
import { woodcarvingPromptCatalog } from "@/app/view/HomePage/compo/styles/woodcarving/woodcarvingPromptCatalog";
...
import { WoodcarvingHeader } from "./WoodcarvingHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, WoodcarvingState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "WOOD CARVING";
...
function reducer(state: WoodcarvingState, action: Action): WoodcarvingState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: WoodcarvingState): string {
  const prompts = woodcarvingPromptCatalog.prompts;
  const familyPrompt =
  ...
  return [
    ...
    state.inputMode === "image" && state.uploadedImage ? woodcarvingPromptCatalog.promptI2I.trim() : familyPrompt.trim(),
    "",
    "PROJECT INPUTS:",
    projectLine,
    "",
...
export function WoodcarvingModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "woodcarving",
            style_version: state.style,
            source: "homepage-woodcarving-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `woodcarving-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `woodcarving-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Wood Carving Generator"
