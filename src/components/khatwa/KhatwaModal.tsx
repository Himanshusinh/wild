...
import { KHATWA_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/khatwa/khatwaPromptCatalog";
...
import { KhatwaHeader } from "./KhatwaHeader";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: KhatwaState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: KhatwaState, action: Action): KhatwaState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: KhatwaState): string {
  const family = KHATWA_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep Khatwa cut-cloth applique logic with stitched shapes, active base cloth, and strong contrast structure.";
  ...
  return [
    ...
    "- Keep full-frame Khatwa identity with cut cloth shapes stitched onto foundation fabric.",
    "- Preserve silhouette-first textile forms that remain cuttable and applique-compatible.",
    "- Keep strong base/applique cloth contrast and active foundation textile presence.",
    "",
    "RENDER SETTINGS:",
...
export function KhatwaModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `khatwa-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `khatwa-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = KHATWA_PROMPT_FAMILIES[state.style];
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. bold fabric silhouettes, stitched applique, high base-cloth contrast"
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Khatwa applique textile scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
