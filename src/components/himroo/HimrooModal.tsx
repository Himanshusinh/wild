...
interface HimrooModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGenerate: (data: { prompt: string; version: HimrooVersion; inputMode: 'text' | 'image' }) => void;
}

const HimrooModal: React.FC<HimrooModalProps> = ({ isOpen, onClose, onGenerate }) => {
  const [state, setState] = useState<HimrooState>(INITIAL_HIMROO_STATE);
  const [isGenerating, setIsGenerating] = useState(false);

  useEffect(() => {
    if (!isOpen) {
      setState(INITIAL_HIMROO_STATE);
    }
  }, [isOpen]);

  if (!isOpen) return null;

  const currentVersionData = himrooPromptCatalog.versions.find(v => v.id === state.version);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl bg-[#0A0A0B] border border-white/10 rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        <HimrooHeader currentVersion={state.version}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] text-white/20">Refined by Himroo Intelligence</span>
            </div>
            ...
            <div className="relative group">
              <textarea disabled={isGenerating}
                ...
                placeholder={state.inputMode === 'text' 
                  ? "Describe the scene you want to generate in Himroo style..." 
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {himrooPromptCatalog.chips.map((chip, index) => (
                <div 
                  key={index}
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2 text-white/40">
                <ShieldCheck size={12} />
                <span className="text-[10px]">Verified Himroo Origin System</span>
              </div>
            </div>
