...
import { KINNAURI_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/kinnauri/kinnauriPromptCatalog";
...
import { KinnauriHeader } from "./KinnauriHeader";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: KinnauriState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: KinnauriState, action: Action): KinnauriState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: KinnauriState): string {
  const family = KINNAURI_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep Kinnauri weaving logic: handwoven wool body, border order, and geometric-symbolic motifs.";
  ...
  return [
    ...
    "- Keep full-frame Kinnauri identity with woven structure, border discipline, and motif order.",
    "- Preserve loom-built geometric motif logic and handwoven wool tactility.",
    "- Keep neutral wool ground with brighter patterned zones and textile object coherence.",
    "",
    "RENDER SETTINGS:",
...
export function KinnauriModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
      ...
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            ...
            source: "homepage-kinnauri-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `kinnauri-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `kinnauri-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = KINNAURI_PROMPT_FAMILIES[state.style];
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea disabled={state.panelState === "loading"}
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. geometric motifs, strong borders, woven wool texture"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Kinnauri shawl scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
