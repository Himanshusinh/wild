...
import { SRIKALAHASTI_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/srikalahasti/srikalahastiPromptCatalog";
...
const STYLE_TAG = "SrikalahastiKalamkari";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: SrikalahastiState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: SrikalahastiState, action: Action): SrikalahastiState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: SrikalahastiState): string {
  const family = SRIKALAHASTI_PROMPT_FAMILIES[state.style];
...
export function SrikalahastiModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `srikalahasti-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(async (index: number) => {
    ...
    try {
      await downloadImageUrl(url, `srikalahasti-${state.style}-${index + 1}`);
      toast.dismiss(t);
  ...
  const familyMeta = SRIKALAHASTI_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Srikalahasti Generator"
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea disabled={state.panelState === "loading"}
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. sacred border threshold, central hierarchy, contour-led faces"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[260px] text-xs leading-relaxed text-white/10">
                    Describe a sacred narrative scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
