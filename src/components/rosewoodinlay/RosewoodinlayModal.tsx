...
import { rosewoodinlayPromptCatalog } from "@/app/view/HomePage/compo/styles/rosewoodinlay/rosewoodinlayPromptCatalog";
...
import { RosewoodinlayHeader } from "./RosewoodinlayHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, RosewoodinlayState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "ROSEWOOD INLAY";
...
function reducer(state: RosewoodinlayState, action: Action): RosewoodinlayState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: RosewoodinlayState): string {
  const prompts = rosewoodinlayPromptCatalog.prompts;
  const familyPrompt =
  ...
  return [
    ...
    state.inputMode === "image" && state.uploadedImage ? rosewoodinlayPromptCatalog.promptI2I.trim() : familyPrompt.trim(),
    "",
    "PROJECT INPUTS:",
    projectLine,
    "",
...
export function RosewoodinlayModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "rosewoodinlay",
            style_version: state.style,
            source: "homepage-rosewoodinlay-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `rosewoodinlay-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `rosewoodinlay-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Rosewood Inlay Generator"
