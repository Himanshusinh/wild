...
import { GABBA_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/gabba/gabbaPromptCatalog";
...
import { GabbaHeader } from "./GabbaHeader";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: GabbaState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: GabbaState, action: Action): GabbaState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: GabbaState): string {
  const family = GABBA_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep reconstructed woolen patchwork and chain-stitch grammar.";
  ...
  return [
    ...
    "- Keep output in Gabba reconstructed woolen cloth and chain-stitch grammar.",
    "- Avoid generic printed rugs, digital vector sharpness, or flat painted ornament.",
...
export function GabbaModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `gabba-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `gabba-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = GABBA_PROMPT_FAMILIES[state.style];
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea disabled={state.panelState !== "empty"}
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. heavy wool texture, Kashmiri appliqué motifs, chain-stitch detail"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Gabba scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
