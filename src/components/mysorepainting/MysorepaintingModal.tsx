...
import { mysorepaintingPromptCatalog } from "@/app/view/HomePage/compo/styles/mysorepainting/mysorepaintingPromptCatalog";
...
import { MysorepaintingHeader } from "./MysorepaintingHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, MysorepaintingState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "MYSORE PAINTING";
...
function reducer(state: MysorepaintingState, action: Action): MysorepaintingState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: MysorepaintingState): string {
  const prompts = mysorepaintingPromptCatalog.prompts;
  const familyPrompt =
  ...
  return [
    ...
    state.inputMode === "image" && state.uploadedImage ? mysorepaintingPromptCatalog.promptI2I.trim() : familyPrompt.trim(),
    "",
    "PROJECT INPUTS:",
    projectLine,
    "",
...
export function MysorepaintingModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "mysorepainting",
            style_version: state.style,
            source: "homepage-mysorepainting-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `mysorepainting-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `mysorepainting-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Mysore Painting Generator"
