...
import { MALERKOTLAZARI_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/malerkotlazari/malerkotlazariPromptCatalog";
...
import { MalerkotlaZariHeader } from "./MalerkotlaZariHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, MalerkotlaZariState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "MALERKOTLA ZARI";
...
function reducer(state: MalerkotlaZariState, action: Action): MalerkotlaZariState {
  switch (action.type) {
...
function buildPrompt(state: MalerkotlaZariState): string {
  const family = MALERKOTLAZARI_PROMPT_FAMILIES[state.style];
...
export function MalerkotlaZariModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const res = await dispatch(falGenerate({
        ...
        meta: { style_premium: true, style_key: "malerkotlazari", style_version: state.style, source: "homepage-malerkotlazari-modal" },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try { await downloadAllImageUrls(urls, `malerkotlazari-${state.style}`); toast.dismiss(t); toast.success("Downloads started"); } 
  ...
  const handleSaveImage = useCallback(async (index: number) => {
    ...
    try { await downloadImageUrl(url, `malerkotlazari-${state.style}-${index + 1}`); toast.dismiss(t); toast.success("Download started"); } 
    catch { toast.dismiss(t); toast.error("Save failed"); }
  }, [state.generatedImages, state.style]);

  if (!isOpen) return null;
  const familyMeta = MALERKOTLAZARI_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div role="dialog" aria-modal="true" aria-label="MALERKOTLA ZARI Generator" className={`relative flex w-[min(1080px,calc(100vw-24px))] h-[min(760px,calc(100vh-24px))] flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#0E0E12]/95 shadow-[0_24px_70px_rgba(0,0,0,0.7)] ring-1 ring-white/[0.04] transition-all duration-300 ${isVisible ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-2 scale-[0.985]"}`}>
        <MalerkotlaZariHeader style={state.style} onStyleChange={(s) => dispatchLocal({ type: "SET_STYLE", payload: s })} onClose={onClose} disabled={state.panelState === "loading"} />
        <div className="grid min-h-0 flex-1 overflow-hidden lg:[grid-template-columns:420px_1fr]">
          <aside className="flex flex-col overflow-hidden border-r border-white/10 bg-[#0E0E12]">
            ...
            <div className="border-t border-white/[0.06] bg-[#0E0E12] px-5 py-4">
              <button type="button" onClick={() => void handleGenerate()} disabled={state.panelState === "loading"} className="w-full rounded-lg bg-[#2F6BFF] py-2.5 text-[12px] font-semibold text-white transition hover:bg-[#2F6BFF]/90 disabled:opacity-50">Generate MALERKOTLA ZARI</button>
            </div>
          </aside>
