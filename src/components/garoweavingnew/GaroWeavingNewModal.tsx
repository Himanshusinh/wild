...
import { GaroWeavingNewHeader } from "./GaroWeavingNewHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, GaroWeavingNewState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "GAROWEAVINGNEW";
...
function reducer(state: GaroWeavingNewState, action: Action): GaroWeavingNewState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: GaroWeavingNewState): string {
  const prompts = garoweavingnewPromptCatalog.prompts;
  const familyPrompt =
  ...
  return [
    ...
    state.inputMode === "image" && state.uploadedImage ? garoweavingnewPromptCatalog.promptI2I[state.style === "V1" ? "v1" : state.style === "V2" ? "v2" : "v3"].trim() : familyPrompt.trim(),
    "",
    "PROJECT INPUTS:",
    projectLine,
    "",
...
export function GaroWeavingNewModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "garoweavingnew",
            style_version: state.style,
            source: "homepage-garoweavingnew-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `garoweavingnew-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `garoweavingnew-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Garo Weaving Generator"
