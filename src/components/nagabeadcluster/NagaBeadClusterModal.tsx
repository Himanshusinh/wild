...
import { NAGABEADCLUSTER_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/nagabeadcluster/nagabeadclusterPromptCatalog";
...
import { NagaBeadClusterHeader } from "./NagaBeadClusterHeader";
import {
  INITIAL_STATE,
  type NagaBeadClusterState,
  type StyleFamily,
  type InputMode,
  type ModelId,
  type ImageCount,
...
const STYLE_TAG = "NagaBeadCluster";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: NagaBeadClusterState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: NagaBeadClusterState, action: Action): NagaBeadClusterState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: NagaBeadClusterState): string {
  const family = NAGABEADCLUSTER_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the NAGA BEAD CLUSTER textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within NAGA BEAD CLUSTER textile geometry and weaving logic.",
...
export function NagaBeadClusterModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "nagabeadcluster",
            style_version: state.style,
            source: "homepage-nagabeadcluster-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `nagabeadcluster-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `nagabeadcluster-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = NAGABEADCLUSTER_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="NAGA BEAD CLUSTER Textile Generator"
        ...
        <div className="grid min-h-0 flex-1 overflow-hidden lg:[grid-template-columns:420px_1fr]">
          <aside className="flex flex-col overflow-hidden border-r border-white/10 bg-[#0E0E12]">
            ...
            <div className="border-t border-white/[0.06] bg-[#0E0E12] px-5 py-4">
              ...
              >
                Generate NAGA BEAD CLUSTER Textile
              </button>
            </div>
          </aside>
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an NAGA BEAD CLUSTER-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
