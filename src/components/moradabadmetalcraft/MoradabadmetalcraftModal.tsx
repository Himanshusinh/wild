...
import { MORADABADMETALCRAFT_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/moradabadmetalcraft/moradabadmetalcraftPromptCatalog";
...
import { MoradabadmetalcraftHeader } from "./MoradabadmetalcraftHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, MoradabadmetalcraftState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "MORADABADMETALCRAFT";
...
function reducer(state: MoradabadmetalcraftState, action: Action): MoradabadmetalcraftState {
  switch (action.type) {
...
function buildPrompt(state: MoradabadmetalcraftState): string {
  const family = MORADABADMETALCRAFT_PROMPT_FAMILIES[state.style];
...
export function MoradabadmetalcraftModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const res = await dispatch(falGenerate({
        ...
        meta: { style_premium: true, style_key: "moradabadmetalcraft", style_version: state.style, source: "homepage-moradabadmetalcraft-modal" },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try { await downloadAllImageUrls(urls, `moradabadmetalcraft-${state.style}`); toast.dismiss(t); toast.success("Downloads started"); } 
  ...
  const handleSaveImage = useCallback(async (index: number) => {
    ...
    try { await downloadImageUrl(url, `moradabadmetalcraft-${state.style}-${index + 1}`); toast.dismiss(t); toast.success("Download started"); } 
    catch { toast.dismiss(t); toast.error("Save failed"); }
  }, [state.generatedImages, state.style]);

  if (!isOpen) return null;
  const familyMeta = MORADABADMETALCRAFT_PROMPT_FAMILIES[state.style];
        ...
        <div className="grid min-h-0 flex-1 overflow-hidden lg:[grid-template-columns:420px_1fr]">
          <aside className="flex flex-col overflow-hidden border-r border-white/10 bg-[#0E0E12]">
            ...
            <div className="border-t border-white/[0.06] bg-[#0E0E12] px-5 py-4">
              <button type="button" onClick={() => void handleGenerate()} disabled={state.panelState === "loading"} className="w-full rounded-lg bg-[#2F6BFF] py-2.5 text-[12px] font-semibold text-white transition hover:bg-[#2F6BFF]/90 disabled:opacity-50">Generate MORADABADMETALCRAFT</button>
            </div>
          </aside>
