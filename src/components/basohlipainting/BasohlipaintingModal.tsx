...
import { BASOHLIPAINTING_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/basohlipainting/basohlipaintingPromptCatalog";
...
import { BasohlipaintingHeader } from "./BasohlipaintingHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, BasohlipaintingState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "BASOHLIPAINTING";
...
function reducer(state: BasohlipaintingState, action: Action): BasohlipaintingState {
  switch (action.type) {
...
function buildPrompt(state: BasohlipaintingState): string {
  const family = BASOHLIPAINTING_PROMPT_FAMILIES[state.style];
...
export function BasohlipaintingModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const res = await dispatch(falGenerate({
        ...
        meta: { style_premium: true, style_key: "basohlipainting", style_version: state.style, source: "homepage-basohlipainting-modal" },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try { await downloadAllImageUrls(urls, `basohlipainting-${state.style}`); toast.dismiss(t); toast.success("Downloads started"); } 
  ...
  const handleSaveImage = useCallback(async (index: number) => {
    ...
    try { await downloadImageUrl(url, `basohlipainting-${state.style}-${index + 1}`); toast.dismiss(t); toast.success("Download started"); } 
    catch { toast.dismiss(t); toast.error("Save failed"); }
  }, [state.generatedImages, state.style]);

  if (!isOpen) return null;
  const familyMeta = BASOHLIPAINTING_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div role="dialog" aria-modal="true" aria-label="SAHARANPURWOODCRAFT BASOHLIPAINTING Generator" className={`relative flex w-[min(1080px,calc(100vw-24px))] h-[min(760px,calc(100vh-24px))] flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#0E0E12]/95 shadow-[0_24px_70px_rgba(0,0,0,0.7)] ring-1 ring-white/[0.04] transition-all duration-300 ${isVisible ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-2 scale-[0.985]"}`}>
        <BasohlipaintingHeader style={state.style} onStyleChange={(s) => dispatchLocal({ type: "SET_STYLE", payload: s })} onClose={onClose} disabled={state.panelState === "loading"} />
