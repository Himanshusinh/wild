...
import { ngotekherhPromptCatalog } from "@/app/view/HomePage/compo/styles/ngotekherh/ngotekherhPromptCatalog";
...
import { NgotekherhHeader } from "./NgotekherhHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, NgotekherhState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "NGOTEKHERH";
...
function reducer(state: NgotekherhState, action: Action): NgotekherhState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: NgotekherhState): string {
  const prompts = ngotekherhPromptCatalog.prompts;
  const familyPrompt =
  ...
  return [
    ...
    state.inputMode === "image" && state.uploadedImage ? ngotekherhPromptCatalog.promptI2I.trim() : familyPrompt.trim(),
    "",
    "PROJECT INPUTS:",
    projectLine,
    "",
...
export function NgotekherhModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "ngotekherh",
            style_version: state.style,
            source: "homepage-ngotekherh-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `ngotekherh-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `ngotekherh-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Ngotekherh Generator"
