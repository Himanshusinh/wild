...
import { MANJUSHA_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/manjusha/manjushaPromptCatalog";
...
import { ManjushaHeader } from "./ManjushaHeader";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: ManjushaState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: ManjushaState, action: Action): ManjushaState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: ManjushaState): string {
  const family = MANJUSHA_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep Manjusha narrative logic: role-bearing figures, serpent symbolism, strong contour, and ritual box-memory containment.";
  ...
  return [
    ...
    "- Keep full-frame Manjusha identity with mask-first character architecture, devotional theatre readability, and satra-grounded performance logic.",
    "- Preserve iconic role-bearing figures, symbolic serpent carriers, and flat folk-graphic structure tied to Bihula-Bishahari narrative logic.",
    "- Keep story-first Manjusha construction and avoid realistic anatomy, Madhubani-like filler takeover, or generic folk-poster styling.",
    "",
    "RENDER SETTINGS:",
...
export function ManjushaModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
      ...
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            ...
            source: "homepage-manjusha-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `manjusha-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `manjusha-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = MANJUSHA_PROMPT_FAMILIES[state.style];
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. Bihula-Bishahari event, serpent motifs, border containment, pink/green/yellow palette control"
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Manjusha ritual narrative scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
