...
import { TawlhlohpuanHeader } from "./TawlhlohpuanHeader";
...
function reducer(state: TawlhlohpuanState, action: Action): TawlhlohpuanState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: TawlhlohpuanState): string {
  const prompts = tawlhlohpuanPromptCatalog.prompts;
  const familyPrompt =
  ...
  return [
    ...
    state.inputMode === "image" && state.uploadedImage ? tawlhlohpuanPromptCatalog.promptI2I.trim() : familyPrompt.trim(),
    "",
    "PROJECT INPUTS:",
    projectLine,
    "",
...
export function TawlhlohpuanModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "tawlhlohpuan",
            style_version: state.style,
            source: "homepage-tawlhlohpuan-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `tawlhlohpuan-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `tawlhlohpuan-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Tawlhlohpuan Generator"
