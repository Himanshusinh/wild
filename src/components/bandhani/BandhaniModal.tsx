...
import { BandhaniHeader } from "./BandhaniHeader";
import {
  INITIAL_STATE,
  type BandhaniState,
  type StyleFamily,
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: BandhaniState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: BandhaniState, action: Action): BandhaniState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: BandhaniState): string {
  const family = BANDHANI_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the BANDHANI textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within BANDHANI textile geometry and weaving logic.",
...
export function BandhaniModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
      ...
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            ...
            source: "homepage-bandhani-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `bandhani-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `bandhani-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = BANDHANI_PROMPT_FAMILIES[state.style];
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="BANDHANI Textile Generator"
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an BANDHANI-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
