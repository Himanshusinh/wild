...
import { ETIKOPPAKA_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/etikoppakaPromptCatalog";
...
import { EtikoppakaHeader } from "./EtikoppakaHeader";
import {
  INITIAL_STATE,
  EtikoppakaState,
  StyleFamily,
...
const STYLE_TAG = "EtikoppakaToys";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: EtikoppakaState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: EtikoppakaState, action: Action): EtikoppakaState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: EtikoppakaState): string {
  const family = ETIKOPPAKA_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Do not invent extra figures, props, or environments; keep the turned-lacquer object world minimal and coherent.";
  ...
  return [
    ...
    "- Keep the output turned, rounded, lacquered and burnished — not carved figurines or plastic toys.",
    "",
    "RENDER SETTINGS:",
    ...
    "- Keep the visible output aligned to WildMind's Etikoppaka style experience.",
  ]
    .filter(Boolean)
    .join("\n");
}

export function EtikoppakaModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "etikoppaka",
            style_version: state.style,
            source: "homepage-etikoppaka-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `etikoppaka-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `etikoppaka-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const handleOpenStudio = useCallback(async () => {
    ...
    const payload = {
      ...
      style: "Etikoppaka Toys",
      ...(uploadedImages?.length ? { uploadedImages } : {}),
      metadata: {
        source: "homepage-etikoppaka-modal",
        etikoppakaVersion: state.style,
        etikoppakaModal: true,
      },
    };
  ...
  const familyMeta = ETIKOPPAKA_PROMPT_FAMILIES[state.style];
  ...
  return (
    <div className="fixed inset-0 z-[160] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Etikoppaka Toys Generator"
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. smooth turned volumes, burnished lacquer, natural dyes"
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[280px] text-xs leading-relaxed text-white/10">
                    Describe a turned lacquer object scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
