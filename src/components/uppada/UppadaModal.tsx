...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: UppadaState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: UppadaState, action: Action): UppadaState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: UppadaState): string {
  const family = UPPADA_PROMPT_FAMILIES[state.style];
...
export function UppadaModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `uppada-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(async (index: number) => {
    ...
    try {
      await downloadImageUrl(url, `uppada-${state.style}-${index + 1}`);
      toast.dismiss(t);
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[260px] text-xs leading-relaxed text-white/10">
                    Describe a motif system / textile layout (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
