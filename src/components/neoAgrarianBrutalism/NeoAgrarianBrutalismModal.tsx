...
import { NEO_AGRARIAN_BRUTALISM_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/neoagrarianbrutalism/neoAgrarianBrutalismPromptCatalog";
...
import { NeoAgrarianBrutalismHeader } from "./NeoAgrarianBrutalismHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import {
  INITIAL_STATE,
  NeoAgrarianBrutalismState,
  StyleFamily,
  InputMode,
  ModelId,
  ImageCount,
...
const STYLE_TAG = "Neo-Agrarian Brutalism";
...
function reducer(state: NeoAgrarianBrutalismState, action: Action): NeoAgrarianBrutalismState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: NeoAgrarianBrutalismState): string {
  const family = NEO_AGRARIAN_BRUTALISM_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep structured neo-agrarian and brutalist logic intact.";
  ...
  return [
    ...
    "- Keep output in a 3D Realistic Neo-Agrarian Brutalist Haryana grammar.",
    "- Avoid pretty concrete-and-crop realism.",
    "- Ensure concrete forms scale heavily to manage crop/storage areas.",
    "",
    "RENDER SETTINGS:",
...
export function NeoAgrarianBrutalismModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "neoAgrarianBrutalism",
            style_version: state.style,
            source: "homepage-neo-agrarian-brutalism-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `neo-agrarian-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `neo-agrarian-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = NEO_AGRARIAN_BRUTALISM_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Neo-Agrarian Brutalism Generator"
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea disabled={state.panelState !== "empty"}
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. cinematic concrete-and-agriculture blend"
