...
import { JHAJJAR_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/jhajjar/jhajjarPromptCatalog";
...
import { JhajjarHeader } from "./JhajjarHeader";
...
const STYLE_TAG = "Jhajjar Pottery";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: JhajjarState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: JhajjarState, action: Action): JhajjarState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: JhajjarState): string {
  const family = JHAJJAR_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep vessel-first Jhajjar surahi logic with porous matte terracotta body and domestic utility truth.";
  ...
  return [
    ...
    "- Keep output vessel-first with slim-necked surahi memory and usable earthen container logic.",
    "- Avoid glossy ceramics, studio pottery styling, or decor-first rustic prop treatment.",
    "- Keep porous fired-clay material truth and matte terracotta behavior.",
    "",
    "RENDER SETTINGS:",
...
export function JhajjarModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `jhajjar-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `jhajjar-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = JHAJJAR_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Jhajjar Pottery Generator"
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. slim-necked surahi, porous terracotta body, matte fired clay"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Jhajjar pottery scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
