...
import { BAMBOOCRAFT_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/bamboocraft/bamboocraftPromptCatalog";
...
import { BambooCraftHeader } from "./BambooCraftHeader";
import {
  INITIAL_STATE,
  type BambooCraftState,
  type StyleFamily,
  type InputMode,
  type ModelId,
  type ImageCount,
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: BambooCraftState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: BambooCraftState, action: Action): BambooCraftState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: BambooCraftState): string {
  const family = BAMBOOCRAFT_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the BAMBOO CRAFT textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within BAMBOO CRAFT textile geometry and weaving logic.",
...
export function BambooCraftModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "bamboocraft",
            style_version: state.style,
            source: "homepage-bamboocraft-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `bamboocraft-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `bamboocraft-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = BAMBOOCRAFT_PROMPT_FAMILIES[state.style];
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="BAMBOO CRAFT Textile Generator"
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an BAMBOO CRAFT-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
