...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: ThangkaState["panelState"] }
...
function buildPrompt(state: ThangkaState): string {
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Do not invent extra figures, objects, borders, symbols, or scenery.";
  ...
  return [
    ...
    "- Keep composition sacred and centered; avoid decorative fillers unless requested.",
    "",
    "RENDER SETTINGS:",
    ...
    "- Keep the visible output aligned to WildMind's Thangka style experience.",
  ]
    .filter(Boolean)
    .join("\n");
}
...
export function ThangkaModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `thangka-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveOne = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `thangka-${state.style}-${index + 1}`);
        toast.dismiss(t);
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. keep central deity dominant, strict axis, no scenery"
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[280px] text-xs leading-relaxed text-white/10">
                    Describe a sacred Thangka scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
              ...
              {state.panelState === "results" ? (
                <div className="flex flex-col gap-4">
                  <OutputGrid
                    ...
                    onSaveImage={(i) => void handleSaveOne(i)}
                    onExpandImage={(i) => {
