...
import { AZULEJOS_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/azulejos/azulejosPromptCatalog";
...
import { AzulejosHeader } from "./AzulejosHeader";
import {
  INITIAL_STATE,
  type AzulejosState,
  type StyleFamily,
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: AzulejosState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: AzulejosState, action: Action): AzulejosState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: AzulejosState): string {
  const family = AZULEJOS_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the AZULEJOS textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within AZULEJOS textile geometry and weaving logic.",
...
export function AzulejosModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
      ...
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            ...
            source: "homepage-azulejos-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `azulejos-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `azulejos-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = AZULEJOS_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="AZULEJOS Textile Generator"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an AZULEJOS-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
