...
import { KAAVI_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/kaavi/kaaviPromptCatalog";
...
import { KaaviHeader } from "./KaaviHeader";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: KaaviState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: KaaviState, action: Action): KaaviState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: KaaviState): string {
  const family = KAAVI_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep incised wall-first Kaavi logic with lime plaster base, red-oxide layer, and white reveal through etching.";
  ...
  return [
    ...
    "- Keep output wall-first and incision-first; avoid painted mural behavior.",
    "- Preserve red surface layer over lime base with white reveal through etched removal.",
    "- Keep Goa Kaavi panel logic and architectural embedding.",
    "",
    "RENDER SETTINGS:",
...
export function KaaviModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `kaavi-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `kaavi-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = KAAVI_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Kaavi Art Generator"
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea disabled={state.panelState === "loading"}
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. red-oxide layer, incised white reveal, lime plaster wall"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Kaavi wall scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
