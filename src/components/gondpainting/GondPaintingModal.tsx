...
interface GondPaintingModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGenerate: (data: { prompt: string; version: GondPaintingVersion; inputMode: 'text' | 'image' }) => void;
}

const GondPaintingModal: React.FC<GondPaintingModalProps> = ({ isOpen, onClose, onGenerate }) => {
  const [state, setState] = useState<GondPaintingState>(INITIAL_GOND_PAINTING_STATE);
  const [isGenerating, setIsGenerating] = useState(false);

  useEffect(() => {
    if (!isOpen) {
      setState(INITIAL_GOND_PAINTING_STATE);
    }
  }, [isOpen]);

  if (!isOpen) return null;

  const currentVersionData = gondpaintingPromptCatalog.versions.find(v => v.id === state.version);
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] text-white/20">Refined by Gond Painting Intelligence</span>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {gondpaintingPromptCatalog.chips.map((chip, index) => (
                <div 
                  key={index}
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2 text-white/40">
                <ShieldCheck size={12} />
                <span className="text-[10px]">Verified Gond Painting Origin System</span>
              </div>
            </div>
...
export default GondPaintingModal;
