...
import { KANGRA_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/kangra/kangraPromptCatalog";
...
import { KangraHeader } from "./KangraHeader";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: KangraState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: KangraState, action: Action): KangraState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: KangraState): string {
  const family = KANGRA_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep Kangra miniature logic with serene figures, delicate line, lush green landscape, and poetic-devotional atmosphere.";
  ...
  return [
    ...
    "- Keep full-frame Kangra identity across faces, figures, space, landscape, and architecture.",
...
export function KangraModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `kangra-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `kangra-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = KANGRA_PROMPT_FAMILIES[state.style];
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Kangra scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
