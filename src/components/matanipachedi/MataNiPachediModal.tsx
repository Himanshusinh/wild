...
import { MATA_NI_PACHEDI_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/matanipachedi/mataNiPachediPromptCatalog";
...
import { MataNiPachediHeader } from "./MataNiPachediHeader";
...
const STYLE_TAG = "Mata ni Pachedi";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: MataNiPachediState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: MataNiPachediState, action: Action): MataNiPachediState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: MataNiPachediState): string {
  const family = MATA_NI_PACHEDI_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep Mata ni Pachedi shrine logic: central Mother Goddess hierarchy, bounded sacred enclosure, and print-paint ritual cloth structure.";
  ...
  return [
    ...
    "- Keep full-frame Mata ni Pachedi identity with goddess-first shrine hierarchy, portable temple-cloth enclosure, and devotional support ordering.",
    "- Preserve block-printed border structure, hand-painted iconographic figures, and disciplined black-red-white ritual palette behavior.",
    "- Keep cloth-as-shrine construction and avoid generic goddess fabric art, fashion-print styling, or open scenic composition.",
    "",
    "RENDER SETTINGS:",
...
export function MataNiPachediModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "matanipachedi",
            style_version: state.style,
            source: "homepage-matanipachedi-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `matanipachedi-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `matanipachedi-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = MATA_NI_PACHEDI_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Mata ni Pachedi Generator"
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea disabled={state.panelState === "loading"}
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. central goddess type, shrine border rhythm, devotee ordering, black-red-white palette control"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Mata ni Pachedi shrine-cloth scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
