...
import { KAREPA_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/karepa/karepaPromptCatalog";
...
import { KarepaHeader } from "./KarepaHeader";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: KarepaState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: KarepaState, action: Action): KarepaState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: KarepaState): string {
  const family = KAREPA_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep Karepa miniature logic with serene figures, delicate line, lush green landscape, and poetic-devotional atmosphere.";
  ...
  return [
    ...
    "- Keep full-frame Karepa identity across faces, figures, space, landscape, and architecture.",
    "- Preserve delicate line quality with serene human type and restrained lyrical gestures.",
    "- Keep soft, fresh, less-intense palette with active green environment and poetic emotional reading.",
    "",
    "RENDER SETTINGS:",
...
export function KarepaModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `karepa-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `karepa-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = KAREPA_PROMPT_FAMILIES[state.style];
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. serene figures, delicate line, soft green landscape"
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Karepa scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
