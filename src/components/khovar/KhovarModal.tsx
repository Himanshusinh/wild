...
import { KHOVAR_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/khovar/khovarPromptCatalog";
...
import { KhovarHeader } from "./KhovarHeader";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: KhovarState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: KhovarState, action: Action): KhovarState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: KhovarState): string {
  const family = KHOVAR_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep Khovar wall logic: black underlayer, white top clay, and comb/finger-etched reveal.";
  ...
  return [
    ...
    "- Keep full-frame Khovar identity with layered clay body and etched reveal process.",
    "- Preserve handmade comb-cut and finger-etched marks with ceremonial wall rhythm.",
    "- Keep black-white clay contrast and active mud-wall support as primary structure.",
    "",
    "RENDER SETTINGS:",
...
export function KhovarModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `khovar-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `khovar-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = KHOVAR_PROMPT_FAMILIES[state.style];
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. layered black-white clay, comb-cut etching, marriage-wall rhythm"
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Khovar marriage-wall scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
