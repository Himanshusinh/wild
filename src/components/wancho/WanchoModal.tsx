...
import { WANCHO_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/wanchoPromptCatalog";
...
import { WanchoHeader } from "./WanchoHeader";
import {
  INITIAL_STATE,
  WanchoState,
  StyleFamily,
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: WanchoState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: WanchoState, action: Action): WanchoState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: WanchoState): string {
  const family = WANCHO_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Do not invent extra figures, objects, weapons, animals, posts, or scenery.";
  ...
  return [
    ...
    "- Keep the output object-first and head-dominant; avoid decorative fillers unless requested.",
    "",
    "RENDER SETTINGS:",
    ...
    "- Keep the visible output aligned to WildMind's Wancho style experience.",
  ]
    .filter(Boolean)
    .join("\n");
}

export function WanchoModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `wancho-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `wancho-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const handleOpenStudio = useCallback(async () => {
    ...
    const payload = {
      ...
      style: "Wancho Carving",
      ...(uploadedImages?.length ? { uploadedImages } : {}),
      metadata: {
        source: "homepage-wancho-modal",
        wanchoVersion: state.style,
        wanchoModal: true,
      },
    };
  ...
  const familyMeta = WANCHO_PROMPT_FAMILIES[state.style];
  ...
  return (
    <div className="fixed inset-0 z-[160] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Wancho Carving Generator"
          ...
          <aside className="flex flex-col overflow-hidden border-r border-white/10 bg-[#0E0E12]">
            <div className="flex flex-1 flex-col gap-5 overflow-y-auto px-5 py-5 [scrollbar-width:thin] [&::-webkit-scrollbar-thumb]:rounded [&::-webkit-scrollbar-thumb]:bg-white/[0.06] [&::-webkit-scrollbar]:w-1">
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. head-dominant mask, strict frontality, matte carved wood"
              ...
              <SettingsPanel
                ...
                onRatioChange={(v) => handleRatioChange(v)}
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[280px] text-xs leading-relaxed text-white/10">
                    Describe a carved head / object (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
