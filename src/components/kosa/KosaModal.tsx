...
import { KOSA_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/kosa/kosaPromptCatalog";
...
import { KosaHeader } from "./KosaHeader";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: KosaState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: KosaState, action: Action): KosaState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: KosaState): string {
  const family = KOSA_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep Kosa weaving logic: Kosa silk fiber ancestry, woven cloth truth, and garment-scale drape.";
  ...
  return [
    ...
    "- Keep full-frame Kosa identity with woven silk structure, cloth-first hierarchy, and natural luster.",
    "- Preserve handloom-built silk cloth behavior, weave density, and process-born textile character.",
    "- Keep controlled silk sheen, soft drape, and Chhattisgarh handloom textile coherence.",
    "",
    "RENDER SETTINGS:",
...
export function KosaModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `kosa-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `kosa-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = KOSA_PROMPT_FAMILIES[state.style];
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. Kosa silk drape, natural sheen, woven handloom texture"
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Kosa silk textile scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
