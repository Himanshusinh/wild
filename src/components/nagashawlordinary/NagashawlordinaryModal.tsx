...
import { nagashawlordinaryPromptCatalog } from "@/app/view/HomePage/compo/styles/nagashawlordinary/nagashawlordinaryPromptCatalog";
...
import { NagashawlordinaryHeader } from "./NagashawlordinaryHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, NagashawlordinaryState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "NAGA SHAWL (ORDINARY)";
...
function reducer(state: NagashawlordinaryState, action: Action): NagashawlordinaryState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: NagashawlordinaryState): string {
  const prompts = nagashawlordinaryPromptCatalog.prompts;
  const familyPrompt =
    state.style === "V1" ? prompts.v1 : state.style === "V2" ? prompts.v2 : prompts.v3;
  ...
  return [
    ...
    state.inputMode === "image" && state.uploadedImage ? nagashawlordinaryPromptCatalog.promptI2I.trim() : familyPrompt.trim(),
    "",
    "PROJECT INPUTS:",
    projectLine,
    "",
...
export function NagashawlordinaryModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "nagashawlordinary",
            style_version: state.style,
            source: "homepage-nagashawlordinary-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `nagashawlordinary-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `nagashawlordinary-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Naga Shawl Generator"
