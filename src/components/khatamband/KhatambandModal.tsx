...
import { KhatambandHeader } from "./KhatambandHeader";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: KhatambandState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: KhatambandState, action: Action): KhatambandState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: KhatambandState): string {
  const family = KHATAMBAND_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep interlocking joinery and polygonal wood grammar.";
  ...
  return [
    ...
    "- Keep output in Khatamband interlocking joinery and wood grammar.",
    "- Avoid flat 2D textures, digital pattern overlays, or visible nails/glue.",
...
export function KhatambandModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "khatamband",
            style_version: state.style,
            source: "homepage-khatamband-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `khatamband-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `khatamband-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = KHATAMBAND_PROMPT_FAMILIES[state.style];
  ...
  return (
    <div className="fixed inset-0 z-[160] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Khatamband Generator"
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. hexagonal patterns, natural Fir wood grain, intricate joinery"
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Khatamband scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
