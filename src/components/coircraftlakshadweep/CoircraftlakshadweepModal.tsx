...
import { COIRCRAFTLAKSHADWEEP_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/coircraftlakshadweep/coircraftlakshadweepPromptCatalog";
...
import { CoircraftlakshadweepHeader } from "./CoircraftlakshadweepHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, CoircraftlakshadweepState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "COIRCRAFTLAKSHADWEEP";
...
function reducer(state: CoircraftlakshadweepState, action: Action): CoircraftlakshadweepState {
  switch (action.type) {
...
function buildPrompt(state: CoircraftlakshadweepState): string {
  const family = COIRCRAFTLAKSHADWEEP_PROMPT_FAMILIES[state.style];
...
export function CoircraftlakshadweepModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const res = await dispatch(falGenerate({
        ...
        meta: { style_premium: true, style_key: "coircraftlakshadweep", style_version: state.style, source: "homepage-coircraftlakshadweep-modal" },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try { await downloadAllImageUrls(urls, `coircraftlakshadweep-${state.style}`); toast.dismiss(t); toast.success("Downloads started"); } 
  ...
  const handleSaveImage = useCallback(async (index: number) => {
    ...
    try { await downloadImageUrl(url, `coircraftlakshadweep-${state.style}-${index + 1}`); toast.dismiss(t); toast.success("Download started"); } 
    catch { toast.dismiss(t); toast.error("Save failed"); }
  }, [state.generatedImages, state.style]);

  if (!isOpen) return null;
  const familyMeta = COIRCRAFTLAKSHADWEEP_PROMPT_FAMILIES[state.style];
