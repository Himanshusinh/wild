...
import { SheerFieldClothHeader } from "./SheerFieldClothHeader";
import {
  INITIAL_STATE,
  type SheerFieldClothState,
  type StyleFamily,
  type InputMode,
  type ModelId,
  type ImageCount,
...
const STYLE_TAG = "SheerFieldCloth";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: SheerFieldClothState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: SheerFieldClothState, action: Action): SheerFieldClothState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: SheerFieldClothState): string {
  const family = SHEERFIELDCLOTH_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the SHEER FIELD CLOTH textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within SHEER FIELD CLOTH textile geometry and weaving logic.",
...
export function SheerFieldClothModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "sheerfieldcloth",
            style_version: state.style,
            source: "homepage-sheerfieldcloth-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `sheerfieldcloth-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `sheerfieldcloth-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = SHEERFIELDCLOTH_PROMPT_FAMILIES[state.style];
  ...
  return (
    <div className="fixed inset-0 z-[160] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="SHEER FIELD CLOTH Textile Generator"
        ...
        <div className="grid min-h-0 flex-1 overflow-hidden lg:[grid-template-columns:420px_1fr]">
          <aside className="flex flex-col overflow-hidden border-r border-white/10 bg-[#0E0E12]">
            ...
            <div className="border-t border-white/[0.06] bg-[#0E0E12] px-5 py-4">
              ...
              >
                Generate SHEER FIELD CLOTH Textile
              </button>
            </div>
          </aside>
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an SHEER FIELD CLOTH-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
