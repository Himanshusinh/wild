...
import { TEMPLEMURAL_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/templemural/templemuralPromptCatalog";
...
import { TempleMuralHeader } from "./TempleMuralHeader";
import {
  INITIAL_STATE,
  type TempleMuralState,
  type StyleFamily,
  type InputMode,
  type ModelId,
  type ImageCount,
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: TempleMuralState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: TempleMuralState, action: Action): TempleMuralState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: TempleMuralState): string {
  const family = TEMPLEMURAL_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the TEMPLE MURAL textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within TEMPLE MURAL textile geometry and weaving logic.",
...
export function TempleMuralModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "templemural",
            style_version: state.style,
            source: "homepage-templemural-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `templemural-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `templemural-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = TEMPLEMURAL_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="TEMPLE MURAL Textile Generator"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an TEMPLE MURAL-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
