...
import { COIRCRAFT_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/coircraft/coircraftPromptCatalog";
...
import { CoirCraftHeader } from "./CoirCraftHeader";
import {
  INITIAL_STATE,
  type CoirCraftState,
  type StyleFamily,
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: CoirCraftState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: CoirCraftState, action: Action): CoirCraftState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: CoirCraftState): string {
  const family = COIRCRAFT_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the COIR CRAFT textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within COIR CRAFT textile geometry and weaving logic.",
...
export function CoirCraftModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
      ...
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            ...
            source: "homepage-coircraft-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `coircraft-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `coircraft-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = COIRCRAFT_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="COIR CRAFT Textile Generator"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an COIR CRAFT-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
