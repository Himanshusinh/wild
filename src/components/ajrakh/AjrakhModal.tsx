...
import { AJRAKH_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/ajrakh/ajrakhPromptCatalog";
...
import { AjrakhHeader } from "./AjrakhHeader";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: AjrakhState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: AjrakhState, action: Action): AjrakhState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: AjrakhState): string {
  const family = AJRAKH_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep structured border-field logic with geometric symmetry and process-built Ajrakh repeat discipline.";
  ...
  return [
    ...
    "- Keep output in process-built Ajrakh resist block-print grammar.",
    "- Avoid generic geometric print, wallpaper repetition, or digital vector sharpness.",
...
export function AjrakhModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `ajrakh-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `ajrakh-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = AJRAKH_PROMPT_FAMILIES[state.style];
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea disabled={state.panelState === "loading"}
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. centered Ajrakh field, indigo-maroon structure, carved-block repeat discipline"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an Ajrakh scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
