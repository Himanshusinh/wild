...
import { KinhalCraftHeader } from "./KinhalCraftHeader";
import {
  INITIAL_STATE,
  type KinhalCraftState,
  type StyleFamily,
  type InputMode,
  type ModelId,
  type ImageCount,
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: KinhalCraftState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: KinhalCraftState, action: Action): KinhalCraftState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: KinhalCraftState): string {
  const family = KINHALCRAFT_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the KINHAL CRAFT textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within KINHAL CRAFT textile geometry and weaving logic.",
...
export function KinhalCraftModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "kinhalcraft",
            style_version: state.style,
            source: "homepage-kinhalcraft-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `kinhalcraft-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `kinhalcraft-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = KINHALCRAFT_PROMPT_FAMILIES[state.style];
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="KINHAL CRAFT Textile Generator"
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an KINHAL CRAFT-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
