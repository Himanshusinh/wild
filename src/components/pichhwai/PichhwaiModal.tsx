...
import { PICHHWAI_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/pichhwai/pichhwaiPromptCatalog";
...
import { PichhwaiHeader } from "./PichhwaiHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, PichhwaiState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "PICHHWAI";
...
function reducer(state: PichhwaiState, action: Action): PichhwaiState {
  switch (action.type) {
...
function buildPrompt(state: PichhwaiState): string {
  const family = PICHHWAI_PROMPT_FAMILIES[state.style];
...
export function PichhwaiModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const res = await dispatch(falGenerate({
        ...
        meta: { style_premium: true, style_key: "pichhwai", style_version: state.style, source: "homepage-pichhwai-modal" },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try { await downloadAllImageUrls(urls, `pichhwai-${state.style}`); toast.dismiss(t); toast.success("Downloads started"); } 
  ...
  const handleSaveImage = useCallback(async (index: number) => {
    ...
    try { await downloadImageUrl(url, `pichhwai-${state.style}-${index + 1}`); toast.dismiss(t); toast.success("Download started"); } 
    catch { toast.dismiss(t); toast.error("Save failed"); }
  }, [state.generatedImages, state.style]);

  if (!isOpen) return null;
  const familyMeta = PICHHWAI_PROMPT_FAMILIES[state.style];
        ...
        <div className="grid min-h-0 flex-1 overflow-hidden lg:[grid-template-columns:420px_1fr]">
          <aside className="flex flex-col overflow-hidden border-r border-white/10 bg-[#0E0E12]">
            ...
            <div className="border-t border-white/[0.06] bg-[#0E0E12] px-5 py-4">
              <button type="button" onClick={() => void handleGenerate()} disabled={state.panelState === "loading"} className="w-full rounded-lg bg-[#2F6BFF] py-2.5 text-[12px] font-semibold text-white transition hover:bg-[#2F6BFF]/90 disabled:opacity-50">Generate PICHHWAI</button>
            </div>
          </aside>
