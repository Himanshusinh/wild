...
import { wroughtironPromptCatalog } from "@/app/view/HomePage/compo/styles/wroughtiron/wroughtironPromptCatalog";
...
import { WroughtironHeader } from "./WroughtironHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, WroughtironState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "WROUGHT IRON";
...
function reducer(state: WroughtironState, action: Action): WroughtironState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: WroughtironState): string {
  const prompts = wroughtironPromptCatalog.prompts;
  const familyPrompt =
  ...
  return [
    ...
    state.inputMode === "image" && state.uploadedImage ? wroughtironPromptCatalog.promptI2I.trim() : familyPrompt.trim(),
    "",
    "PROJECT INPUTS:",
    projectLine,
    "",
...
export function WroughtironModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "wroughtiron",
            style_version: state.style,
            source: "homepage-wroughtiron-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `wroughtiron-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `wroughtiron-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Wrought Iron Generator"
