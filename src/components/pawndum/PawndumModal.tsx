...
import { PawndumHeader } from "./PawndumHeader";
import {
  INITIAL_STATE,
  type PawndumState,
  type StyleFamily,
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: PawndumState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: PawndumState, action: Action): PawndumState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: PawndumState): string {
  const family = PAWNDUM_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the PAWNDUM textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within PAWNDUM textile geometry and weaving logic.",
...
export function PawndumModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `pawndum-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `pawndum-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = PAWNDUM_PROMPT_FAMILIES[state.style];
  ...
  return (
    <div className="fixed inset-0 z-[160] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="PAWNDUM Textile Generator"
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an PAWNDUM-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
