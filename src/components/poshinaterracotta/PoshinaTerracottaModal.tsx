...
import { POSHINATERRACOTTA_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/poshinaterracotta/poshinaterracottaPromptCatalog";
...
import { PoshinaTerracottaHeader } from "./PoshinaTerracottaHeader";
import {
  INITIAL_STATE,
  type PoshinaTerracottaState,
  type StyleFamily,
  type InputMode,
  type ModelId,
  type ImageCount,
...
const STYLE_TAG = "PoshinaTerracotta";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: PoshinaTerracottaState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: PoshinaTerracottaState, action: Action): PoshinaTerracottaState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: PoshinaTerracottaState): string {
  const family = POSHINATERRACOTTA_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the POSHINA TERRACOTTA textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within POSHINA TERRACOTTA textile geometry and weaving logic.",
...
export function PoshinaTerracottaModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "poshinaterracotta",
            style_version: state.style,
            source: "homepage-poshinaterracotta-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `poshinaterracotta-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `poshinaterracotta-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = POSHINATERRACOTTA_PROMPT_FAMILIES[state.style];
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="POSHINA TERRACOTTA Textile Generator"
        ...
        <div className="grid min-h-0 flex-1 overflow-hidden lg:[grid-template-columns:420px_1fr]">
          <aside className="flex flex-col overflow-hidden border-r border-white/10 bg-[#0E0E12]">
            ...
            <div className="border-t border-white/[0.06] bg-[#0E0E12] px-5 py-4">
              ...
              >
                Generate POSHINA TERRACOTTA Textile
              </button>
            </div>
          </aside>
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an POSHINA TERRACOTTA-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
