...
import { NAGASHAWL_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/nagashawl/nagashawlPromptCatalog";
...
import { NagaShawlHeader } from "./NagaShawlHeader";
import {
  INITIAL_STATE,
  type NagaShawlState,
  type StyleFamily,
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: NagaShawlState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: NagaShawlState, action: Action): NagaShawlState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: NagaShawlState): string {
  const family = NAGASHAWL_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the NAGA SHAWL textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within NAGA SHAWL textile geometry and weaving logic.",
...
export function NagaShawlModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
      ...
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            ...
            source: "homepage-nagashawl-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `nagashawl-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `nagashawl-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = NAGASHAWL_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="NAGA SHAWL Textile Generator"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an NAGA SHAWL-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
