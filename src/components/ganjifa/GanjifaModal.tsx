...
import { GANJIFA_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/ganjifaPromptCatalog";
...
import { GanjifaHeader } from "./GanjifaHeader";
import {
  INITIAL_STATE,
  type GanjifaState,
  type StyleFamily,
...
import { GanjifaHeader } from "./GanjifaHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, GanjifaState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "GANJIFA CARDS";
>>>>>>> 41f03ea4f3321f74f1a8a57e5e42f0b4b4cb3e8e
...
function reducer(state: GanjifaState, action: Action): GanjifaState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: GanjifaState): string {
...
>>>>>>> 41f03ea4f3321f74f1a8a57e5e42f0b4b4cb3e8e
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `ganjifa-${state.style}-${index + 1}`);
        toast.dismiss(t);
