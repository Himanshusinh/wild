...
import { LIPPAN_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/lippan/lippanPromptCatalog";
...
import { LippanHeader } from "./LippanHeader";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: LippanState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: LippanState, action: Action): LippanState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: LippanState): string {
  const family = LIPPAN_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep Lippan wall logic: raised mud relief, embedded mirrors, and wall-bounded composition.";
  ...
  return [
    ...
    "- Keep full-frame Lippan identity with earthen relief massing, embedded mirrors, and architectural field order.",
    "- Preserve hand-built mud relief behavior, mirror inlay structure, and tactile earthen surface logic.",
    "- Keep relief-first wall construction and avoid flat painted or decor-only mirror styling.",
    "",
    "RENDER SETTINGS:",
...
export function LippanModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `lippan-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `lippan-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = LIPPAN_PROMPT_FAMILIES[state.style];
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. raised mud motifs, embedded mirror inlay, earthen wall texture"
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Lippan Kaam wall scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
