...
import { PichhwaiNewHeader } from "./PichhwaiNewHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, PichhwaiNewState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "PICHHWAINEW";
...
function reducer(state: PichhwaiNewState, action: Action): PichhwaiNewState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: PichhwaiNewState): string {
  const prompts = pichhwainewPromptCatalog.prompts;
  const familyPrompt =
  ...
  return [
    ...
    state.inputMode === "image" && state.uploadedImage ? pichhwainewPromptCatalog.promptI2I[state.style === "V1" ? "v1" : state.style === "V2" ? "v2" : "v3"].trim() : familyPrompt.trim(),
    "",
    "PROJECT INPUTS:",
    projectLine,
    "",
...
export function PichhwaiNewModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "pichhwainew",
            style_version: state.style,
            source: "homepage-pichhwainew-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `pichhwainew-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `pichhwainew-${state.style}-${index + 1}`);
        toast.dismiss(t);
