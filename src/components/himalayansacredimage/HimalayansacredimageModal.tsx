...
import { HIMALAYANSACREDIMAGE_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/himalayansacredimage/himalayansacredimagePromptCatalog";
...
import { HimalayansacredimageHeader } from "./HimalayansacredimageHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import {
  INITIAL_STATE,
  HimalayansacredimageState,
  StyleFamily,
  InputMode,
  ModelId,
  ImageCount,
...
const STYLE_TAG = "Himalayan Sacred Image";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: HimalayansacredimageState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: HimalayansacredimageState, action: Action): HimalayansacredimageState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: HimalayansacredimageState): string {
  const family = HIMALAYANSACREDIMAGE_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep iconometric proportion and mineral pigment sacred grammar.";
  ...
  return [
    ...
    "- Keep output in Ladakh sacred thangka iconometry and mineral pigment grammar.",
    "- Avoid western realism, digital-first styling, or non-iconometric proportions.",
    "- Keep border-and-center field structure coherent across the full frame.",
    "",
    "RENDER SETTINGS:",
...
export function HimalayansacredimageModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "himalayansacredimage",
            style_version: state.style,
            source: "homepage-himalayansacredimage-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `himalayan-sacred-image-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `himalayan-sacred-image-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = HIMALAYANSACREDIMAGE_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Himalayan Sacred Image Generator"
              ...
              {state.inputMode === "text" ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Optional notes... e.g. serene deity figure, mineral pigments, gold Guldris detail"
              ...
              {state.panelState === "empty" ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe a Sacred scene (or upload an image), then Generate.
                  </p>
                </div>
              ) : null}
