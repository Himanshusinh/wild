...
import { LEATHERTOYS_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/leathertoys/leathertoysPromptCatalog";
...
import { LeatherToysHeader } from "./LeatherToysHeader";
import {
  INITIAL_STATE,
  type LeatherToysState,
  type StyleFamily,
  type InputMode,
  type ModelId,
  type ImageCount,
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: LeatherToysState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: LeatherToysState, action: Action): LeatherToysState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: LeatherToysState): string {
  const family = LEATHERTOYS_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the LEATHER TOYS textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within LEATHER TOYS textile geometry and weaving logic.",
...
export function LeatherToysModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "leathertoys",
            style_version: state.style,
            source: "homepage-leathertoys-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `leathertoys-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `leathertoys-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = LEATHERTOYS_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="LEATHER TOYS Textile Generator"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an LEATHER TOYS-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
