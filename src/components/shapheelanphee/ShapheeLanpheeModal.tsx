...
import { SHAPHEELANPHEE_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/shapheelanphee/shapheelanpheePromptCatalog";
...
import { ShapheeLanpheeHeader } from "./ShapheeLanpheeHeader";
import {
  INITIAL_STATE,
  type ShapheeLanpheeState,
  type StyleFamily,
  type InputMode,
  type ModelId,
  type ImageCount,
...
const STYLE_TAG = "ShapheeLanphee";
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: ShapheeLanpheeState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: ShapheeLanpheeState, action: Action): ShapheeLanpheeState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: ShapheeLanpheeState): string {
  const family = SHAPHEELANPHEE_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the SHAPHEE LANPHEE textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within SHAPHEE LANPHEE textile geometry and weaving logic.",
...
export function ShapheeLanpheeModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "shapheelanphee",
            style_version: state.style,
            source: "homepage-shapheelanphee-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `shapheelanphee-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `shapheelanphee-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = SHAPHEELANPHEE_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="SHAPHEE LANPHEE Textile Generator"
        ...
        <div className="grid min-h-0 flex-1 overflow-hidden lg:[grid-template-columns:420px_1fr]">
          <aside className="flex flex-col overflow-hidden border-r border-white/10 bg-[#0E0E12]">
            ...
            <div className="border-t border-white/[0.06] bg-[#0E0E12] px-5 py-4">
              ...
              >
                Generate SHAPHEE LANPHEE Textile
              </button>
            </div>
          </aside>
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an SHAPHEE LANPHEE-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
