...
import { TIKULIART_PROMPT_FAMILIES } from "@/app/view/HomePage/compo/styles/tikuliart/tikuliartPromptCatalog";
...
import { TikuliArtHeader } from "./TikuliArtHeader";
import {
  INITIAL_STATE,
  type TikuliArtState,
  type StyleFamily,
...
type Action =
  ...
  | { type: "SET_PANEL_STATE"; payload: TikuliArtState["panelState"] }
  | { type: "SET_GENERATED_IMAGES"; payload: string[] }
  | { type: "SET_ASSEMBLED_PROMPT"; payload: string }
  | { type: "RESET" };

function reducer(state: TikuliArtState, action: Action): TikuliArtState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: TikuliArtState): string {
  const family = TIKULIART_PROMPT_FAMILIES[state.style];
  ...
  const projectLine = projectInputs
    ? `- ${projectInputs}`
    : "- (none). Keep the TIKULI ART textile language centered and coherent; avoid random motif drift.";
  ...
  return [
    ...
    "- Keep the output within TIKULI ART textile geometry and weaving logic.",
...
export function TikuliArtModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
      ...
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            ...
            source: "homepage-tikuliart-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `tikuliart-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `tikuliart-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  const familyMeta = TIKULIART_PROMPT_FAMILIES[state.style];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="TIKULI ART Textile Generator"
              ...
              {state.panelState === "empty" ? (
                <div className="p-5">
                  ...
                  <p className="max-w-[320px] text-xs leading-relaxed text-white/10">
                    Describe an TIKULI ART-inspired textile scene (or upload an image), then Generate.
                  </p>
                </div>
                </div>
              ) : null}
