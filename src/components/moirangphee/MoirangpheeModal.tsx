...
import { moirangpheePromptCatalog } from "@/app/view/HomePage/compo/styles/moirangphee/moirangpheePromptCatalog";
...
import { MoirangpheeHeader } from "./MoirangpheeHeader";
import { FullscreenImageViewer } from "@/components/common/FullscreenImageViewer";
import { INITIAL_STATE, MoirangpheeState, StyleFamily, InputMode, ModelId, ImageCount, AspectRatio, MODELS, STYLE_LABELS, RightPanelState } from "./types";

const STYLE_TAG = "MOIRANG PHEE";
...
function reducer(state: MoirangpheeState, action: Action): MoirangpheeState {
  switch (action.type) {
    case "SET_STYLE":
...
function buildPrompt(state: MoirangpheeState): string {
  const prompts = moirangpheePromptCatalog.prompts;
  const familyPrompt =
  ...
  return [
    ...
    state.inputMode === "image" && state.uploadedImage ? moirangpheePromptCatalog.promptI2I.trim() : familyPrompt.trim(),
    "",
    "PROJECT INPUTS:",
    projectLine,
    "",
...
export function MoirangpheeModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      const res = await dispatch(
        falGenerate({
          ...
          meta: {
            style_premium: true,
            style_key: "moirangphee",
            style_version: state.style,
            source: "homepage-moirangphee-modal",
          },
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      await downloadAllImageUrls(urls, `moirangphee-${state.style}`);
      toast.dismiss(t);
  ...
  const handleSaveImage = useCallback(
    async (index: number) => {
      ...
      try {
        await downloadImageUrl(url, `moirangphee-${state.style}-${index + 1}`);
        toast.dismiss(t);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/45 p-3 sm:p-6 backdrop-blur-2xl">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Moirang Phee Generator"
