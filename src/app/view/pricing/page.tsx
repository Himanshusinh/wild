  ...
  export default function Home() {
    ...
    return (
      <div className="min-h-screen bg-[#07070B]">
        ...
        <div className="flex md:ml-[68px] "> {/* spacing to account for persistent Nav + SidePanel */}
          <div className="flex-1 min-w-0">
            <main className="text-white min-h-screen">
              <section className="header md:space-y-4 space-y-2 md:px-6 px-4 py-0 md:py-6 w-full">
                <h2 className="head md:text-4xl text-2xl font-semibold text-center pt-10">Plans Made for Everyone</h2>
                <p className="line md:text-base text-sm text-center font-xs">Scalable solutions tailored to teams of all sizes.</p>

                {/* Toggle Switch - simplified style like reference */}
                <div className="flex items-center justify-center md:mt-6 mt-2 rounded-lg">
                  <div className="flex items-center md:gap-4 gap-2 rounded-lg bg-white/10 backdrop-blur-2xl backdrop-saturate-150 border border-white/10 shadow-xl p-2 ring-1 ring-white/10">
                    <button
                      onClick={() => setIsAnnual(false)}
                      aria-pressed={!isAnnual}
                      className={`md:text-base text-sm font-medium transition-colors ${!isAnnual ? 'text-white' : 'text-white/70'}`}
                    >
                      Monthly
                    </button>
                    <button
                      ...
                      className={`relative inline-flex md:h-6 h-6 md:w-12 w-8 items-center rounded-md transition-colors duration-300 ${isAnnual ? 'bg-[#1C303D]' : 'bg-white'}`}
                    >
                      <span
                    ...
                    <div className="relative flex items-center justify-center">
                      <div className=" md:ml-16 ml-2 absolute md:-top-6 -top-5 md:left-1/2 left-3/4 -translate-x-1/2 bg-green-500 text-white md:text-xs text-[8px] font-semibold md:px-3 px-2 md:py-1 py-1 rounded-lg whitespace-nowrap shadow-md">
              ...
              <section className="pricing-packages w-full  pt-2 flex flex-col items-center md:gap-10 gap-4 px-4 md:px-14">
                {/* {plans.map((p, i) => (
                ...
                <div className="flex flex-col md:flex-row flex-wrap md:gap-4 gap-2 justify-start w-full">
                  <CurrentPlan />
                  <StudentDiscount />
                </div>
