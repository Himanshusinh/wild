...
function CurrentPlan() {
  ...
  return (
    <div className="relative text-white rounded-lg
      ...
      <div className="min-h-[92px] pr-28 md:pr-0">
        <h1 className="text-xl sm:text-2xl font-semibold mx-2">Current Plan</h1>
        <p className="text-sm sm:text-base text-white/80 mt-1 mx-2">{loading ? '...' : (plan || 'Free')}</p>
      </div>
      ...
      <div className="mt-4 mx-2">
        {activeSince && (
          <div className="text-xs sm:text-sm text-white/70 mt-16 mb-2 ml-2">Active since: {activeSince}</div>
        )}
        <div className="flex items-center">
          <button className="mt-0 bg-[#1C303D] hover:bg-[#1c3c52] text-white rounded-full px-4 sm:px-5 py-2 text-medium font-regular ring-1 ring-white/15 transition-colors">
            Change Plan
          </button>
        </div>
      </div>
