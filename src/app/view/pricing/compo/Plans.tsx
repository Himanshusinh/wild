  ...
  return (  
    <div className="py-2 w-full">
      <div className="w-full">
        <h1 className="text-gray-900 dark:text-white text-4xl font-semibold text-left mb-6">Plans</h1>
        {(() => {
          const nonEnterprise = plans.filter((p) => p.name !== 'Enterprise');
          const enterprise = plans.find((p) => p.name === 'Enterprise');

          const firstRow = nonEnterprise.slice(0, 3);
          const secondRow = nonEnterprise.slice(3, 5);
          ...
          return (
            <>
              ...
              <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
                {firstRow.map((plan) => (
                  <div 
                    ...
                    border border-gray-200 dark:border-white/10 shadow-[0_8px_30px_rgba(0,0,0,0.1)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.35)]
                    ring-1 ring-gray-200 dark:ring-white/10
                    p-5 py-6
                    w-full overflow-hidden isolate flex flex-col
                    transition-all duration-300 hover:bg-white dark:hover:bg-white/10 hover:backdrop-saturate-200 hover:shadow-[0_12px_40px_rgba(0,0,0,0.15)] dark:hover:shadow-[0_12px_40px_rgba(0,0,0,0.45)] hover:border-gray-300 dark:hover:border-white/15`}
                  >
                    ...
                    <div className={`min-h-[120px]`}>
                        ...
                        {typeof plan.monthlyPrice !== 'undefined' && (
                          <div>
                            <div className={`flex items-baseline gap-2 mt-4`}>
                              ...
                              <div className={`text-sm md:text-base text-gray-900 dark:text-gray-600 dark:text-white/80`}>
                                / month
                              </div>
                            </div>
                            {isAnnual && typeof plan.monthlyPrice === 'number' && typeof plan.annualPrice === 'number' && (
                              <span className={`block text-blue-600 dark:text-[#ADD8E6] text-xs mt-1`}>
                      ...
                      {plan.subtitle && (
                        <p className={`text-sm text-gray-900 dark:text-gray-600 dark:text-white/70 mx-3 leading-snug break-words text-center`}>{plan.subtitle}</p>
                      )}
                    </div>

                    <div className={`mt-4 mb-3 mx-3`}>
                      <button className={`bg-blue-600 hover:bg-blue-700 dark:bg-[#1C303D] dark:hover:bg-[#1c3c52] text-white rounded-full px-5 py-2 text-sm font-medium ring-1 ring-gray-300 dark:ring-white/15 transition-colors w-full`}>
                        {plan.buttonText}
                      </button>
                    </div>

                    <ul className={`mt-2.5 space-y-2 mx-3 text-gray-900 dark:text-gray-700 dark:text-white/90`}>
                      {plan.features.map((f, i) => (
                        <li key={i} className="flex items-start gap-3">
                          <span className={`mt-1.5 h-1.5 w-1.5 rounded-full bg-gray-400 dark:bg-white/70 flex-shrink-0`} />
              ...
              {/* Second row: 2 cards (same sizing as first row, third column intentionally empty) */}
              <div className="mt-3 grid gap-3 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
                {secondRow.map((plan) => (
                  ...
                  >
                    ...
                    <div className="pointer-events-none absolute inset-0 rounded-[2rem] shadow-[inset_0_1px_0_0_rgba(0,0,0,0.05)] dark:shadow-[inset_0_1px_0_0_rgba(255,255,255,0.12)]" aria-hidden />
                    <div className={`min-h-[120px]`}>
                      <h3 className={`text-3xl font-medium leading-tight mx-4 mt-3 mb-1`}>{plan.name}</h3>
                      <div className={`mx-4 ${typeof plan.monthlyPrice !== 'undefined' ? 'min-h-[70px]' : 'min-h-0'}`}>
                        {typeof plan.monthlyPrice !== 'undefined' && (
                          <div>
                            <div className={`flex items-baseline gap-2 mt-4`}>
              ...
              {/* Last row: Enterprise full width */}
              {enterprise && (
                <div className="mt-3">
                  <div 
                    className={`relative text-gray-900 dark:text-white rounded-[2rem]
                  ...
                  >
                    ...
                    <div className={`min-h-[60px] ml-2`}>
                      <div className="flex items-center justify-between gap-3 mx-3 mt-2 mb-0.5">
                        ...
                        <button className={`bg-blue-600 hover:bg-blue-700 dark:bg-[#1C303D] dark:hover:bg-[#1c3c52] text-white rounded-full px-6 py-2.5 text-sm font-medium ring-1 ring-gray-300 dark:ring-white/15 transition-colors min-w-[180px]`}>
                      ...
                      {enterprise.subtitle && (
                        <p className={`text-sm text-gray-900 dark:text-gray-600 dark:text-white/70 mt-2 mx-3 leading-snug break-words`}>{enterprise.subtitle}</p>
                      )}
                    </div>

                    <div className={`mt-4 mx-3 text-gray-900 dark:text-gray-700 dark:text-white/90 grid grid-cols-1 sm:grid-cols-2 gap-x-6 pb-4`}>
                        ...
                        <ul key={colIdx} className={`space-y-2`}>
                          {col.map((f, i) => (
                            <li key={`${colIdx}-${i}`} className="flex items-start gap-3">
                              <span className={`mt-4 h-1.5 w-1.5 rounded-full bg-gray-400 dark:bg-white/70 flex-shrink-0`} />
