...
export const EditImageEraseControls: React.FC<EditImageEraseControlsProps> = ({
  ...
  return (
    <div className="flex flex-col gap-2 w-full">
      {/* Mode toggle */}
      <div className="flex bg-white/3 border border-white/12 rounded-xl p-1 w-full">
        <button
          onClick={() => setMode("replace")}
          className={`flex-1 px-3 py-2 text-[13px] font-semibold rounded-xl transition-colors ${mode === "replace" ? "bg-white text-black" : "text-white/80 hover:bg-white/10"}`}
        >
          Replace
        </button>
        <button
          onClick={() => setMode("erase")}
          className={`flex-1 px-3 py-2 text-[13px] font-semibold rounded-xl transition-colors ${mode === "erase" ? "bg-white text-black" : "text-white/80 hover:bg-white/10"}`}
        >
          Erase
        </button>
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="w-7 h-7 rounded-full bg-white/3 border border-white/12 hover:bg-white/10 flex items-center justify-center text-white/80 text-xs transition active:scale-95"
          >
            -
          </button>
          <input
