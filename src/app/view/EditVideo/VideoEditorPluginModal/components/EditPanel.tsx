...
interface EditPanelProps {
  ...
  onOpenEffectView?: (
    view: "text-effects" | "color" | "animate" | "font",
  ) => void;
  onAlign?: (
  ...
  setEraserSettings: React.Dispatch<
...
// --- Color Constants ---
const PHOTO_COLORS = ["#ffffff", "#d1ccc0", "#2c2c2c", "#a67c52", "#7d5fff"];

const GRADIENT_STYLES = [
  {
    id: "90deg",
    label: "Linear 90°",
    css: "linear-gradient(90deg, #e2e8f0, #64748b)",
    value: "90deg",
  },
  {
    id: "180deg",
    label: "Linear 180°",
    css: "linear-gradient(180deg, #e2e8f0, #64748b)",
    value: "180deg",
  },
  {
    id: "135deg",
    label: "Linear 135°",
    css: "linear-gradient(135deg, #e2e8f0, #64748b)",
    value: "135deg",
  },
  {
    id: "center",
    label: "Radial Center",
    css: "radial-gradient(circle at center, #e2e8f0, #64748b)",
    value: "circle at center",
  },
  {
    id: "top-left",
    label: "Radial Top-Left",
    css: "radial-gradient(circle at top left, #e2e8f0, #64748b)",
    value: "circle at top left",
  },
];
...
const EditPanel: React.FC<EditPanelProps> = ({
  ...
  const [cachedItem, setCachedItem] = useState<TimelineItem | null>(
    selectedItem,
  );
    ...
    onUpdate(
      {
        ...
        animation: {
          type: animId,
          category: "page",
          timing: itemToRender.animation?.timing || "both",
        },
      },
      true,
    );
  ...
  const handleAnimClick = (animId: string) => {
    ...
    const newAnim = {
      ...
      timing: itemToRender.animation?.timing || ("both" as const),
    };
  ...
  const [pickerTarget, setPickerTarget] = useState<"background" | "border">(
    "background",
  );
          ...
          <div
            ...
            style={{
              background:
                pickerTab === "solid"
                  ? pickerColor
                  : gradientStops[activeStopIndex]?.color || "#000",
            }}
          ></div>
          <input
            type="text"
            value={(pickerTab === "solid"
              ? pickerColor
              : gradientStops[activeStopIndex]?.color || "#000"
            ).toUpperCase()}
            onChange={(e) => {
    ...
    >
      <div className="flex border-b border-gray-100">
        ...
        >
          Solid
          {pickerTab === "solid" && (
            <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#2F6BFF] rounded-t-full mx-4"></div>
          )}
        </button>
        <button
      ...
      <div className="p-3">
        {pickerTab === "solid" ? (
          ...
          <>
            <div className="mb-3">
              <h4 className="text-[10px] font-bold text-gray-500 uppercase mb-1.5 tracking-wide">
                Gradient Colors
              </h4>
    ...
    <div className="animate-in slide-in-from-right duration-200 pb-24 relative min-h-full">
      <div className="sticky top-0 bg-white/80 backdrop-blur-sm z-40 pb-3 border-b border-gray-100 mb-3 -mx-1 px-1 pt-0">
        <button
          ...
          className="flex items-center gap-1 text-xs text-gray-500 hover:text-gray-900 font-medium mb-2"
        >
      ...
      <div className="space-y-4">
          ...
          {(
            ...
            >
              ...
              >
                ...
                {type === "lift" && (
                  <span style={{ textShadow: "0 10px 20px rgba(0,0,0,0.5)" }}>
                    Tx
                  </span>
                )}
                {type === "hollow" && (
                ...
                {type === "glitch" && (
                  <span style={{ textShadow: "2px 0 #ff00ff, -2px 0 #00ffff" }}>
                    Tx
                  </span>
                )}
                {type === "neon" && (
                  <span
                    style={{
                      textShadow: "0 0 5px #d946ef, 0 0 10px #d946ef",
                      color: "white",
                    }}
                  >
                    Tx
                ...
                {type === "background" && (
                  <span className="bg-gray-300 px-1 rounded">Tx</span>
                )}
              </div>
              <span className="text-[9px] font-medium capitalize text-gray-600 truncate w-full text-center">
                {type}
              </span>
            </button>
          ))}
        ...
        {itemToRender?.textEffect &&
          itemToRender.textEffect.type !== "none" && (
            <div className="space-y-4 pt-4 border-t border-gray-100">
              {[
                ...
                "glitch",
                "neon",
              ].includes(itemToRender.textEffect.type) && (
                <div className="space-y-1">
                  <div className="flex justify-between text-[10px] text-gray-500">
                    <span>Intensity</span>
                    <span>{itemToRender.textEffect.intensity}</span>
                  </div>
                  <input
                    type="range"
                    min="0"
                    max="100"
                    value={itemToRender.textEffect.intensity}
                    onChange={(e) =>
                      itemToRender &&
                      onUpdate({
                        ...itemToRender,
              ...
              {["shadow", "lift", "echo", "glitch", "splice"].includes(
                itemToRender.textEffect.type,
              ) && (
                <div className="space-y-1">
                  <div className="flex justify-between text-[10px] text-gray-500">
                    <span>Offset</span>
                    <span>{itemToRender.textEffect.offset}</span>
                  </div>
                  <input
                    type="range"
                    min="0"
                    max="100"
                    value={itemToRender.textEffect.offset}
                    onChange={(e) =>
                      itemToRender &&
                      onUpdate({
                        ...itemToRender,
              ...
              {["shadow", "splice", "outline", "neon", "background"].includes(
                itemToRender.textEffect.type,
              ) && (
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <input
                      type="color"
                      value={itemToRender.textEffect.color || "#000000"}
                      onChange={(e) =>
                        itemToRender &&
                        onUpdate({
                          ...itemToRender,
          ...
          <div className="space-y-1">
            {filteredFonts.map((font) => (
              <button
                ...
                onClick={() => {
                  itemToRender &&
                    onUpdate({ ...itemToRender, fontFamily: font.family });
                }}
              ...
              >
                {font.name}
                {itemToRender?.fontFamily === font.family && (
        ...
        <div className="space-y-1">
          <div className="flex justify-between text-xs text-gray-600">
            ...
            <span className="font-mono text-[10px] bg-gray-100 px-1.5 py-0.5 rounded">
              {Math.round(itemToRender?.width || 100)}%
            </span>
          </div>
          <input
            type="range"
      ...
      <div className="space-y-2 mb-3">
        <div className="space-y-1">
          <div className="flex justify-between text-xs text-gray-600">
            <span>Opacity</span>
            <span className="font-mono text-[10px] bg-gray-100 px-1.5 py-0.5 rounded">
              {Math.round(itemToRender?.opacity ?? 100)}%
            </span>
          </div>
          <input
            type="range"
            ...
            onChange={(e) =>
              itemToRender &&
              onUpdate({ ...itemToRender, opacity: Number(e.target.value) })
            }
            className="w-full accent-violet-600 h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer"
          />
        </div>
        ...
        <div className="space-y-1">
          ...
          <div className="relative">
            <input
              ...
              onChange={(e) =>
                itemToRender &&
                onUpdate({ ...itemToRender, x: Number(e.target.value) })
              }
        ...
        <div className="space-y-1">
          ...
          <div className="relative">
            <input
              ...
              onChange={(e) =>
                itemToRender &&
                onUpdate({ ...itemToRender, y: Number(e.target.value) })
              }
              className="w-full pl-2 pr-1 py-1.5 bg-gray-50 border border-gray-200 rounded text-xs text-gray-900 focus:border-[#2F6BFF] focus:ring-1 focus:ring-blue-200 outline-none transition-all font-mono"
            />
            <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-gray-400 pointer-events-none">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() =>
                ...
                onUpdate({
                  ...itemToRender,
                  rotation: (itemToRender.rotation || 0) - 90,
                })
              }
            ...
            <input
              ...
              onChange={(e) =>
                itemToRender &&
                onUpdate({ ...itemToRender, rotation: Number(e.target.value) })
              }
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <button
              onClick={() =>
                itemToRender &&
                onUpdate({ ...itemToRender, flipH: !itemToRender.flipH })
              }
            ...
            <button
              onClick={() =>
                itemToRender &&
                onUpdate({ ...itemToRender, flipV: !itemToRender.flipV })
              }
  ...
  const renderMainAdjust = () => (
    <div className="mb-4">
      ...
      {(itemToRender?.type === "image" || itemToRender?.type === "video") &&
        ...
        <div className="mb-6 border-b border-gray-100 pb-4">
          ...
          <div className="space-y-2 mb-3">
            ...
            <input
              ...
              onChange={(e) =>
                itemToRender &&
                onUpdate({ ...itemToRender, volume: Number(e.target.value) })
              }
              className="w-full accent-violet-600 h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer"
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              onChange={(e) =>
                itemToRender &&
                onUpdate({ ...itemToRender, speed: Number(e.target.value) })
              }
              className="w-full accent-violet-600 h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer"
            />
          </div>
      ...
      <div
        className={`grid grid-cols-6 gap-1.5 mb-4 ${itemToRender?.isBackground ? "opacity-50 pointer-events-none grayscale" : ""}`}
      >
        <button
          onClick={() => onAlign && onAlign("left")}
          className="p-2 bg-white border border-gray-100 rounded-lg flex items-center justify-center hover:border-blue-300 hover:text-[#2F6BFF] text-gray-600 transition-all"
          title="Align Left"
        >
          <AlignLeft size={16} />
        </button>
      ...
      <div className="grid grid-cols-2 gap-1.5 mb-2.5">
            ...
            <div className="w-8 h-8 rounded-md bg-gray-50 border border-gray-200 flex items-center justify-center group-hover:scale-105 transition-transform overflow-hidden">
              {itemToRender?.type === "color" ? (
                <div
                  ...
                  style={{ background: itemToRender.src }}
                ></div>
              ) : (
                <PaintBucket
                  size={16}
                  className="text-gray-600 group-hover:text-[#2F6BFF]"
                />
              )}
            </div>
            ...
            >
              ...
              <div className="min-w-0 w-full">
                ...
                <p className="text-[9px] text-gray-500 truncate">
                  {itemToRender.fontFamily?.split(",")[0].replace(/"/g, "") ||
                    "Default"}
                </p>
              </div>
            </button>
        ...
        >
          ...
          <div className="min-w-0 w-full">
            <p className="md:text-xs text-[10px] font-bold text-gray-800 truncate">
              Adjustments
            </p>
            <p className="md:text-[9px] text-[8px] text-gray-500 truncate">
              Light, color & more
            </p>
          </div>
        </button>
  ...
  const renderAnimateView = () => (
    <div className="animate-in slide-in-from-right duration-200 min-h-full pb-20">
      <div className="sticky top-0 bg-white/80 backdrop-blur-sm z-10 pb-2 border-b border-gray-100 mb-4 -mx-1 px-1 pt-0">
        <button
          onClick={() => setView("main")}
          className="flex items-center gap-1 text-xs text-gray-500 hover:text-gray-900 font-medium mb-3"
        >
          ...
          {["enter", "exit", "both"].map((t) => (
            <button
              ...
              onClick={() =>
                ...
                onUpdate({
                  ...itemToRender,
                  animation: { ...itemToRender.animation, timing: t as any },
                })
              }
              disabled={!itemToRender?.animation}
              className={`flex-1 py-1.5 text-[10px] font-bold rounded-md capitalize transition-all ${
                itemToRender?.animation?.timing === t ||
                (!itemToRender?.animation?.timing && t === "both")
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-4 gap-2">
          <button
            onClick={() =>
              itemToRender &&
              onUpdate({ ...itemToRender, animation: undefined })
            }
          ...
          {ANIMATIONS.filter((a) => a.category === animCategory).map((anim) => (
            ...
            >
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${itemToRender?.animation?.type === anim.id ? "bg-blue-100 text-[#2F6BFF]" : "bg-gray-100 text-gray-500 group-hover:bg-white group-hover:text-blue-500"}`}
              >
                {anim.icon}
              </div>
              <span className="text-[9px] font-medium text-gray-600 truncate w-full text-center group-hover:text-[#2F6BFF]">
                {anim.name}
              </span>
            </button>
          ))}
    ...
    <div className="animate-in slide-in-from-right duration-200 flex flex-col h-full pt-2">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className="text-xs font-bold text-[#2F6BFF] hover:text-[#2a5fe3]"
        >
          Exit
        </button>
      </div>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-gray-100">
        ...
        <button
          onClick={() =>
            setEraserSettings({
              ...eraserSettings,
              showOriginal: !eraserSettings.showOriginal,
            })
          }
        ...
        >
          <div
            className={`w-4 h-4 bg-white rounded-full shadow-sm transition-transform duration-200 ease-in-out ${eraserSettings.showOriginal ? "translate-x-4" : "translate-x-0"}`}
          ></div>
        </button>
      </div>
      ...
      <div className="mt-auto pt-4 border-t border-gray-100 grid grid-cols-2 gap-2">
        <button
          onClick={() =>
            itemToRender && onUpdate({ ...itemToRender, maskImage: undefined })
          }
  ...
  return (
    <div
      className={`bg-white md:h-full h-auto border-r border-gray-200 flex flex-col transition-all duration-300 ease-in-out overflow-hidden relative z-20 ${isOpen ? "md:w-80 w-36" : "w-0 opacity-0"}`}
    >
          ...
          <>
            {view !== "eraser" && (
              <div className="md:h-12 h-10 flex items-center justify-between md:px-4 px-3 border-b border-gray-100 shrink-0 bg-white z-10">
                <span className="font-bold text-gray-800 capitalize md:text-lg text-[10px] tracking-tight">
                  {getHeaderTitle()}
                </span>
                <button
                  onClick={onClose}
                  className="md:p-2 p-1 hover:bg-gray-100 rounded-full text-gray-500 transition-colors"
                >
            ...
            <div
              className={`md:flex-1 flex-none overflow-y-auto custom-scrollbar relative ${view === "eraser" ? "p-3" : "p-3"}`}
            >
              {view === "main" && (
                  ...
                  {itemToRender.type !== "text" && (
                    <div className="mb-6">
                      <div className="flex items-center justify-between md:mb-3 mb-2">
                        <h3 className="md:text-[11px] text-[10px] font-bold text-gray-500 uppercase tracking-wider">
                          Filters
                        </h3>
                      </div>
                      <div className="grid md:grid-cols-4 grid-cols-2 gap-2">
                        {FILTERS.map((f) => (
                          <div
                            key={f.id}
                            className="flex flex-col md:items-center md:gap-1 gap-0.5 group cursor-pointer"
                            onClick={() =>
                              itemToRender &&
                              onUpdate({
                                ...itemToRender,
                          ...
                          >
                            <div
                              className={`md:w-full w-6 md:aspect-square aspect-square rounded-lg overflow-hidden border-2 transition-all relative ${itemToRender.filter === f.id ? "border-[#2F6BFF] ring-2 ring-blue-100" : "border-transparent hover:border-gray-200"}`}
                            >
                              <div className="md:w-full w-6 md:h-full h-6 bg-gray-100 relative">
                                <img
                                  src={
                                    itemToRender.thumbnail || itemToRender.src
                                  }
                                  className="md:w-full w-6 md:h-full h-6 object-cover"
                            ...
                            <span
                              className={`text-[9px] font-bold truncate w-full text-center ${itemToRender.filter === f.id ? "text-[#2F6BFF]" : "text-gray-400"}`}
                            >
                              {f.name}
                            </span>
                          </div>
                ...
                <div className="animate-in slide-in-from-right duration-200">
                  <div className="sticky md:top-0 top-12 bg-white/80 backdrop-blur-sm z-10 md:pb-4 pb-3 border-b border-gray-100 md:mb-4 mb-3 -mx-1 px-1 pt-0">
                      ...
                      <div className="md:space-y-3 space-y-2 md:pl-1 pl-0.5">
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            className="w-full accent-violet-600 md:h-1 h-0.5 bg-gray-200 rounded-lg appearance-none cursor-pointer"
                          />
                        </div>
                        <div className="space-y-1">
                          <div className="flex justify-between">
                    ...
                    <button
                      ...
                      className="w-full py-2 border border-gray-200 rounded-lg text-xs font-bold text-gray-500 hover:bg-gray-50 transition-colors"
                    >
                      Reset
                    </button>
                  </div>
