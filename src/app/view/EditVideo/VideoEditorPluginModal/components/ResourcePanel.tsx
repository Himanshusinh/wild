...
interface ResourcePanelProps {
  ...
  onAddClip: (
    src: string,
    type: "video" | "image" | "color" | "text" | "audio",
...
const ResourcePanel: React.FC<ResourcePanelProps> = ({
  ...
  const renderText = () => (
    <div className="md:p-4 p-2 md:space-y-6 space-y-3 pb-24">
      ...
      <div className="md:pt-4 pt-2 border-t border-gray-100">
        <h4 className="font-bold md:text-sm text-[10px] text-gray-700 md:mb-3 mb-1">
          Alignment
        </h4>
        <div
          className={`grid md:grid-cols-6 grid-cols-3 gap-1.5 ${!selectedItem ? "opacity-50 pointer-events-none grayscale" : ""}`}
        >
          <button
            onClick={() => onAlign("left")}
            className="md:p-2 p-1 bg-white border border-gray-200 rounded-lg flex items-center justify-center hover:border-blue-300 hover:text-[#2F6BFF] text-gray-600 transition-all"
            title="Align Left"
          >
            <AlignLeft size={16} />
          </button>
      ...
      <div className="pt-4 border-t border-gray-100">
        <h4 className="font-bold md:text-sm text-[10px] text-gray-700 md:mb-4 mb-2">
          Font Combinations
        </h4>
        <div className="grid md:grid-cols-2 grid-cols-2 md:gap-3 gap-1">
          {FONT_COMBINATIONS.map((combo) => (
            ...
            >
              ...
              <p className="md:text-[10px] text-[8px] text-gray-400 md:mt-2 mt-1 font-sans">
                {combo.name}
              </p>
            </div>
          ))}
  ...
  const renderUploads = () => (
    <div className="md:p-4 p-2 h-full flex flex-col pb-24">
      ...
      >
        <Plus className="md:size-4 size-3" size={18} /> Upload files
      </button>

      <div className="md:flex-1 flex-auto md:overflow-y-auto overflow-y-scroll custom-scrollbar">
        <h4 className="md:text-xs text-[10px] font-bold text-gray-500 uppercase tracking-wider md:mb-3 mb-1">
          Recent Uploads
        </h4>
        {uploads.length > 0 ? (
          <div className="grid md:grid-cols-2 grid-cols-2 md:gap-3 gap-1">
            {uploads.map((item) => (
              <div
                ...
                onClick={() =>
                  onAddClip(item.src, item.type as any, {
                    thumbnail: item.thumbnail,
                  })
                }
              >
                ...
                {item.type === "video" && (
                  <>
                    ...
                    <div className="absolute inset-0 flex items-center justify-center bg-black/20 group-hover:bg-black/10 transition-colors">
                      <Play
                        className="md:size-4 size-3 text-white fill-current"
                        size={20}
                      />
                    </div>
                    {item.duration && (
                ...
                {item.type === "audio" && (
                  <div className="w-full h-full flex flex-col items-center justify-center bg-blue-50 text-[#2F6BFF] md:p-2 p-1 text-center">
                    ...
                    <span className="md:text-xs text-[10px] font-medium truncate w-full">
                      {item.name}
                    </span>
                    {item.duration && (
                      <span className="md:text-[10px] text-[8px] text-gray-400 md:mt-1 mt-0.5">
                        {item.duration}
                      </span>
                    )}
                  </div>
      ...
      <div className="md:p-4 p-2 pb-24">
        <div className="md:mb-4 mb-2 flex items-center bg-gray-100 rounded-lg md:px-3 px-2 border border-transparent focus-within:border-[#2F6BFF] focus-within:bg-white transition-all">
          ...
          <input
            ...
            className="w-full bg-transparent md:p-2.5 p-1.5 text-sm outline-none"
          />
        </div>

        <div className="flex items-center justify-between md:mb-3 mb-1">
          <h3 className="font-bold text-gray-800 md:text-sm text-[10px]">
            Your Audio
          </h3>
          <button
            onClick={onUpload}
            ...
            {userAudio.map((audio) => (
              ...
              >
                ...
                <div
                  ...
                  onClick={() =>
                    onAddClip(audio.src, "audio", {
                      name: audio.name,
                      duration: parseDurationString(audio.duration),
                    })
                  }
                >
                  <p className="md:text-xs text-[10px] font-bold text-gray-700 truncate">
                    {audio.name}
                  </p>
                  <p className="md:text-[10px] text-[8px] text-gray-400">
                    Imported • {audio.duration || "0:00"}
                  </p>
                </div>
                <button
                  onClick={() =>
                    onAddClip(audio.src, "audio", { name: audio.name })
                  }
        ...
        <h3 className="font-bold text-gray-800 md:mb-3 mb-1 md:text-sm text-[10px]">
          Stock Audio
        </h3>
        <div className="md:space-y-2 space-y-1 md:mb-6 mb-3">
  ...
  const renderContent = () => {
    switch (activeTab) {
            ...
            <div className="grid md:grid-cols-2 grid-cols-2 md:gap-3 gap-1">
              {MOCK_IMAGES.map((img) => (
                <div
                  ...
                  className="md:aspect-[4/3] aspect-video rounded-lg overflow-hidden cursor-grab active:cursor-grabbing relative group"
                  onClick={() => onAddClip(img.src, "image")}
                >
                  <img
                    src={img.src}
                    alt={img.name}
                  ...
                  onDragStart={(e) => {
                    e.dataTransfer.setData(
                      ...
                      JSON.stringify({
                        ...
                        duration: parseDurationString(vid.duration),
                      }),
                    );
                  }}
                  className="md:aspect-video aspect-square rounded-lg overflow-hidden cursor-grab active:cursor-grabbing relative group bg-black shadow-sm hover:shadow-md transition-all"
                  onClick={() => onAddClip(vid.src, "video")}
                >
                  <img
                    src={vid.thumbnail}
                    alt={vid.name}
            ...
            <div className="grid md:grid-cols-2 grid-cols-2 md:gap-4 gap-1">
              {MOCK_PROJECTS.map((p) => (
                <div key={p.id} className="group cursor-pointer">
                  ...
                  <p className="md:text-xs text-[10px] font-bold text-gray-700 truncate group-hover:text-[#2F6BFF]">
                    {p.name}
                  </p>
                  <p className="md:text-[10px] text-[8px] text-gray-400">
                    {p.lastModified}
      ...
      default:
        return (
          <div className="md:p-4 p-2 text-gray-500 md:text-sm text-[10px]">
  ...
  return (
    <div
      className={`bg-white h-full border-r border-gray-200 flex flex-col transition-all duration-300 ease-in-out overflow-hidden relative z-20 ${isOpen ? "md:w-80 w-36" : "w-0 opacity-0"}`}
    >
      {/* 
      ...
      <div className="md:w-80 w-36 h-full flex flex-col overflow-x-hidden">
        <div className="md:h-16 h-10 flex items-center justify-between md:px-5 px-3 border-b border-gray-100 shrink-0 bg-white z-10">
          <span className="font-bold text-gray-800 capitalize md:text-lg text-[10px] tracking-tight">
            {activeTab === "images" ? "Photos" : activeTab}
          </span>
          <button
            onClick={onClose}
