...
export default function CharacterSheet() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (response.data?.responseStatus === 'success' && response.data?.data?.images?.[0]?.url) {
        ...
        toast.success('Character sheet generated successfully!');
      } else {
      ...
      toast.error(error.response?.data?.message || error.message || 'Failed to generate character sheet');
    } finally {
  ...
  const handleDownload = async () => {
    ...
    try {
      await downloadFileWithNaming(generatedImage, null, 'image', 'character-sheet');
                    ...
                    {originalImage ? (
                      ...
                      <>
                        ...
                        <div className="text-center">
                          ...
                          <span className="text-[10px] text-slate-500">Full body suggested</span>
                        </div>
                      </>
                    )}
                  </div>
                ...
                <div className="relative w-full h-full flex items-center justify-center p-8">
                  ...
                  {isGenerating && (
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center z-10 transition-all duration-500">
                      ...
                      <p className="text-white font-medium text-lg animate-pulse">Generating character sheet...</p>
                    </div>
                  )}
                </div>
              ) : (
