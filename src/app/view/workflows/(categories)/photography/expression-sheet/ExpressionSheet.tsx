...
export default function ExpressionSheet() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (response.data?.responseStatus === 'success' && response.data?.data?.images?.[0]?.url) {
        ...
        toast.success('Expression sheet generated successfully!');
      } else {
      ...
      toast.error(error.response?.data?.message || error.message || 'Failed to generate expression sheet');
    } finally {
  ...
  const handleDownload = async () => {
    ...
    try {
      await downloadFileWithNaming(generatedImage, null, 'image', 'expression-sheet');
                    ...
                    {originalImage ? (
                      <>
                        ...
                        <div className="relative z-10">
                          <span className="text-xs text-white font-medium bg-black/50 px-3 py-1.5 rounded-full backdrop-blur text-center">Change Character Image</span>
                        </div>
                      </>
                    ) : (
                      <>
                        ...
                        <div className="text-center">
                          <span className="text-sm text-slate-300 block font-medium">Upload Character Image</span>
                          <span className="text-[10px] text-slate-500">Portrait suggested</span>
                        </div>
                      </>
                    )}
                  </div>
                ...
                <div className="relative w-full h-full flex items-center justify-center p-8">
                  ...
                  {isGenerating && (
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center z-10 transition-all duration-500">
                      ...
                      <p className="text-white font-medium text-lg animate-pulse">Generating expression sheet...</p>
                    </div>
                  )}
                </div>
              ) : (
