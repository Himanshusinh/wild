...
import { X, Camera, Zap, Download, Sparkles, Box } from 'lucide-react';
...
export default function DynamicCameraAngle() {
  ...
  // Workflow Data
  const workflowData = (WORKFLOWS_DATA.find(w => w.id === "dynamic-camera-angle") || {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/api/workflows/photography/dynamic-camera-angle', {
        image: productImage,
        angle: selectedAngle,
        additionalDetails,
        isPublic: true
      ...
      if (response.data?.responseStatus === 'success' && response.data?.data?.images?.[0]?.url) {
        ...
        toast.success('Dynamic angle shot generated successfully!');
      } else {
        throw new Error(response.data?.message || 'Invalid response from server');
      }

    } catch (error: any) {
      console.error('Dynamic Camera Angle error:', error);
      rollbackOptimisticDeduction(CREDIT_COST);
      toast.error(error.response?.data?.message || error.message || 'Failed to generate dynamic angle');
    } finally {
  ...
  const handleDownload = async () => {
    ...
    try {
      await downloadFileWithNaming(generatedImage, null, 'image', 'dynamic-camera-angle');
              ...
              <div className="flex-1">
                ...
                <div className="mb-4">
                  ...
                  <textarea
                    ...
                    placeholder="E.g. Minimalist background, professional lighting, soft shadows..."
                  ></textarea>
                </div>
              </div>
                ...
                <div className="relative w-full h-full flex items-center justify-center p-8">
                  ...
                  {isGenerating && (
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center z-10 transition-all duration-500">
                      ...
                      <p className="text-white font-medium text-lg animate-pulse">Reimagining product angle...</p>
                    </div>
                  )}
                </div>
              ) : (
