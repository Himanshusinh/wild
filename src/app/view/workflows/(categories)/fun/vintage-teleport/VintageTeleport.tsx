...
export default function VintageTeleport() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/api/workflows/fun/vintage-teleport', {
        image: originalImage,
        isPublic: true,
      ...
      console.error('Vintage Teleport error:', error);
      rollbackOptimisticDeduction(CREDIT_COST);
      toast.error(error.response?.data?.message || error.message || 'Failed to teleport');
    } finally {
  ...
  const handleDownload = async () => {
    ...
    try {
      await downloadFileWithNaming(generatedImage, null, 'image', 'vintage-teleport-result');
          ...
          <div className="flex w-full h-full flex-col md:flex-row shadow-2xl">
              ...
              <div className="flex-1 pb-6">
                ...
                <div className="mb-8">
                  ...
                  <textarea
                    ...
                    placeholder="Describe specific vintage details or instructions..."
                  ></textarea>
                </div>
              </div>
            ...
            <div className="w-full md:flex-1 h-[45%] md:h-full items-center justify-center bg-[#050505] relative overflow-hidden flex border-t md:border-t-0 md:border-l border-white/10 shrink-0 shadow-inner">
              ...
              {originalImage && generatedImage ? (
                ...
                <div className="relative w-full h-full flex items-center justify-center p-8">
                  ...
                  {isGenerating && (
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center z-10 transition-all duration-500">
                      ...
                      <p className="text-white font-medium text-lg animate-pulse">Teleporting...</p>
                    </div>
                  )}
                </div>
              ) : (
                <div className="relative w-full h-full flex items-center justify-center p-8">
                  <ImageComparisonSlider
                    beforeImage="https://idr01.zata.ai/devstoragev1/public/workflow-samples/vintage-teleport-before.avif"
                    afterImage="https://idr01.zata.ai/devstoragev1/public/workflow-samples/vintage-teleport-after.avif"
                    beforeLabel="Before"
                    afterLabel="After"
                    imageFit="object-contain"
                    imagePosition="object-center"
