...
export default function ChangeSeasons() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || error.message || 'Failed to change season');
    } finally {
  ...
  const handleDownload = async () => {
    ...
    try {
      await downloadFileWithNaming(generatedImage, null, 'image', 'change-seasons-result');
            ...
            <div className="w-full md:flex-1 h-[45%] md:h-full items-center justify-center bg-[#050505] relative overflow-hidden flex border-t md:border-t-0 md:border-l border-white/10 shrink-0 shadow-inner">
              ...
              {originalImage && generatedImage ? (
                ...
                <div className="relative w-full h-full flex items-center justify-center p-8 transition-all duration-300">
                  ...
                  {isGenerating && (
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center z-10 transition-all duration-500">
                      ...
                      <p className="text-white font-medium text-lg animate-pulse">Changing season...</p>
                    </div>
                  )}
                </div>
              ) : (
                <div className="relative w-full h-full flex items-center justify-center p-8">
                  <ImageComparisonSlider
                    beforeImage="https://idr01.zata.ai/devstoragev1/public/workflow-samples/change-seasons-before.avif"
                    afterImage="https://idr01.zata.ai/devstoragev1/public/workflow-samples/change-seasons-after.avif"
                    beforeLabel="Before"
                    afterLabel="After"
                    imageFit="object-cover"
                    imagePosition="object-center"
