...
export default function CCTVFootage() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || error.message || 'Failed to generate CCTV footage');
    } finally {
  ...
  const handleDownload = async () => {
    ...
    try {
      await downloadFileWithNaming(generatedImage, null, 'image', 'cctv-footage-result');
          ...
          <div className="flex w-full h-full flex-col md:flex-row">
            ...
            <div className="w-full md:w-[40%] h-[55%] md:h-full p-8 lg:p-12 flex flex-col border-r border-white/5 bg-[#0A0A0A] relative z-20 overflow-y-auto">
              <div className="flex-1">
                ...
                {/* Optional Text Box */}
                <div className="mb-8">
                  <label className="block text-slate-400 text-sm font-medium mb-3">Additional Instructions (Optional)</label>
                  <textarea
                    value={additionalText}
                    onChange={(e) => setAdditionalText(e.target.value)}
                    placeholder="E.g., Dark warehouse, flickering lights, surveillance style..."
                    className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white text-sm placeholder:text-slate-600 focus:outline-none focus:border-[#60a5fa]/50 focus:ring-1 focus:ring-[#60a5fa]/50 transition-all resize-none h-32"
                  />
                </div>
              </div>
            ...
            <div className="w-full md:flex-1 h-[45%] md:h-full items-center justify-center bg-[#050505] relative overflow-hidden flex border-t md:border-t-0 md:border-l border-white/10 shrink-0">
              ...
              {originalImage && generatedImage ? (
                ...
                <div className="relative w-full h-full flex items-center justify-center p-8">
                  ...
                  {isGenerating && (
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center z-10 transition-all duration-500">
                      ...
                      <p className="text-white font-medium text-lg animate-pulse">Generating CCTV footage...</p>
                    </div>
                  )}
                </div>
              ) : (
                <div className="relative w-full h-full flex items-center justify-center p-8">
                  <img
                    src="/workflow-samples/cctv-footage.jpg"
                    alt="CCTV Footage Example"
