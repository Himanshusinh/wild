...
export default function RemoveWatermark() {
    ...
    const handleRun = async () => {
        ...
        try {
            ...
            const response = await axiosInstance.post('/api/workflows/general/remove-watermark', payload);
            ...
            toast.error(error.response?.data?.message || error.message || 'Failed to remove watermark');
        } finally {
    ...
    const handleDownload = async () => {
        ...
        try {
            await downloadFileWithNaming(generatedImage, null, 'image', 'watermark-removed');
            ...
            <div className={`fixed inset-0 z-[80] flex items-center justify-center px-4 md:pl-20 transition-all duration-300 ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}>
                ...
                <div className={`relative w-full max-w-6xl h-[90vh] bg-[#0A0A0A] border border-white/10 rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row transition-all duration-500 ${isOpen ? 'scale-100 translate-y-0' : 'scale-95 translate-y-10'}`}>
                  ...
                  />
                                ...
                                <div className="relative w-full h-full flex items-center justify-center p-8">
                                    <ImageComparisonSlider
                                        beforeImage="https://idr01.zata.ai/devstoragev1/public/workflow-samples/remove-watermark-before-v2.avif"
                                        afterImage="https://idr01.zata.ai/devstoragev1/public/workflow-samples/remove-watermark-after-v2.avif"
                                        beforeLabel="Before"
                                        afterLabel="After"
                                        imageFit="object-cover"
                    imagePosition="object-center"
