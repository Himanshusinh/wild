...
export default function LineDrawingToPhoto() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const payload = {
        image: originalImage,
        image_input: [originalImage],
        uploadedImages: [originalImage],
      ...
      const response = await axiosInstance.post(
        "/api/workflows/general/line-drawing-to-photo",
        payload,
      );
      ...
      toast.error(
        error.response?.data?.message || error.message || "Failed to generate",
      );
    } finally {
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-0 z-[80] flex items-center justify-center px-4 md:pl-20 transition-all duration-300 ${isOpen ? "opacity-100 visible" : "opacity-0 invisible pointer-events-none"}`}
      >
        <div
          className="absolute top-0 right-0 bottom-0 left-0 md:left-20 bg-black/80 backdrop-blur-xl"
          onClick={onClose}
        ></div>
        ...
        <div
          className={`relative w-full max-w-6xl h-[90vh] bg-[#0A0A0A] border border-white/10 rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row transition-all duration-500 ${isOpen ? "scale-100 translate-y-0" : "scale-95 translate-y-10"}`}
        >
          <div className="flex w-full h-full flex-col md:flex-row">
            ...
            <div className="w-full md:w-[40%] h-[55%] md:h-full p-8 lg:p-12 flex flex-col border-r border-white/5 bg-[#0A0A0A] relative z-20 overflow-y-auto">
              <div className="flex-1">
                ...
                <div className="mb-8">
                  <WorkflowUploadArea
                    placeholderLabel="Upload Line Drawing"
                  ...
                  {isGenerating ? (
                    ...
                    <>
                      <Zap
                        size={16}
                        className={
                          !originalImage ? "fill-slate-500" : "fill-black"
                        }
                      />
                      Run Workflow
                    </>
            ...
            <div className="w-full md:flex-1 h-[45%] md:h-full items-center justify-center bg-[#050505] relative overflow-hidden flex border-t md:border-t-0 md:border-l border-white/10 shrink-0">
              ...
              {originalImage && generatedImage ? (
                ...
                <div className="relative w-full h-full flex items-center justify-center p-8">
                  <ImageComparisonSlider
                    beforeImage="https://idr01.zata.ai/devstoragev1/public/workflow-samples/line-to-photo-before-v2.avif"
                    afterImage="https://idr01.zata.ai/devstoragev1/public/workflow-samples/line-to-photo-after-v2.avif"
                    beforeLabel="Line Drawing"
                    afterLabel="Photorealistic"
                    imageFit="object-cover"
                    imagePosition="object-center"
