...
export default function InteriorRefine() {
  ...
  // Workflow Data
  const workflowData = WORKFLOWS_DATA.find(w => w.id === "interior-refine") || {
    id: "interior-refine",
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/api/workflows/architecture/interior-refine', {
        image: originalImage,
        isPublic: true
      });
      ...
      if (response.data?.responseStatus === 'success' && response.data?.data?.images?.[0]?.url) {
        ...
        toast.success('Interior refined successfully!');
      } else {
        throw new Error(response.data?.message || 'Invalid response from server');
      }

    } catch (error: any) {
      console.error('Interior Refine error:', error);
      rollbackOptimisticDeduction(CREDIT_COST);
      toast.error(error.response?.data?.message || error.message || 'Failed to refine interior');
    } finally {
  ...
  const handleDownload = async () => {
    ...
    try {
      await downloadFileWithNaming(generatedImage, null, 'image', 'interior-refined');
          ...
          <div className="flex w-full h-full">
            <div className="w-full md:w-[40%] p-8 lg:p-12 flex flex-col border-r border-white/5 bg-[#0A0A0A] relative z-20 overflow-y-auto">
              <div className="flex-1">
                    ...
                    {originalImage ? (
                      <>
                        ...
                        <div className="relative z-10 flex flex-col items-center gap-2">
                          <span className="text-white font-medium bg-black/50 px-3 py-1 rounded-full backdrop-blur">Change Interior</span>
                        </div>
                      </>
                    ) : (
                      <>
                        ...
                        <div className="text-center">
                          <span className="text-sm text-slate-300 block font-medium">Upload Interior</span>
                ...
                <div className="mb-4">
                  <label className="text-xs font-bold uppercase text-slate-500 mb-2 block">REFINE OPTIONS (OPTIONAL)</label>
                  <textarea
                    ...
                    placeholder="E.g. Warmer lighting, add plants, modern furniture style..."
                  ></textarea>
                </div>
              </div>
                ...
                <div className="relative w-full h-full flex items-center justify-center p-8">
                  ...
                  {isGenerating && (
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center z-10 transition-all duration-500">
                      ...
                      <p className="text-white font-medium text-lg animate-pulse">Refining interior...</p>
                    </div>
                  )}
                </div>
              ) : (
