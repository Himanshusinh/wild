...
export default function InteriorProduct() {
  ...
  // Workflow Data
  const workflowData = WORKFLOWS_DATA.find(w => w.id === "interior-product") || {
    id: "interior-product",
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/api/workflows/architecture/interior-product', {
        image: originalImage,
      ...
      if (response.data?.responseStatus === 'success' && response.data?.data?.images?.[0]?.url) {
        ...
        toast.success('Product display generated successfully!');
      } else {
        throw new Error(response.data?.message || 'Invalid response from server');
      }

    } catch (error: any) {
      console.error('Interior Product error:', error);
      rollbackOptimisticDeduction(CREDIT_COST);
      toast.error(error.response?.data?.message || error.message || 'Failed to generate product display');
    } finally {
  ...
  const handleDownload = async () => {
    ...
    try {
      await downloadFileWithNaming(generatedImage, null, 'image', 'interior-product');
          ...
          <div className="flex w-full h-full">
            <div className="w-full md:w-[48%] p-8 lg:p-12 flex flex-col border-r border-white/5 bg-[#0A0A0A] relative z-20 overflow-y-auto">
              <div className="flex-1">
                ...
                <div className="mb-8">
                  <label className="text-xs font-bold uppercase text-slate-500 mb-4 block">Interior Space Elements (Select Multiple)</label>
                ...
                <div className="mb-4">
                  <label className="text-xs font-bold uppercase text-slate-500 mb-2 block">DISPLAY OPTIONS (OPTIONAL)</label>
                  <textarea
                    className="w-full bg-black/20 border border-white/10 rounded-xl p-4 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-[#60a5fa]/50 focus:bg-black/30 transition-all resize-none h-32 mb-6"
                    placeholder="E.g. Luxury living room, minimal bedroom, studio lighting..."
                  ></textarea>

                  <div className="mb-4">
                    <label className="text-[10px] font-bold uppercase text-slate-500 mb-4 block">FILE TYPE</label>
                    <div className="flex gap-3">
                      {['PNG', 'SVG', 'JPG'].map((type) => (
                        <button
                          key={type}
                          onClick={() => setFileType(type)}
                          className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all border ${fileType === type
            ...
            <div className="hidden md:flex flex-1 items-center justify-center bg-[#050505] relative overflow-hidden">
              ...
              {originalImage && generatedImage ? (
                <div className="relative w-full h-full flex items-center justify-center p-8 bg-white/5 overflow-hidden">
                  <div className="w-full h-full bg-[#f8f9fa] rounded-2xl shadow-inner border border-white/5 flex items-center justify-center overflow-hidden p-4">
                    <img
                      src={generatedImage}
                ...
                <div className="relative w-full h-full flex items-center justify-center p-8">
                  ...
                  {isGenerating && (
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center z-10 transition-all duration-500">
                      ...
                      <p className="text-white font-medium text-lg animate-pulse">Generating product display...</p>
                    </div>
                  )}
                </div>
              ) : (
