...
export default function GenerateTexture() {
  ...
  // Workflow Data
  const workflowData = WORKFLOWS_DATA.find(w => w.id === "generate-texture") || {
    id: "generate-texture",
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/api/workflows/architecture/generate-texture', {
        image: originalImage,
        isPublic: true
      });
      ...
      toast.error(error.response?.data?.message || error.message || 'Failed to generate texture');
    } finally {
  ...
  const handleDownload = async () => {
    ...
    try {
      await downloadFileWithNaming(generatedImage, null, 'image', 'texture-generated');
          ...
          <div className="flex w-full h-full flex-col md:flex-row">
            <div className="w-full md:w-[40%] h-[55%] md:h-full p-8 lg:p-12 flex flex-col border-r border-white/5 bg-[#0A0A0A] relative z-20 overflow-y-auto">
              <div className="flex-1">
                    ...
                    {originalImage ? (
                      <>
                        ...
                        <div className="relative z-10 flex flex-col items-center gap-2">
                          <span className="text-white font-medium bg-black/50 px-3 py-1 rounded-full backdrop-blur">Change Reference</span>
                        </div>
                      </>
                    ) : (
                      <>
                ...
                <div className="mb-4">
                  <label className="text-xs font-bold uppercase text-slate-500 mb-2 block">TEXTURE STYLE (OPTIONAL)</label>
                  <textarea
                    ...
                    placeholder="E.g. Polished oak, weathered stone, futuristic metal grid..."
                  ></textarea>
                </div>
              </div>
                ...
                <div className="relative w-full h-full flex items-center justify-center p-8">
                  ...
                  {isGenerating && (
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center z-10 transition-all duration-500">
                      ...
                      <p className="text-white font-medium text-lg animate-pulse">Generating texture...</p>
                    </div>
                  )}
                </div>
              ) : (
