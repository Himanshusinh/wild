...
export default function StyleTransferViral() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || error.message || 'Failed to process style transfer');
    } finally {
  ...
  const handleDownload = async () => {
    ...
    try {
      await downloadFileWithNaming(generatedImage, null, 'image', 'style-transfer-result');
          ...
          <div className="flex w-full h-full flex-col md:flex-row">
              ...
              <div className="flex-1">
                ...
                <div className="mb-8">
                  <div className="border border-dashed border-white/15 rounded-xl bg-black/20 h-48 flex flex-col items-center justify-center gap-4 cursor-pointer hover:bg-[#60a5fa]/5 transition-colors relative overflow-hidden group"
                    ...
                    {contentImage ? (
                      <>
                        <img src={contentImage} className="absolute inset-0 w-full h-full object-cover opacity-50 group-hover:opacity-30 transition-opacity" alt="Content" />
                ...
                <div className="mb-8 animate-in fade-in slide-in-from-top-5 duration-1000">
                  ...
                  <textarea
                    ...
                    placeholder="Describe specific details for the style transfer..."
                  ></textarea>
                </div>
              </div>
              ...
              <div className="mt-auto pt-6 border-t border-white/5">
                ...
                >
                  {isGenerating ? (
                    ...
                    <>
                      <Zap size={16} className={(!contentImage) ? "fill-slate-500" : "fill-black"} />
                      Run Workflow
                    </>
                  )}
                </button>
            ...
            <div className="w-full md:flex-1 h-[45%] md:h-full items-center justify-center bg-[#050505] relative overflow-hidden flex border-t md:border-t-0 md:border-l border-white/10 shrink-0">
              ...
              {contentImage && generatedImage ? (
                ...
                <div className="relative w-full h-full flex items-center justify-center p-8">
                  <img src={contentImage} className="max-w-full max-h-full object-contain rounded-lg shadow-2xl" alt="Preview" />
                  {isGenerating && (
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center z-10 transition-all duration-500">
                      ...
                      <p className="text-white font-medium text-lg animate-pulse">Generatng style transfer...</p>
                    </div>
                  )}
                </div>
              ) : (
