  ...
  return (
    <div className="relative dropdown-container">
      ...
      >
        ...
        <ChevronUp className={`w-3 h-3 md:w-4 md:h-4 transition-transform duration-200 ${activeDropdown === 'product-generation-mode' ? 'rotate-180' : ''}`} />
      </button>
      {activeDropdown === 'product-generation-mode' && (
        <div className="absolute bottom-full left-0 mb-2 max-w-44 w-20 md:w-64 bg-black/70 backdrop-blur-3xl rounded-lg overflow-hidden ring-1 ring-white/30 pb-1.5 md:pb-2 pt-1.5 md:pt-2 max-h-[50vh] overflow-y-auto dropdown-scrollbar">
          {modes.map((mode) => (
            <button
              key={mode.value}
              onClick={(e) => {
