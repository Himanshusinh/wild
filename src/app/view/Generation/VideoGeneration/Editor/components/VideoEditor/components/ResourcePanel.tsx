      ...
      try {
        ...
        if (response.responseStatus === "success" && response.data) {
          ...
          if (response.data.pagination) {
            ...
            setLibraryHasMore(
              p.hasMoreImages || p.hasMoreVideos || p.hasMoreUploaded,
            );
          } else {
        ...
        >
          <div className="grid grid-cols-2 gap-3">
            {items.map((item, index) => {
              ...
              const isVideo =
                item.type === "video" || mediaUrl.match(/\.(mp4|webm|mov)$/i);
              const isMusic =
                item.type === "music" || mediaUrl.match(/\.(mp3|wav|ogg)$/i);
              ...
              return (
                <div
                  ...
                  onDragStart={(e) =>
                    handleDragStart(
                      e,
                      item.type === "music" ? "audio" : item.type,
                      mediaUrl,
                      {
                        name: item.prompt
                  ...
                  onClick={() =>
                    onAddClip(
                      ...
                      item.type === "music"
                        ? "audio"
                        : (item.type as any) || (isVideo ? "video" : "image"),
                      {
                        name: item.prompt
                          ? item.prompt.length > 30
                            ? item.prompt.substring(0, 30) + "..."
  ...
  const renderText = () => (
    <div className="p-4 space-y-6 pb-24">
      ...
      <div className="flex gap-2 w-1/2">
        ...
        >
          <span className="text-sm font-bold text-gray-700 group-hover:text-violet-600">
            H1
          </span>
        </button>
        <button
        ...
        <button
          ...
          className="flex-1 aspect-square bg-white border border-gray-200 hover:border-violet-500 hover:bg-violet-50 rounded-lg flex items-center justify-center transition-all group"
        >
          <span className="text-[10px] font-normal text-gray-700 group-hover:text-violet-600">
            Aa
          </span>
        </button>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 aspect-square bg-white border border-gray-200 hover:border-violet-500 hover:bg-violet-50 rounded-lg flex items-center justify-center transition-all text-gray-600 hover:text-violet-600"
          >
            <AlignLeft size={18} />
          </button>
          <button
            ...
            <div className="grid grid-cols-2 gap-2">
              {MOCK_IMAGES.map((img) => (
                <div
                  ...
                  className="aspect-[4/3] rounded-lg overflow-hidden cursor-grab active:cursor-grabbing relative group"
            ...
            <div className="grid grid-cols-1 gap-3">
              {MOCK_VIDEOS.map((vid) => (
                <div
                  ...
                  className="aspect-video rounded-lg overflow-hidden cursor-grab active:cursor-grabbing relative group bg-black shadow-sm hover:shadow-md transition-all"
            ...
            <div className="grid grid-cols-2 gap-4">
              {MOCK_PROJECTS.map((p) => (
                <div key={p.id} className="group cursor-pointer">
                  ...
                  <p className="text-xs font-bold text-gray-700 truncate group-hover:text-violet-700">
                    {p.name}
                  </p>
