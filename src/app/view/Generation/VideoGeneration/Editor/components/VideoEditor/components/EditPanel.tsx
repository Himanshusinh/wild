...
const EditPanel = ({
    ...
    >
      <div className="flex border-b border-gray-100">
        ...
        >
          Solid
          {pickerTab === "solid" && (
            <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-violet-600 rounded-t-full mx-4"></div>
          )}
        </button>
        <button
  ...
  const renderTransformSection = () => (
    <div className="mb-4 border-b border-gray-100 pb-4">
      ...
      {/* Scale */}
      <div
        className={`space-y-2 mb-3 ${itemToRender?.isBackground ? "opacity-50 pointer-events-none grayscale" : ""}`}
      >
        <div className="space-y-1">
      ...
      {/* Position */}
      <div
        className={`grid grid-cols-2 gap-2 mb-3 ${itemToRender?.isBackground ? "opacity-50 pointer-events-none grayscale" : ""}`}
      >
        <div className="space-y-1">
    ...
    <div className="mb-4">
          ...
          <div className="flex items-center justify-between mb-3 p-2 bg-gray-50 rounded-lg">
            ...
            <button
              onClick={() =>
                ...
                onUpdate({
                  ...itemToRender,
                  muteVideo: !itemToRender.muteVideo,
                })
              }
            ...
            >
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${itemToRender.muteVideo ? "translate-x-5" : "translate-x-0.5"}`}
              />
            </button>
          </div>
      ...
      >
        <button
          ...
          className="p-2 bg-white border border-gray-100 rounded-lg flex items-center justify-center hover:border-violet-300 hover:text-violet-600 text-gray-600 transition-all"
          title="Align Left"
        >
          <AlignLeft size={16} />
        </button>
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-4 gap-2">
          ...
          {ANIMATIONS.filter((a) => a.category === animCategory).map((anim) => (
            ...
            >
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${itemToRender?.animation?.type === anim.id ? "bg-violet-100 text-violet-600" : "bg-gray-100 text-gray-500 group-hover:bg-white group-hover:text-violet-500"}`}
              >
                {anim.icon}
              </div>
              <span className="text-[9px] font-medium text-gray-600 truncate w-full text-center group-hover:text-violet-700">
                {anim.name}
              </span>
            </button>
          ))}
  ...
  return (
    <div
      className={`bg-white h-full border-r border-gray-200 flex flex-col transition-all duration-300 ease-in-out overflow-hidden relative z-20 ${isOpen ? "w-80" : "w-0 opacity-0"}`}
    >
      <div className="w-80 h-full flex flex-col overflow-x-hidden">
        {itemToRender && (
          <>
            ...
            <div
              className={`flex-1 overflow-y-auto custom-scrollbar relative ${view === "eraser" ? "p-3" : "p-3"}`}
            >
              {view === "main" && (
                    ...
                    <div className="mb-6">
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        {FILTERS.map((f) => (
                          ...
                          >
                            <div
                              className={`w-full aspect-square rounded-lg overflow-hidden border-2 transition-all relative ${itemToRender.filter === f.id ? "border-violet-600 ring-2 ring-violet-100" : "border-transparent hover:border-gray-200"}`}
                            >
                            ...
                            <span
                              className={`text-[9px] font-bold truncate w-full text-center ${itemToRender.filter === f.id ? "text-violet-600" : "text-gray-400"}`}
                            >
                              {f.name}
                            </span>
                          </div>
