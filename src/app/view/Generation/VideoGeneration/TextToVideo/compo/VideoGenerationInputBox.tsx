...
const VideoGenerationInputBox = () => {
  ...
  const removeReferenceImage = (index: number) => {
    setState((prev) => ({
      ...
      videoToVideo: {
        ...prev.videoToVideo,
        references:
          prev.videoToVideo.references?.filter((_, i) => i !== index) || [],
      },
    }));
  };
  ...
  const validateGeneration = (): { isValid: boolean; error?: string } => {
    if (state.mode === "image_to_video") {
      ...
      if (Array.isArray(state.imageToVideo.promptImage)) {
        const hasLast = state.imageToVideo.promptImage.some(
          (p) => p.position === "last",
        );
        if (hasLast && state.imageToVideo.model !== "gen3a_turbo") {
          return {
            isValid: false,
            error: 'position: "last" is only supported by Gen-3a Turbo',
          };
        }
      }
    } else {
      if (!state.videoToVideo.videoUri) {
        return {
          isValid: false,
          error: "Please upload a video for video-to-video generation",
        };
      }
      if (!state.videoToVideo.promptText) {
        return {
          isValid: false,
          error: "Please provide a prompt for video-to-video generation",
        };
      }
    }
  ...
  const handleGenerate = async () => {
    ...
    const currentPrompt =
      state.mode === "image_to_video"
        ? state.imageToVideo.promptText || ""
        : state.videoToVideo.promptText;
    const currentModel =
      state.mode === "image_to_video"
        ? state.imageToVideo.model
        : state.videoToVideo.model;
    const currentRatio =
      state.mode === "image_to_video"
        ? state.imageToVideo.ratio
        : state.videoToVideo.ratio;
    ...
    try {
      ...
      const sku = (() => {
        if (state.mode === "image_to_video") {
          ...
          if (state.imageToVideo.model === "gen4_turbo")
            return `Gen-4  Turbo ${dur}s`;
          if (state.imageToVideo.model === "gen3a_turbo")
            return `Gen-3a  Turbo ${dur}s`;
        } else {
      ...
      const finalStatus = await waitForRunwayVideoCompletion(
        ...
        (progress, status) => {
          ...
          if (firebaseHistoryId) {
            updateHistoryEntry(firebaseHistoryId, {
  ...
  const getCurrentState = () =>
    state.mode === "image_to_video" ? state.imageToVideo : state.videoToVideo;
            ...
            {state.mode === "image_to_video" ? (
              <>
                <option value="gen4_turbo">
                  Gen-4 Turbo (
                  {getModelCreditInfo("gen4_turbo", "5s")?.credits || 0}{" "}
                  credits)
                </option>
              </>
            ) : (
              <option value="gen4_aleph">
                Gen-4 Aleph ({getModelCreditInfo("gen4_aleph")?.credits || 0}{" "}
                credits)
              </option>
            )}
          </select>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1 text-xs text-white/60">
                  <HelpCircle className="w-3 h-3" />
                  <span>
                    Add 1–5 images to guide style/palette. Motion comes from
                    your video.
                  </span>
                </div>
              </div>
              ...
              {state.videoToVideo.references &&
                state.videoToVideo.references.length > 0 && (
        ...
        {showAdvanced && (
          <div className="p-4 bg-white/5 rounded-lg space-y-4">
            <div className="flex items-center gap-4">
              ...
              <select
                value={
                  getCurrentState().contentModeration?.publicFigureThreshold ||
                  "auto"
                }
                onChange={(e) => {
