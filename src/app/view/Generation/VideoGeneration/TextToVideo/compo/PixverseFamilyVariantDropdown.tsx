...
interface PixverseVariantOption {
  value: string;
  label: string;
  info?: string;
}

interface PixverseFamilyVariantDropdownProps {
  options: PixverseVariantOption[];
  selectedValue: string;
  onChange: (value: string) => void;
  onCloseOtherDropdowns?: () => void;
}

const PixverseFamilyVariantDropdown: React.FC<
  PixverseFamilyVariantDropdownProps
> = ({ options, selectedValue, onChange, onCloseOtherDropdowns }) => {
  ...
  const dropdownId = "pixverse-family-variant-dropdown";
  ...
  const dropdownContent =
    isOpen && dropdownPosition ? (
      <div
        ...
        className="fixed z-[9999] w-[min(calc(100vw-32px),248px)] max-w-[min(calc(100vw-32px),248px)] overflow-hidden rounded-lg border border-white/15 bg-black/90 shadow-2xl backdrop-blur-3xl md:w-auto md:min-w-[260px] md:max-w-[min(calc(100vw-24px),360px)]"
        style={{
...
export default PixverseFamilyVariantDropdown;
