 ...
 */
export const normalizeGenerationType = (
...
export const isVideoType = (entry: any): boolean => {
  const normalizedType = normalizeGenerationType(
    entry?.generationType || entry?.params?.generationType,
  );
  return [
    "text-to-video",
    "image-to-video",
 ...
 */
export const convertFrameSizeToMiniMaxResolution = (
  frameSize: string,
): string => {
 ...
 */
export const waitForMiniMaxVideoCompletion = async (
  taskId: string,
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      const { data: statusEnvelope } = await api.get(
        "/api/minimax/video/status",
      ...
      if (status === "Success" && fileId) {
        ...
        try {
          ...
          const { data: fileEnvelope } = await api.get(
            "/api/minimax/video/file",
            {
              params: {
                file_id: fileId,
                ...(opts?.historyId ? { history_id: opts.historyId } : {}),
              },
            },
          );
          ...
          if (
            fileData?.file &&
            (fileData.file.download_url || fileData.file.backup_download_url)
          ) {
            return {
              status: "Success",
              download_url:
          ...
          if (Array.isArray(fileData?.videos) && fileData.videos[0]?.url) {
            return {
              status: "Success",
              download_url: fileData.videos[0].url,
              videos: fileData.videos,
            };
          }
          ...
          console.warn(
            "⚠️ File retrieval failed, but video generation was successful. Video should be available in database.",
          );
        ...
        return {
          status: "Fail",
          error: statusData?.base_resp?.status_msg || "Generation failed",
        };
      } else if (
        status === "Queueing" ||
        status === "Preparing" ||
  ...
  console.error(
    "⏰ MiniMax video completion timeout after",
    maxAttempts,
    "attempts",
  );
...
export const getCleanPrompt = (text: string): string => {
  try {
    return (text || "").replace(/\[\s*Style:\s*[^\]]+\]/i, "").trim();
  } catch {
    return text;
  }
};
...
export const getModelCapabilities = (model: string): ModelCapabilities => {
  ...
  if (model === "seedance-2.0-t2v") {
    ...
    (model.includes("ltx-2.3-fast") || model.includes("ltx-2.3-pro")) &&
    !model.includes("i2v")
  ) {
