                      ...
                      <div key={`local-only-${idx}`} className="relative w-48 h-48 rounded-lg overflow-hidden bg-black/40 backdrop-blur-xl ring-1 ring-white/10">
                        {localGeneratingEntries[0].status === 'generating' ? (
                          ...
                          <div className="relative w-full h-full group">
                            ...
                            <div className="pointer-events-none absolute bottom-0 left-0 right-0 rounded-t-xl bg-white/5 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity px-2 py-1 flex items-center gap-2 min-h-[40px]">
                              <span title={getCleanPrompt(localGeneratingEntries[0].prompt)} className="text-xs text-white flex-1 leading-snug" style={{ display: '-webkit-box', WebkitLineClamp: 3 as any, WebkitBoxOrient: 'vertical' as any, overflow: 'hidden' }}>
                                {getCleanPrompt(localGeneratingEntries[0].prompt)}
                              </span>
                              <button aria-label="Copy prompt" className="pointer-events-auto p-1 rounded hover:bg-white/20 text-white/90 transition-colors" onClick={(e) => { e.stopPropagation(); e.preventDefault(); e.nativeEvent.stopImmediatePropagation(); copyPrompt(e, getCleanPrompt(localGeneratingEntries[0].prompt)); }} onMouseDown={(e) => { e.stopPropagation(); e.preventDefault(); e.nativeEvent.stopImmediatePropagation(); }} onMouseUp={(e) => { e.stopPropagation(); e.preventDefault(); e.nativeEvent.stopImmediatePropagation(); }} onMouseEnter={(e) => { e.stopPropagation(); }} onMouseLeave={(e) => { e.stopPropagation(); }}>
                        ...
                        >
                          {entry.status === 'generating' ? (
                            ...
                            <div className="relative w-full h-full group">
                              ...
                              <div className="shimmer absolute inset-0 opacity-100 transition-opacity duration-300" />
                              <div className="pointer-events-none absolute bottom-0 left-0 right-0 rounded-t-xl bg-black/40 backdrop-blur-3xl opacity-0 group-hover:opacity-100 transition-opacity px-2 py-1 flex items-center gap-2 min-h-[40px]">
                                <span title={getCleanPrompt(entry.prompt)} className="text-xs text-white/90 flex-1 leading-snug" style={{ display: '-webkit-box', WebkitLineClamp: 3 as any, WebkitBoxOrient: 'vertical' as any, overflow: 'hidden' }}>
