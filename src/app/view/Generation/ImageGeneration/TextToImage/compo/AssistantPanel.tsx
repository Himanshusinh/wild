...
import { deductCreditsOptimistic, rollbackCreditsOptimistic, syncCreditsWithBackend } from '@/store/slices/creditsSlice';
import {
...
const MODE_META: Record<AssistantMode, { title: string; subtitle: string }> = {
    agent: {
        ...
        subtitle: 'Use the assistant to shape prompts, refine ideas, and turn replies into image-ready generations.',
    },
    chat: {
        title: 'Chat with frontier models',
        subtitle: 'Switch models, ask anything naturally, and keep the conversation focused in one clean workspace.',
    },
};
...
function parseThreadResponse(data: any): { thread: AssistantThread | null; messages: ChatMessage[] } {
...
const AssistantPanel: React.FC<AssistantPanelProps> = ({ isOpen, onClose, onApplyPrompt }) => {
    ...
    const effectiveModelId = activeThread?.modelId ?? (effectiveMode === 'chat' ? draftModelId : AGENT_DEFAULT_MODEL_ID);
    const currentModelLabel = effectiveMode === 'chat' ? getChatModelLabel(effectiveModelId) : 'Agent workflow';
    const isGeminiThread = effectiveMode === 'chat' && effectiveModelId === GEMINI_MODEL_ID;
    const threadRailWidth = isThreadRailCollapsed ? THREAD_RAIL_COLLAPSED_WIDTH : THREAD_RAIL_EXPANDED_WIDTH;
    const isCompactLayout = panelWidth < 560;
    const panelHorizontalPadding = isCompactLayout ? 'px-3' : 'px-5';
    const imageAttachmentCount = composerAttachments.filter((item) => item.type === 'image').length;
    const videoAttachmentCount = composerAttachments.filter((item) => item.type === 'video').length;
    const audioAttachmentCount = composerAttachments.filter((item) => item.type === 'audio').length;
    ...
    useEffect(() => {
        const applyViewportConstraints = () => {
            ...
            setPanelWidth((prev) => Math.min(Math.max(MIN_PANEL_WIDTH, prev), nextMax));
    ...
    const loadThread = useCallback(async (threadId: string) => {
        ...
        try {
            const res = await fetch(`/api/assistant/threads/${threadId}?limit=120`, { cache: 'no-store' });
    ...
    const loadThreads = useCallback(async (options?: { selectThreadId?: string; preserveDraft?: boolean }) => {
        setIsThreadsLoading(true);
        try {
            const res = await fetch('/api/assistant/threads?limit=40', { cache: 'no-store' });
            const data = await res.json();
            ...
            if (!activeThread && !isDraftingNewThread && !options?.preserveDraft && nextThreads[0]) {
    ...
    const validateAttachmentSelection = (
        ...
        const rule = GEMINI_ATTACHMENT_LIMITS[type];
        const nextCount = (
            type === 'image' ? imageAttachmentCount :
                type === 'video' ? videoAttachmentCount :
    ...
    const uploadAttachments = async (type: AssistantAttachment['type'], fileList: FileList | null) => {
    ...
    const handleSendChat = async () => {
        ...
        const currentAttachments = effectiveMode === 'chat' ? composerAttachments : [];
        ...
        try {
            const endpoint = effectiveMode === 'chat' ? '/api/assistant/chat-mode' : '/api/assistant/chat';
            const chatModelInput = effectiveMode === 'chat' && effectiveModelId === GEMINI_MODEL_ID
                ? GEMINI_DEFAULT_INPUT
                : undefined;
            const payload = effectiveMode === 'chat'
                ? {
                    message: text,
                    ...
                    modelInput: chatModelInput
                        ? {
                            ...chatModelInput,
                            images: currentAttachments.filter((item) => item.type === 'image').map((item) => item.url),
                            videos: currentAttachments.filter((item) => item.type === 'video').map((item) => item.url),
                            audio: currentAttachments.find((item) => item.type === 'audio')?.url ?? null,
                        }
                        : undefined,
    ...
    return (
        <div
            className={`fixed top-[64px] right-0 bottom-0 z-[100] transition-all duration-500 ease-in-out transform ${isOpen ? 'translate-x-0' : 'translate-x-full'
        ...
        >
            <div
                className={`absolute left-0 top-0 bottom-0 z-[2] w-3 cursor-ew-resize transition-opacity ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'
                    }`}
                onMouseDown={handleResizeStart}
                title="Resize assistant panel"
            >
                <div className="flex h-full w-full items-center justify-center">
                    <div className={`rounded-full border border-white/10 bg-black/35 p-1 text-zinc-500 transition-colors ${isResizing ? 'text-blue-300 border-blue-400/30' : ''}`}>
            ...
            >
                <div className="flex items-center justify-between gap-2">
                    <button
                        ...
                        className={`inline-flex items-center justify-center gap-1 rounded-xl bg-blue-600 py-2.5 text-[11px] font-medium text-white hover:bg-blue-500 transition-colors ${isThreadRailCollapsed ? 'w-10 px-0' : 'flex-1 px-3'
                            }`}
                        title="New chat"
                    >
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        >
                            {isThreadRailCollapsed ? <PanelLeftOpen className="w-4 h-4" /> : <PanelLeftClose className="w-4 h-4" />}
                        </button>
                ...
                <div className="mt-4 flex-1 min-h-0">
                    ...
                    <div className="space-y-2 overflow-y-auto pr-1 no-scrollbar">
                        {isThreadsLoading && threads.length === 0 ? (
                            <div className={`rounded-2xl border border-white/8 bg-white/[0.03] text-[11px] text-zinc-500 ${isThreadRailCollapsed ? 'p-2 text-center' : 'p-3'}`}>
                                {isThreadRailCollapsed ? '...' : 'Loading...'}
                            </div>
                        ) : threads.length === 0 ? (
                            <div className={`rounded-2xl border border-white/8 bg-white/[0.03] text-[11px] leading-5 text-zinc-500 ${isThreadRailCollapsed ? 'p-2 text-center' : 'p-3'}`}>
                                {isThreadRailCollapsed ? '0' : 'Your saved assistant chats will appear here.'}
                            </div>
                        ) : (
                            threads.map((thread) => {
                                    ...
                                    >
                                        {isThreadRailCollapsed ? (
                                            <div className="flex flex-col items-center gap-1 text-zinc-400">
                                                {thread.mode === 'agent' ? <Bot className="w-4 h-4" /> : <MessageSquare className="w-4 h-4" />}
                                                <span className="text-[9px]">{thread.messageCount}</span>
                                            </div>
                                        ) : (
                                            <>
                                                <div className="flex items-center justify-between gap-2 text-[10px] text-zinc-500">
                                                    <div className="flex items-center gap-1.5 min-w-0">
                                                        {thread.mode === 'agent' ? <Bot className="w-3 h-3" /> : <MessageSquare className="w-3 h-3" />}
                                                        <span className="truncate">{thread.mode === 'chat' ? getChatModelLabel(thread.modelId) : 'Agent'}</span>
                                                    </div>
                                                    <span className="shrink-0 text-[9px] text-zinc-600">{formatRelativeTime(thread.updatedAt || thread.createdAt)}</span>
                                                </div>
            ...
            <div className="flex-1 min-w-0 flex flex-col">
                <div className={`flex items-center justify-between ${panelHorizontalPadding} py-4 border-b border-white/5`}>
                            ...
                            <div className="flex items-center gap-2 text-[11px] text-zinc-500 truncate">
                                ...
                                {activeThread && (
                                    <span className="inline-flex items-center gap-1 shrink-0 text-zinc-600">
                                        ...
                                        <span>{formatRelativeTime(activeThread.updatedAt || activeThread.createdAt)}</span>
                                    </span>
                                )}
                            </div>
                        </div>
                    ...
                    {activeThread && (
                        <button
                            type="button"
                            onClick={() => {
                                void fetch(`/api/assistant/threads/${activeThread.id}`, { method: 'DELETE' });
                                handleStartNewChat();
                                setThreads((prev) => prev.filter((item) => item.id !== activeThread.id));
                            }}
                ...
                <div className={`flex-1 overflow-y-auto ${panelHorizontalPadding} py-5 no-scrollbar`}>
                        ...
                        <div className="h-full flex items-center justify-center">
                            <div className="w-full max-w-[360px] rounded-3xl border border-white/8 bg-white/[0.03] px-6 py-7 text-center shadow-[0_20px_80px_rgba(0,0,0,0.35)]">
                                <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-2xl border border-blue-400/20 bg-blue-500/10 text-blue-300">
                                    {effectiveMode === 'agent' ? <Bot className="w-5 h-5" /> : <MessageSquare className="w-5 h-5" />}
                                </div>
                                <p className="text-white text-base font-semibold">{MODE_META[effectiveMode].title}</p>
                                ...
                                <div className="mt-4 inline-flex items-center rounded-full border border-white/8 bg-black/25 px-3 py-1 text-[10px] text-zinc-500">
                                    {effectiveMode === 'chat' ? `Ready for ${currentModelLabel}` : 'Start with an idea, prompt, or creative ask'}
                                </div>
                            </div>
                        </div>
                    )}
                                    ...
                                    >
                                        {msg.role === 'assistant' && (
                                            <div className="shrink-0 w-7 h-7 rounded-2xl bg-blue-500/15 border border-blue-400/20 flex items-center justify-center mt-0.5 text-blue-300">
                                                {effectiveMode === 'agent' ? <Bot className="w-3.5 h-3.5" /> : <MessageSquare className="w-3.5 h-3.5" />}
                                            </div>
                                        )}
                                        <div className={`flex flex-col gap-1.5 ${isCompactLayout ? 'max-w-[96%]' : 'max-w-[92%]'}`}>
                                            <div className={`rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed whitespace-pre-wrap shadow-[0_12px_40px_rgba(0,0,0,0.22)] ${msg.role === 'user'
                                                        ...
                                                        >
                                                            {attachment.type === 'image' ? (
                                                                ...
                                                                <>
                                                                    ...
                                                                    <span className="max-w-[140px] truncate">{attachmentLabel(attachment)}</span>
                                                                </>
                                                            )}
                                                        </a>
                                                    ))}
                                            ...
                                            {msg.createdAt && (
                                                <div className={`px-1 text-[10px] ${msg.role === 'user' ? 'text-right text-blue-200/70' : 'text-zinc-600'}`}>
                ...
                <div className={`${isCompactLayout ? 'p-3' : 'p-4'} border-t border-white/5 bg-black/40 backdrop-blur-md`}>
                    <input
                        ref={imageInputRef}
                        type="file"
                    ...
                    <div className="rounded-[24px] border border-white/10 bg-white/[0.04] p-3 shadow-[0_-10px_40px_rgba(0,0,0,0.25)] focus-within:border-blue-500/40 focus-within:ring-1 focus-within:ring-blue-500/30 transition-all">
                        ...
                        {isGeminiThread && (
                            <div className="mb-2">
                                ...
                                <button
                                    ...
                                    disabled={isUploadingAttachments || imageAttachmentCount >= GEMINI_ATTACHMENT_LIMITS.image.maxCount}
                                ...
                                >
                                    ...
                                    <span>Images {imageAttachmentCount}/{GEMINI_ATTACHMENT_LIMITS.image.maxCount}</span>
                                </button>
                                <button
                                    type="button"
                                    onClick={() => videoInputRef.current?.click()}
                                    disabled={isUploadingAttachments || videoAttachmentCount >= GEMINI_ATTACHMENT_LIMITS.video.maxCount}
                                    className="inline-flex items-center gap-1 rounded-full border border-white/10 bg-black/30 px-2.5 py-1 text-[10px] text-zinc-300 hover:bg-white/[0.06] transition-colors"
                                >
                                    <Film className="w-3 h-3" />
                                    <span>Videos {videoAttachmentCount}/{GEMINI_ATTACHMENT_LIMITS.video.maxCount}</span>
                                </button>
                                <button
                                    type="button"
                                    onClick={() => audioInputRef.current?.click()}
                                    disabled={isUploadingAttachments || audioAttachmentCount >= GEMINI_ATTACHMENT_LIMITS.audio.maxCount}
                                ...
                                >
                                    ...
                                    <span>Audio {audioAttachmentCount}/{GEMINI_ATTACHMENT_LIMITS.audio.maxCount}</span>
                                </button>
                                ...
                                <p className="mt-2 text-[10px] leading-4 text-zinc-500">
                                    {GEMINI_ATTACHMENT_LIMITS.image.helper} {GEMINI_ATTACHMENT_LIMITS.video.helper} {GEMINI_ATTACHMENT_LIMITS.audio.helper}
                                </p>
                                {attachmentError && (
                                    ...
                                    >
                                        {attachment.type === 'image' ? (
                                            <div className="flex flex-col">
                                                ...
                                                <div className="flex items-center gap-1 px-2 py-1.5">
                                                    <ImageIcon className="w-3 h-3 shrink-0" />
                                                    <span className="min-w-0 flex-1 truncate">{attachmentLabel(attachment)}</span>
                                                    <button
                                                        type="button"
                                            ...
                                            <div className="inline-flex items-center gap-1 px-2.5 py-1.5">
                                                {attachmentIcon(attachment)}
                                                <span className="max-w-[150px] truncate">{attachmentLabel(attachment)}</span>
                                                <button
                                                    type="button"
                                                    onClick={() => removeAttachment(attachment.id)}
                                                    className="text-zinc-500 hover:text-white"
                        ...
                        <div className="flex items-end gap-2">
                            <textarea
                                ...
                                placeholder={effectiveMode === 'chat' ? `Ask ${isCompactLayout ? 'model' : currentModelLabel}...` : 'Ask assistant...'}
                            ...
                            <AnimatePresence mode="wait">
                                <motion.button
                                    ...
                                    disabled={!prompt.trim() || isAiThinking || isUploadingAttachments}
                    ...
                    <p className="text-center text-[9px] text-zinc-600 mt-2">WildMind AI can make mistakes. Review responses before using them in production prompts.</p>
                </div>
            </div>
        </div>
    );
