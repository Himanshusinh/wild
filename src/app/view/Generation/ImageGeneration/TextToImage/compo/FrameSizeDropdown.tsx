...
const FrameSizeDropdown = ({
  ...
  useEffect(() => {
    const updateDropdownPosition = () => {
      ...
      if (
        ...
        const shouldOpenUp =
          openDirection === "up" ||
          (spaceAbove > spaceBelow && openDirection !== "down");
  ...
  const isIdeogram =
    selectedModel === "ideogram-ai/ideogram-v3" ||
  ...
  const isQwen2 =
    selectedModel === "qwen/qwen-image-2" ||
    selectedModel === "qwen/qwen-image-2-pro";
  ...
  useEffect(() => {
    ...
    const isCurrentFrameSizeSupported = frameSizes.some(
      (size) => size.value === currentFrameSize,
    );
  ...
  const dropdownContent =
    activeDropdown === "frameSize" && isActiveInstance && dropdownPosition ? (
      <div
      ...
      >
        {frameSizes.map((size) => (
          <button
            ...
            className={`w-full px-3 py-2 text-left transition md:text-[13px] text-[11px] flex items-center justify-between gap-3 ${
          ...
          >
            <span className="flex items-center gap-2">
              ...
              {size.value !== "match_input_image" &&
                !(size as any).hideValue && (
                  <span
                    className={`text-[12px] ${frameSize === size.value ? "text-black/70" : "text-white/50"}`}
                  >
                    {size.value}
                  </span>
                )}
    ...
    <>
      <div className="relative dropdown-container">
        <button
          ...
          className={`h-[22px] md:h-[32px] md:px-4 px-2 rounded-lg md:text-[13px] text-[10px] font-medium ring-1 ring-white/20 hover:ring-white/30 transition flex items-center gap-1 ${
            frameSize !== "1:1"
        ...
        >
          ...
          <ChevronUp
            className={`w-4 h-4 transition-transform duration-200 ${activeDropdown === "frameSize" ? "rotate-180" : ""}`}
          />
        </button>
      </div>
