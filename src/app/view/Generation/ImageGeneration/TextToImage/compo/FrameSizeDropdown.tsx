  ...
  return (
    <div className="relative dropdown-container">
      <button
        ...
        className={`h-[32px] px-4 rounded-full text-[13px] font-medium ring-1 ring-black/20 dark:ring-white/20 hover:ring-black/30 dark:hover:ring-white/30 transition flex items-center gap-1 ${frameSize !== '1:1'
      ...
      {activeDropdown === 'frameSize' && (
        <div className={`absolute ${openDirection === 'down' ? 'top-full mt-2' : 'bottom-full mb-2'} left-0 w-44 bg-white/90 dark:bg-black/90 backdrop-blur-xl rounded-xl overflow-hidden ring-1 ring-black/30 dark:ring-white/30 pb-2 pt-2 transition-colors duration-300`}>
          {frameSizes.map((size) => (
            <button
              key={size.value}
              onClick={(e) => {
            ...
            >
              <span className="flex items-center gap-2">
                {/* Icon */}
                {size.icon === 'square' && (
                  <span className={`inline-block w-4 h-4 border ${frameSize === size.value ? 'border-white dark:border-black' : 'border-black/60 dark:border-white/60'
                    }`}></span>
                )}
                {size.icon === 'portrait' && (
                  <span className={`inline-block w-3 h-4 border ${frameSize === size.value ? 'border-white dark:border-black' : 'border-black/60 dark:border-white/60'
                    }`}></span>
                )}
                {size.icon === 'landscape' && (
                  <span className={`inline-block w-4 h-3 border ${frameSize === size.value ? 'border-white dark:border-black' : 'border-black/60 dark:border-white/60'
                    }`}></span>
                )}
                {size.icon === 'ultrawide' && (
                  <span className={`inline-block w-5 h-2 border ${frameSize === size.value ? 'border-white dark:border-black' : 'border-black/60 dark:border-white/60'
                    }`}></span>
                )}
                <span>{size.name}</span>
                <span className={`text-[12px] ${frameSize === size.value ? 'text-white/70 dark:text-black/70' : 'text-black/50 dark:text-white/50'
                  }`}>{size.value}</span>
              </span>
              {frameSize === size.value && <div className="w-2 h-2 bg-white dark:bg-black rounded-full" />}
            </button>
