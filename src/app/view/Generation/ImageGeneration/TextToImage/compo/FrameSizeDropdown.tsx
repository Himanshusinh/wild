...
const FrameSizeDropdown = ({ openDirection = 'up', light = false }: FrameSizeDropdownProps) => {
  ...
  return (
    <div className="relative dropdown-container">
      <button
        ...
        className={`${light ? 'h-[34px] md:h-[36px] px-3 md:px-4 rounded-md text-[13px] font-medium ring-1 ring-gray-200 bg-white text-black hover:bg-gray-50' : 'h-[28px] md:h-[32px] px-2 md:px-4 rounded-lg text-[10px] md:text-[13px] font-medium ring-1 ring-white/20 hover:ring-white/30 transition flex items-center gap-1'} ${frameSize !== '1:1' ? '' : ''}`}
      >
      ...
      {activeDropdown === 'frameSize' && (
        <div className={`${light ? `${openDirection === 'down' ? 'top-full mt-2' : 'bottom-full mb-2'} left-0 w-full md:w-44 bg-white rounded-md ring-1 ring-gray-200 pb-1.5 md:pb-2 pt-1.5 md:pt-2` : `absolute ${openDirection === 'down' ? 'top-full mt-2' : 'bottom-full mb-2'} left-0 w-full md:w-44 bg-black/80 backdrop-blur-xl rounded-xl ring-1 ring-white/30 pb-1.5 md:pb-2 pt-1.5 md:pt-2`}` }>
          {frameSizes.map((size) => (
              <button
              key={size.value}
              onClick={(e) => {
