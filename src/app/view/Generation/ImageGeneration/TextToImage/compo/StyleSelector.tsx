    ...
    <>
      <div className="relative dropdown-container">
        <button
          ...
          className={`h-[28px] md:h-[32px] px-2 md:px-4 rounded-lg text-[10px] md:text-[13px] font-medium ring-1 ring-white/20 hover:ring-white/30 transition flex justify-center items-center gap-1.5 md:gap-2 ${style !== 'none'
              ? 'bg-transparent text-white/90'
              : 'bg-transparent text-white/90 hover:bg-white/5'
            }`}
        >
          <span className="capitalize">{style === 'none' ? 'Style' : style}</span>
          <div className={`w-3 h-3 md:w-4 md:h-4 flex  items-center justify-center ${style !== 'none' ? 'text-white/90' : 'text-white/90'
            }`}>
            <ChevronUp className={`w-3 h-3 md:w-4 md:h-4 transition-transform duration-200 ${isStylePopupOpen ? 'rotate-180' : ''}`} />
          </div>
