    ...
    <>
      <div className="relative dropdown-container">
        ...
        >
          ...
          <ChevronUp className={`w-3 h-3 md:w-4 md:h-4 transition-transform duration-200 ${activeDropdown === 'phoenixOptions' ? 'rotate-180' : ''}`} />
        </button>
            ...
            <div className="max-h-32 overflow-y-auto custom-scrollbar">
              {contrastOptions.map((contrast) => (
                <button
                  ...
                  className={`w-full px-2 md:px-3 py-1.5 md:py-2 text-left transition text-[10px] md:text-[13px] flex items-center justify-between ${phoenixContrast === contrast.value
                      ? 'bg-white text-black'
                      : 'text-white/90 hover:bg-white/10'
                    }`}
                >
                  <span>{contrast.label}</span>
                  {phoenixContrast === contrast.value && <div className="w-1.5 h-1.5 md:w-2 md:h-2 bg-black rounded-full" />}
                </button>
              ))}
            </div>
          </div>
            ...
            <div className="max-h-32 overflow-y-auto custom-scrollbar">
              {modeOptions.map((mode) => (
                <button
                  ...
                  className={`w-full px-2 md:px-3 py-1.5 md:py-2 text-left transition text-[10px] md:text-[13px] flex items-center justify-between ${phoenixMode === mode.value
                      ? 'bg-white text-black'
                      : 'text-white/90 hover:bg-white/10'
                    }`}
                >
                  <span>{mode.label}</span>
                  {phoenixMode === mode.value && <div className="w-1.5 h-1.5 md:w-2 md:h-2 bg-black rounded-full" />}
                </button>
              ))}
            </div>
          </div>
