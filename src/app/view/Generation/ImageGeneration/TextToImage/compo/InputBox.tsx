  ...
  return (
    <>
      ...
      {(historyEntries.length > 0 || localGeneratingEntries.length > 0) && (
        <div ref={scrollRootRef} className=" inset-0  pl-0 pr-1 md:pr-6 pb-6 overflow-y-auto no-scrollbar z-0">
                ...
                {selectedModel === 'seedream-v4' && (
                  <div className="flex items-center gap-2 relative">
                    <div className="relative dropdown-container">
                      ...
                      {activeDropdown === 'seedreamSize' && (
                        <div className={`absolute bottom-full mb-2 w-18 bg-black/80 backdrop-blur-xl shadow-2xl rounded-lg overflow-hidden ring-1 ring-white/30 py-1 z-50`}>
                          {['1K','2K','4K','custom'].map((opt) => (
                            <button
                              key={opt}
                              onClick={(e) => { e.stopPropagation(); setSeedreamSize(opt as any); dispatch(toggleDropdown('')); }}
