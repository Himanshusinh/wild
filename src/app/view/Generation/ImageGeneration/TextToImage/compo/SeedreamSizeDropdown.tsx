...
const SeedreamSizeDropdown = ({ 
  ...
  useEffect(() => {
    const updateDropdownPosition = () => {
      ...
      if (activeDropdown === 'seedreamSize' && isActiveInstance && buttonRef.current) {
    ...
    if (activeDropdown === 'seedreamSize') {
      ...
      const handleClickOutside = (event: MouseEvent) => {
        ...
        if (target.closest('[data-dropdown="seedreamSize"]')) {
          return;
        }
        // Close the dropdown
        setIsActiveInstance(false);
    ...
    >
      {options.map((opt) => (
        <button
          ...
          className={`w-18 md:px-2 px-2 md:py-2 py-1 text-left md:text-[13px] text-[11px] flex items-center justify-between ${size === opt ? 'bg-white text-black' : 'text-white/90 hover:bg-white/10'}`}
        >
          <span>{opt}</span>
          {size === opt && (
...
export default SeedreamSizeDropdown;
