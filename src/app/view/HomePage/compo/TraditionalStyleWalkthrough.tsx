...
import { X, Play, Sparkles, Wand2, Boxes, Palette } from "lucide-react";
import { useRouter } from "next/navigation";

interface TraditionalStyleWalkthroughProps {
  isOpen: boolean;
  onClose: () => void;
  styleId: string;
  styleTitle: string;
...
const TraditionalStyleWalkthrough: React.FC<TraditionalStyleWalkthroughProps> = ({
  isOpen,
  onClose,
  styleId,
  styleTitle,
  ...
  const handleTryIt = () => {
    // Basic prompt construction based on description
    const promptData = {
        promptHard: styleDesc,
        promptArt: `${styleName} style, ${styleDesc}`,
        promptCinematic: `Cinematic 3D render of ${styleName}, ${styleDesc}, detailed textures, professional lighting`
    };
    
    localStorage.setItem("selectedPrompt", JSON.stringify(promptData));
    localStorage.setItem("selectedStyleID", styleId);
    localStorage.setItem("selectedStyleName", styleTitle);
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-[#0E0E12] lg:bg-black/90 backdrop-blur-xl">
            ...
            <div className="space-y-6">
              ...
              <div className="p-6 rounded-2xl bg-white/[0.03] border border-white/5 group hover:border-white/10 transition-all">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {["Cultural Integrity", "Traditional Grammar", "Artistic Balance", "System Logic"].map((rule, idx) => (
...
export default TraditionalStyleWalkthrough;
