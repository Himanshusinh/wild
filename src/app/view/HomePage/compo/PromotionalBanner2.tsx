...
export default function HeroCarousel() {
  ...
  return (
    <section className="relative w-full md:min-h-[650px] max-h-[250px] bg-black text-white overflow-hidden border-b border-white/5 font-sans selection:bg-[#60a5fa] selection:text-white">
      ...
      <div className="absolute md:bottom-8 bottom-4 left-0 right-0 z-30 flex items-center justify-center gap-6">
        ...
        <button onClick={nextSlide} className="md:p-2 p-1 rounded-full border border-white/10 bg-black/50 hover:bg-white hover:text-black transition-all backdrop-blur-md"><ChevronRight size={20} /></button>
      </div>
    </section>
  );
}
...
function PromotionalBannerImage() {
  const desktopSrc = "https://firebasestorage.googleapis.com/v0/b/wild-mind-ai.firebasestorage.app/o/core%2Fbanner-1-dark-theme.jpg?alt=media&token=3cf24d94-3802-4cc8-8bd3-2b641161af9b";
  const mobileSrc = "https://idr01.zata.ai/devstoragev1/users/chinnar/image/g6mlYfzINz7UxvI30qjn/g6mlYfzINz7UxvI30qjn-image-1.png";
  ...
  return (
    <section className="w-full md:px-6 px-4 md:pt-4 pt-6 pb-0 md:pb-6">
      <div className="relative w-full rounded-lg overflow-hidden border border-white/10 group cursor-pointer hover:border-white/20 transition-all duration-300 shadow-lg hover:shadow-xl">
        {/* Mobile banner */}
        <div className="relative w-full aspect-[3/2] md:hidden">
          <Image
            src={mobileSrc}
            alt="WILDMIND AI Launching Offer - Mobile"
            fill
        ...
        {/* Desktop banner */}
        <div className="relative w-full aspect-[3/1] md:aspect-[3.25/1] hidden md:block">
          <Image
            src={desktopSrc}
...
function SlideOneContent() {
  ...
  return (
    ...
    <div className="w-full md:h-full h-[230px] flex items-center justify-center relative">


      {/* Blue/Indigo Glows matching the screenshot */}
      <div className="absolute md:top-[-10%] top-[-5%] md:left-[20%] left-[10%] md:w-[600px] md:h-[600px] w-[230px] h-[250px] bg-blue-600/[0.08] rounded-full blur-[120px]" />
      <div className="absolute md:bottom-[-10%] bottom-[-5%] md:right-[20%] right-[10%] md:w-[500px] md:h-[500px] w-[250px] h-[250px] bg-indigo-600/[0.08] rounded-full blur-[100px]" />

      <div className="max-w-[1400px] mx-auto md:px-6 px-4 md:grid grid-cols-1 lg:grid-cols-2 gap-12 items-center w-full">

        {/* Left: Typography */}
        <div className="md:pt-0 pt-6 flex flex-col items-start text-left animate-in slide-in-from-left-10 duration-700 fade-in">
          <div className="inline-flex items-center gap-2 border border-[#60a5fa]/30 bg-[#60a5fa]/10 rounded-full md:px-4 px-2 md:py-1.5 py-1 md:text-xs text-[8px] font-bold uppercase tracking-widest text-[#60a5fa] md:mb-8 mb-2 shadow-[0_0_15px_rgba(96,165,250,0.1)]">
            <span className="md:w-2 md:h-2 w-1 h-1 rounded-full bg-[#60a5fa] animate-pulse"></span>
            Z-Image Turbo
          </div>
          <h1 className="text-lg md:text-7xl font-semibold tracking-tight text-white text-center md:text-left md:mb-6 mb-2 leading-[1.1] md:px-0 px-1">
          ...
          <p className="text-xs md:text-lg text-slate-400 max-w-xl md:mb-10 mb-2 leading-relaxed md:px-0 px-1">
          ...
          <div className="flex md:gap-4 gap-2 justify-center md:justify-start md:px-0 px-1">
            <button
              onClick={() => router.push('/text-to-image')}
              className="group relative md:px-8 px-2 md:py-4 py-1 bg-white text-black rounded-full font-bold overflow-hidden transition-all hover:scale-105"
            >
              <span className="relative z-10 flex items-center gap-2"><span className="md:block hidden">Start Creating</span> <ArrowUpRight size={18} /></span>
...
function SlideTwoContent() {
  ...
  return (
    <div className="w-full md:h-full h-[230px] flex items-center justify-center relative bg-black">
      <div className="absolute md:top-0 top-[-5%] md:left-1/4 left-[10%] md:w-[500px] md:h-[500px] w-[250px] h-[250px] bg-purple-600/[0.1] rounded-full blur-[100px]" />
      <div className="absolute md:bottom-0 bottom-[-5%] md:right-1/4 right-[10%] md:w-[500px] md:h-[500px] w-[250px] h-[250px] bg-emerald-600/[0.1] rounded-full blur-[100px]" />

      <div className="max-w-[1400px] mx-auto md:px-6 px-4 grid grid-cols-1 md:grid-cols-12 gap-8 items-center w-full z-10 md:pt-0 pt-0">
        <div className="lg:col-span-4 flex flex-col items-start animate-in slide-in-from-bottom-8 duration-700 fade-in">
          <div className="inline-flex items-center gap-2 border border-purple-500/30 bg-purple-500/10 rounded-full md:px-4 px-2 md:py-1.5 py-1 md:text-xs text-[8px] text-center md:text-left font-bold uppercase tracking-widest text-purple-400 md:mb-6 mb-1">
            <Box size={12} /> Model Library
          </div>
          <h1 className="text-xs md:text-5xl font-medium tracking-tight text-white md:mb-6 mb-1 md:px-0 px-1">
            <span>Nano Banana Pro </span> <br />
            <span>Seedream V4.5</span><br />
            <span>Flux 2 Pro & more.</span> <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600">All in One Place.</span>

          </h1>
          <p className="text-xs md:text-lg text-slate-400 md:mb-8 mb-2 md:px-0 px-1  ">
            Why choose WILDMIND AI? <br /> Access the world's most powerful image and video models through a unified API.
          </p>
          <button
            className="flex items-center gap-2 text-white border-b border-[#60a5fa] pb-1 hover:text-[#60a5fa] transition-colors md:px-0 px-1 md:mb-8 mb-2 md:text-sm text-[10px]"
            onClick={() => {
        ...
        <div className="hidden md:grid md:col-span-8 grid-cols-3 gap-4 md:h-[400px]">
...
function SlideThreeContent() {
  return (
    <div className="w-full md:h-full h-[230px] flex items-center justify-center relative bg-black">
      <div className="absolute md:top-1/2 top-[-5%] md:left-1/2 left-[10%] -translate-x-1/2 -translate-y-1/2 md:w-[1000px] md:h-[600px] w-[250px] h-[250px] bg-amber-900/[0.05] rounded-full blur-[120px]" />

      <div className="max-w-[1400px] mx-auto md:px-6 px-4 grid grid-cols-2 lg:grid-cols-2 md:gap-16 gap-2 items-center w-full z-10">
        {/* Visual: Industry Grid (Left Side) */}
        <div className="grid grid-cols-2 md:gap-4 gap-1 animate-in fade-in zoom-in-95 duration-700">
          {[
            ...
            return (
              <div key={i} className={`bg-[#080808] border border-white/5 md:p-2 p-1 rounded-2xl group hover:border-[#60a5fa]/30 transition-colors ${item.offset ? 'md:translate-y-8 translate-y-2' : ''}`}>
                <div className="md:h-48 h-12 rounded-xl overflow-hidden relative md:mb-2 mb-1">
        ...
        <div className="flex flex-col items-start animate-in slide-in-from-right-8 duration-700 fade-in">
          <div className="inline-flex items-center md:gap-2 gap-1 border border-[#60a5fa]/30 bg-[#60a5fa]/10 rounded-full md:px-4 px-2 md:py-1.5 py-1 md:text-xs text-[8px] font-bold uppercase tracking-widest text-[#60a5fa] md:mb-8 mb-2 shadow-[0_0_15px_rgba(96,165,250,0.1)]">
            <span className="md:w-2 md:h-2 w-1 h-1 rounded-full bg-[#60a5fa] animate-pulse"></span>
            For Business
          </div>
          <h1 className="text-sm md:text-7xl font-semibold tracking-tight text-white md:mb-6 mb-2 leading-[1.1] md:px-0 px-1">
          ...
          <p className="text-[8px] md:text-lg text-slate-400 md:mb-8 mb-2 max-w-lg md:px-0 px-1">
            Generate professional-grade assets for e-commerce, advertising, and prototyping.
          </p>
          <div className="grid md:grid-cols-2 grid-cols-1 md:gap-4 gap-1 w-full max-w-md">
            {["Virtual Try-on", "3D Texture Gen", "Fashion Photography", "Brand-Ready Visual Consistency", "Product Photography"].map(f => (
              <div key={f} className="flex items-center md:gap-2 gap-1 md:text-sm text-[8px] text-slate-100"><div className="md:w-1.5 md:h-1.5 w-1 h-1 bg-[#60a5fa] rounded-full"></div> {f}</div>
            ))}
          </div>
        </div>
      </div>
