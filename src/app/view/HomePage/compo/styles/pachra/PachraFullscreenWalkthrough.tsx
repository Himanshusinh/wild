...
interface PachraFullscreenWalkthroughProps {
  isOpen: boolean;
  onClose: () => void;
}

const PachraFullscreenWalkthrough: React.FC<PachraFullscreenWalkthroughProps> = ({ isOpen, onClose }) => {
  const style = STYLES.find((s) => s.id === "pachra");
  return (
    <TraditionalStyleWalkthrough
      isOpen={isOpen}
      onClose={onClose}
      styleId={style?.id || "pachra"}
      styleTitle={style?.title || "Pachra"}
...
export default PachraFullscreenWalkthrough;
