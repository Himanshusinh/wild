...
export async function POST(request: NextRequest) {
  try {
    ...
    // For manual mode, ensure mode is set to "manual"
    if (backendPrompt.mode !== 'manual') {
      return NextResponse.json(
        { error: 'This endpoint is for manual mode only. Set mode to "manual"' },
        { status: 400 }
      );
    }
    ...
    console.log('Manual mode: Calling FAL AI API with compiled payload:', {
    ...
    console.log('Manual mode: FAL AI API response:', falResult);
    ...
    console.error('Error in manual-video API:', error);
