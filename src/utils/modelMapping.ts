...
export const MODEL_MAPPING: ModelMapping[] = [
  ...
  {
    frontendValue: "seedance-2.0-fast-r2v",
    creditModelName: "Bytedance Seedance 2.0 Fast Reference",
    generationType: "video",
    provider: "fal",
    options: {
      resolution: ["480p", "720p"],
  ...
  {
    frontendValue: "seedance-2.0-r2v",
    creditModelName: "Bytedance Seedance 2.0 Reference",
    generationType: "video",
    provider: "fal",
    options: {
      resolution: ["480p", "720p"],
...
export const buildCreditModelName = (
  ...
  if (
    mapping.frontendValue === "MiniMax-Hailuo-02" &&
    options?.resolution &&
    options?.duration
  ) {
  ...
  // Handle MiniMax 2.3 Standard
    ...
    mapping.frontendValue === "MiniMax-Hailuo-2.3" &&
    options?.resolution &&
    options?.duration
  ) {
  ...
  // Handle MiniMax 2.3 Fast
    ...
    mapping.frontendValue === "MiniMax-Hailuo-2.3-Fast" &&
    options?.resolution &&
    options?.duration
  ) {
  ...
  // Handle Runway turbo models with duration
    ...
    (mapping.frontendValue === "gen4_turbo" ||
      mapping.frontendValue === "gen3a_turbo") &&
    options?.duration
  ) {
  ...
  // Handle WAN 2.5 models with duration and resolution
    ...
    mapping.frontendValue.includes("wan-2.5") &&
    options?.duration &&
    options?.resolution
  ) {
  ...
  // Handle Kling models
    ...
    if (
      mapping.frontendValue === "kling-v3-standard" ||
      mapping.frontendValue === "kling-v3-pro"
    ) {
      const hasAudio = (options as any)?.generateAudio !== false;
      const audioSuffix = hasAudio ? " Audio On" : " Audio Off";
      const tierLabel =
        mapping.frontendValue === "kling-v3-pro" ? "Pro" : "Standard";
      ...
      const mode =
        (options as any)?.generationMode === "image-to-video" ? "I2V" : "T2V";
      modelName = `Kling 2.6 Pro ${mode} ${d}s${audioSuffix}`;
    } else if (
      mapping.frontendValue.includes("v2.5") &&
      mapping.frontendValue.includes("t2v")
    ) {
      modelName = `Kling 2.5 Turbo Pro T2V ${d}s`;
    } else if (
      mapping.frontendValue.includes("v2.5") &&
      mapping.frontendValue.includes("i2v")
    ) {
      modelName = `Kling 2.5 Turbo Pro I2V ${d}s`;
    } else if (
      mapping.frontendValue.includes("v2.1") &&
      mapping.frontendValue.includes("t2v")
    ) {
      const res = (options?.resolution || "").toLowerCase().includes("1080")
        ? "1080p"
        : "720p";
  ...
  // Handle Seedance models
  ...
  } else if (mapping.frontendValue === "seedance-2.0-fast-r2v") {
    modelName = "Bytedance Seedance 2.0 Fast Reference";
  } else if (mapping.frontendValue === "seedance-2.0-r2v") {
    modelName = "Bytedance Seedance 2.0 Reference";
  } else if (
    ...
    mapping.frontendValue.includes("seedance") &&
    options?.duration &&
    options?.resolution
  ) {
    const d = options.duration;
    const res = String(options.resolution).toLowerCase();
    const resNormalized = res.includes("480")
      ? "480p"
      : res.includes("720")
        ? "720p"
        : "1080p";
    ...
    const tier = mapping.frontendValue.includes("lite")
      ? "Lite"
      : isProFast
        ? "Pro Fast"
        : "Pro";
  ...
  // Handle Google nano banana pro with resolution
    ...
    mapping.frontendValue === "google/nano-banana-pro" &&
    options?.resolution
  ) {
    const rawRes = String(options.resolution).toUpperCase();
    const res =
      rawRes === "4K" ||
      rawRes.includes("AUTO_4K") ||
      rawRes.includes("2160") ||
      rawRes.includes("4096")
        ? "4K"
        : rawRes === "1K" || rawRes.includes("1024")
          ? "1K"
          : "2K";
  ...
  // Handle Crystal Upscaler with resolution
    ...
    mapping.frontendValue === "replicate-crystal-upscaler" &&
    options?.resolution
  ) {
  ...
  // Handle SeedVR2 with duration and resolution
    ...
    mapping.frontendValue === "seedvr2" &&
    options?.duration &&
    options?.resolution
  ) {
    const d = options.duration;
    const res = String(options.resolution).toLowerCase();
    const resNormalized = res.includes("2k")
      ? "2k"
      : res.includes("1080")
        ? "1080p"
        : "720p";
  ...
  // Handle GPT Image 1.5 with quality
    ...
    mapping.frontendValue === "openai/gpt-image-1.5" &&
    options?.quality
  ) {
  ...
  // Handle FLUX.2 Pro with resolution
    ...
    mapping.frontendValue === "flux-2-pro-1080p" ||
    mapping.frontendValue === "flux-2-pro-2k"
  ) {
  ...
  // Handle Flux 2 Pro with resolution and I2I/T2I
    ...
    const hasImages =
      ...
      (options as any).uploadedImages?.length > 0;
