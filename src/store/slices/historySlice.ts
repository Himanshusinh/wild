...
const normalizeGenerationTypeValue = (t?: string) =>
  t ? String(t).replace(/[_-]/g, "-").toLowerCase() : "";

const isVideoGenerationTypeValue = (t?: string) =>
  [
    "text-to-video",
    "image-to-video",
    "video-to-video",
    "video",
    "video-generation",
  ].includes(normalizeGenerationTypeValue(t));

const expectedTypeMatchesCurrent = (expected?: string, current?: string) => {
  const normalizedExpected = normalizeGenerationTypeValue(expected);
  const normalizedCurrent = normalizeGenerationTypeValue(
    current === "image-to-image" ? "text-to-image" : current,
  );

  if (!normalizedExpected) return true;
  if (normalizedExpected === normalizedCurrent) return true;
  if (
    normalizedExpected === "video" &&
    isVideoGenerationTypeValue(normalizedCurrent)
  )
    return true;
  if (normalizedExpected === "logo" && normalizedCurrent === "logo-generation")
    return true;
  if (normalizedExpected === "logo-generation" && normalizedCurrent === "logo")
    return true;
...
const mapGenerationTypeForBackend = (
  ...
  if (Array.isArray(type)) {
    return type
      .map((t) => mapGenerationTypeForBackend(t) as string)
      .filter(Boolean);
  }
  ...
  async (
    {
      ...
      filters?: HistoryFilters;
      backendFilters?: HistoryFilters;
      paginationParams?: PaginationParams;
      requestOrigin?: "central" | "page" | "character-modal";
      expectedType?: string;
      debugTag?: string;
    ...
    try {
      ...
      const uiType: string =
        (state && state.ui && state.ui.currentGenerationType) ||
        "text-to-image";
      const uiView: string =
        (state && state.ui && state.ui.currentView) || "home";
      const currentType = normalizeGenerationTypeValue(
        uiType === "image-to-image" ? "text-to-image" : uiType,
      );
      const expected = normalizeGenerationTypeValue(expectedType);
      ...
      const isGlobalHistoryView =
        uiView === "history" || uiView === "bookmarks";
      const expectedMatches =
        expectedTypeMatchesCurrent(expected, currentType) ||
        isCharacterModal ||
        isGlobalHistoryView;
      const bypassExpectedTypeCheck = forceRefresh === true;
      if (!expectedMatches && !bypassExpectedTypeCheck) {
      ...
      const canonicalAudioType = (
        ...
        const normalized = arr.map((t) => {
          ...
          if (
            ...
            return "sfx";
          if (n === "dialogue" || n === "text_to_dialogue")
      ...
      if (
        ...
        typeof (filtersForBackend as any).search === "string" &&
        (filtersForBackend as any).search.trim()
      ) {
      ...
      if (
        (filtersForBackend as any)?.dateRange &&
        (filtersForBackend as any).dateRange.start &&
        (filtersForBackend as any).dateRange.end
      ) {
        const dr = (filtersForBackend as any).dateRange as any;
        (params as any).dateStart =
          typeof dr.start === "string"
            ? dr.start
            : new Date(dr.start).toISOString();
        (params as any).dateEnd =
          typeof dr.end === "string" ? dr.end : new Date(dr.end).toISOString();
      }
      ...
      if (
        ...
        (typeof error?.message === "string" &&
          error.message === "__CONDITION_ABORT__")
      ) {
        return rejectWithValue("__CONDITION_ABORT__");
      }
    ...
    condition: (args = {} as any, { getState }) => {
      try {
        ...
        if ((args as any)?.forceRefresh === true) {
          console.log(
            "[HistorySlice] forceRefresh=true - bypassing condition check, allowing API call",
          );
          return true;
        }
        ...
        const expected = normalizeGenerationTypeValue(
          (args as any)?.expectedType,
        );
        const origin = (args as any)?.requestOrigin;
        const debugTag = (args as any)?.debugTag;
        const fType = normalizeGenerationTypeValue(
          (args as any)?.filters?.generationType,
        );
        ...
        if (origin === "central") {
          ...
          if (fType) {
            if (
              fType !== currentType &&
              !(fType === "logo" && currentType === "logo-generation") &&
              !(fType === "logo-generation" && currentType === "logo")
            ) {
              return false;
            }
          }
          // If filter is a video mode, ensure UI is on a video type
          if (fMode === "video" && !isVideoGenerationTypeValue(currentType)) {
            return false;
          }
        }
        return true;
...
export const loadMoreHistory = createAsyncThunk(
  ...
  async (
    ...
    try {
      console.log(
        "[HistorySlice] ========== loadMoreHistory CALLED ==========",
      );
      console.log("[HistorySlice] Request params:", {
        filters,
        paginationParams,
      ...
      if (currentEntries.length > 0) {
        ...
        const typeMatches = (
          ...
          if (
            (f === "logo" && e === "logo-generation") ||
            (f === "logo-generation" && e === "logo")
          )
            return true;
          if (
            (f === "sticker-generation" && e === "sticker") ||
            (f === "sticker" && e === "sticker-generation")
          )
            return true;
          if (
            (f === "product-generation" && e === "product") ||
            (f === "product" && e === "product-generation")
          )
            return true;
          return false;
        };
        const matchesFilters = (entry: any): boolean => {
          ...
          if (filters?.generationType) {
            ...
            if (Array.isArray(filterType)) {
              ...
              if (
                !filterType.some((ft) => typeMatches(entry.generationType, ft))
              ) {
                return false;
              }
            } else {
      ...
      console.log(
        "[HistorySlice] Making loadMoreHistory API call with params:",
        params,
      );
...
const historySlice = createSlice({
  ...
  reducers: {
    ...
    updateHistoryEntry: (
      state,
      action: PayloadAction<{ id: string; updates: Partial<HistoryEntry> }>,
    ) => {
    ...
    clearHistoryByType: (state, action) => {
      ...
      if (
        ...
        normalizedPayload === "video-generation"
      ) {
      ...
      state.entries = state.entries.filter((entry) => {
        const normalizedEntryType = normalizeGenerationType(
    ...
    builder
      ...
      .addCase(loadHistory.pending, (state, action) => {
        ...
        try {
          ...
          state.currentRequestKey = JSON.stringify({
            type: "load",
            filters: pendingArg?.filters || {},
            limit: pendingArg?.paginationParams?.limit || 10,
            cursor: pendingArg?.paginationParams?.cursor?.id,
          });
        } catch {
          state.currentRequestKey = "unknown";
        }
        state.error = null;
      ...
      .addCase(loadHistory.fulfilled, (state, action) => {
        ...
        const incomingFilters: any =
          (action.meta as any)?.arg?.filters ||
          (action.meta as any)?.arg?.backendFilters ||
          {};
        ...
        const requestedLimit =
          (action.meta &&
            action.meta.arg &&
            action.meta.arg.paginationParams &&
            action.meta.arg.paginationParams.limit) ||
          10;
        ...
        console.log(
          "[HistorySlice] Sample entries (first 3):",
          action.payload?.entries?.slice(0, 3).map((e: any) => ({
            id: e.id,
        ...
        if (forceRefresh) {
          console.log(
            "[HistorySlice] forceRefresh=true - REPLACING all entries with fresh data",
          );
          state.entries = action.payload.entries;
        } else {
          const existingMap = new Map<string, any>(
            state.entries.map((e: any) => [String(e?.id || ""), e]),
          );
          const backendEntries: any[] = Array.isArray(action.payload.entries)
            ? action.payload.entries
            : [];
          const backendIds = new Set<string>(
            backendEntries.map((e: any) => String(e?.id || "")),
          );
          ...
          const mergedInOrder = backendEntries.map((backendEntry: any) => {
            ...
            const existingTimestamp = new Date(
              existingEntry.timestamp || existingEntry.createdAt || 0,
            ).getTime();
            const backendTimestamp = new Date(
              backendEntry.timestamp || backendEntry.createdAt || 0,
            ).getTime();
            if (
              ...
              (existingEntry.status !== "generating" &&
                backendEntry.status === "generating") ||
              (Array.isArray(existingEntry.audios) &&
                existingEntry.audios.length > 0 &&
                (!Array.isArray(backendEntry.audios) ||
                  backendEntry.audios.length === 0))
            ) {
        ...
        if (usedTypeAny) {
          ...
          const typeMatches = (eType?: string, fType?: string): boolean => {
            ...
            if (f === "text-to-character" && e === "text-to-character")
              return true;
            return false;
          };
          const usedTypesArr = Array.isArray(usedTypeAny)
            ? usedTypeAny
            : [usedTypeAny];
          const isTtsRequest = usedTypesArr
            .map(normalize)
            .some((t: string) =>
              ["text-to-speech", "text_to_speech", "tts"].includes(t),
            );
          const isSfxRequest = usedTypesArr
            .map(normalize)
            .some((t: string) =>
              [
                "sfx",
          ...
          const isDialogueRequest = usedTypesArr
            .map(normalize)
            .some((t: string) =>
              ["text-to-dialogue", "dialogue", "text_to_dialogue"].includes(t),
            );
          state.entries = state.entries.filter((it: any) => {
            // Standard type match
            if (
              usedTypesArr.some((f: string) =>
                typeMatches(it?.generationType, f),
              )
            )
              return true;
            ...
            if (isTtsRequest) {
              const model =
                normalize(it?.model) ||
                normalize((it as any)?.backendModel) ||
                normalize((it as any)?.apiModel) ||
                "";
              const hasAudioData =
                (Array.isArray((it as any)?.audios) &&
                  (it as any).audios.length > 0) ||
                !!(it as any)?.audio ||
                (Array.isArray((it as any)?.images) &&
                  (it as any).images.some((img: any) => img?.type === "audio"));
              if (
            ...
            if (isSfxRequest) {
              ...
              const hasAudioData =
                ...
                (Array.isArray((it as any)?.images) &&
                  (it as any).images.some((img: any) => img?.type === "audio"));
              if (
                (model.includes("sound-effects") ||
                  model.includes("sound_effects") ||
                  model.includes("sfx")) &&
                hasAudioData
              )
                return true;
            ...
            if (isDialogueRequest) {
              ...
              const hasAudioData =
                ...
                (Array.isArray((it as any)?.images) &&
                  (it as any).images.some((img: any) => img?.type === "audio"));
              if (
                (model.includes("dialogue") ||
                  model.includes("conversation")) &&
                hasAudioData
              )
                return true;
            }
        ...
        console.log(
          "[HistorySlice] State updated after fulfilled (after filtering):",
          {
      ...
      .addCase(loadMoreHistory.fulfilled, (state, action) => {
        console.log(
          "[HistorySlice] ========== loadMoreHistory.fulfilled ==========",
        );
        console.log("[HistorySlice] Payload received:", {
          entriesCount: action.payload?.entries?.length || 0,
          hasMore: action.payload?.hasMore,
        ...
        let newEntries = action.payload.entries.filter(
          (newEntry: HistoryEntry) =>
            !state.entries.some(
              (existingEntry: HistoryEntry) => existingEntry.id === newEntry.id,
            ),
        );
        ...
        const requestBackendFilters =
          (action.meta as any)?.arg?.backendFilters || {};
        const requestedLimit =
          (action.meta as any)?.arg?.paginationParams?.limit || 10;
        const hasModeFilter = !!(
          requestFilters?.mode || requestBackendFilters?.mode
        );
        ...
        if (!hasModeFilter) {
          const usedTypeAny = (requestFilters?.generationType ||
            requestBackendFilters?.generationType ||
            state.filters?.generationType) as any;
          if (usedTypeAny) {
            const normalize = (t?: string): string =>
              t ? String(t).replace(/[_-]/g, "-").toLowerCase() : "";
            const typeMatches = (eType?: string, fType?: string): boolean => {
            ...
            const isDialogueRequest = usedTypesArr
              .map(normalize)
              .some((t: string) =>
                ["text-to-dialogue", "dialogue", "text_to_dialogue"].includes(
                  t,
                ),
              );
            newEntries = newEntries.filter((it: any) => {
              ...
              if (isTtsRequest) {
                ...
                const hasAudioData =
                  ...
                  !!(it as any)?.audio ||
                  (Array.isArray((it as any)?.images) &&
                    (it as any).images.some(
                      (img: any) => img?.type === "audio",
                    ));
                if (
                  (model.includes("chatterbox") ||
                    model.includes("eleven") ||
        ...
        if (payloadEntries.length === 0 && !hasNextCursor) {
          ...
          if (
            backendCursorStr &&
            requestCursorStr &&
            backendCursorStr === requestCursorStr
          ) {
            console.warn(
