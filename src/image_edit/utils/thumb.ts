 ...
 * We route through the app's same-origin proxy route which handles CORS headers correctly for canvas.
 *
 * Important: This must be same-origin (relative URL) so the editor doesn't accidentally
 * send requests to the API gateway (e.g. localhost:5000) which may not expose /api/proxy/media.
 */
export function toMediaProxy(urlOrPath: string): string {
    const path = toZataPath(urlOrPath);

    const MEDIA_PROXY_PREFIX = '/api/proxy/media/';

    // If it's a Zata path, proxy it
    if (path) {
        return `${MEDIA_PROXY_PREFIX}${encodeURIComponent(path)}`;
    }

    // If it's an external URL (http/https), we can also proxy it if needed for CORS
    if (urlOrPath.startsWith('http')) {
        return `${MEDIA_PROXY_PREFIX}${encodeURIComponent(urlOrPath)}`;
    }
