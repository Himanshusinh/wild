...
CATEGORIES.forEach(cat => {
    ...
    // Filter Data
    const categoryData = cat === "All"
        ? WORKFLOWS_DATA
        : WORKFLOWS_DATA.filter(wf => wf.category === cat);

    // Write data.js
    const dataContent = `export const DATA = ${JSON.stringify(categoryData, null, 2)};`;
...
`;
    ...
    console.log(`Generated content for ${cat}`);
});
