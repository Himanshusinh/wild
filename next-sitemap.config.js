...
try {
  ...
  console.log(`✅ Loaded ${blogPosts.length} blog posts from sitemap data file`);
} catch (jsonError) {
  ...
  try {
    ...
    console.log(`✅ Loaded ${blogPosts.length} blog posts from TypeScript file (fallback)`);
  } catch (tsError) {
    console.error('❌ Error loading blog posts for sitemap:', tsError.message);
    console.warn('⚠️  Sitemap will be generated without blog posts.');
    console.warn('💡 Run "npm run prebuild" to generate sitemap data file.');
    blogPosts = [];
  }
}
